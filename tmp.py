...
from qiskit.providers.fake_provider import FakeAthens, FakeAthensV2, FakeVigoV2, FakeAlmaden, FakeAlmadenV2
...
class GenericTarget(Target):
    ...
    def __init__(
        ...
        for name in self._basis_gates:
            ...
            self.add_instruction(instruction, props, instruction.name)

        # backend1 = FakeGeneva()
        # for op_name, obj in backend1.target._gate_map.items():
        #     print(backend1.target._gate_name_map[op_name], obj)
        #     self.add_instruction(backend1.target._gate_name_map[op_name], obj, op_name)
    ...
    def noise_defaults(self) -> dict[str, tuple | None]:
        ...
        return {
            ...
            "delay": None,
            "reset": (1.447111111111111e-06, None),
        }
...
backend = FakeGeneric(
    num_qubits=27, basis_gates = ['x', 'sx', 'cx', 'id', 'rz'], coupling_map=FakeGeneva().coupling_map
)
noise_model = NoiseModel.from_backend(backend)
backend = AerSimulator()
backend.set_options(noise_model=noise_model)

result = backend.run(qc, seed_simulator=4242424242, shots=1000).result()
print("3")
