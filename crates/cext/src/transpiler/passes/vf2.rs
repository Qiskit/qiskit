...
/// QkVF2LayoutConfiguration *config = qk_vf2_layout_configuration_new();
/// qk_vf2_layout_configuration_set_call_limit(config, 10000, 10000);
/// QkVF2LayoutResult *layout_result = qk_dag_transpiler_pass_standalone_vf2_layout_average(dag, target, config, false);
/// qk_vf2_layout_result_free(layout_result);
/// qk_vf2_layout_configuration_free(config);
/// ```
///
...
#[unsafe(no_mangle)]
#[cfg(feature = "cbinding")]
pub unsafe extern "C" fn qk_dag_transpiler_pass_standalone_vf2_layout_average(
...
/// QkVF2LayoutConfiguration *config = qk_vf2_layout_configuration_new();
/// qk_vf2_layout_configuration_set_call_limit(config, 10000, 10000);
/// QkVF2LayoutResult *layout_result = qk_dag_transpiler_pass_standalone_vf2_layout_exact(dag, target, config);
/// qk_vf2_layout_result_free(layout_result);
/// qk_vf2_layout_configuration_free(config);
/// ```
///
...
#[unsafe(no_mangle)]
#[cfg(feature = "cbinding")]
pub unsafe extern "C" fn qk_dag_transpiler_pass_standalone_vf2_layout_exact(
    dag: *const DAGCircuit,
    target: *const Target,
    config: *const VF2LayoutConfiguration,
) -> *mut VF2LayoutResult {
...
/// @ingroup QkTranspilerPasses
/// Deprecated version of `qk_dag_transpiler_pass_standalone_vf2_layout_average`.
///
/// This legacy interface does not use `QkVf2LayoutConfiguration`, and has a name that is not clear
/// about how it handles the error heuristic (it averages over all gates in the `QkTarget` for a
/// given qubit or link).
///
/// \qk_deprecated{2.3.0|Replaced by :c:func:`qk_dag_transpiler_pass_standalone_vf2_layout_average`.}
///
/// @param dag As in `qk_dag_transpiler_pass_standalone_vf2_layout_average`.
/// @param target As in `qk_dag_transpiler_pass_standalone_vf2_layout_average`.
/// @param strict_direction As in `qk_dag_transpiler_pass_standalone_vf2_layout_average`.
/// @param call_limit As in `qk_vf2_layout_configuration_set_call_limit`, but the same value is used
///     for both `before` and `after`.
/// @param time_limit As in `qk_vf2_layout_configuration_set_time_limit`.
/// @param max_trials As in `qk_vf2_layout_configuration_set_max_trials`.
///
/// @return As in `qk_dag_transpiler_pass_standalone_vf2_layout_average`.
///
/// # Safety
///
/// The safety requirements of `qk_dag_transpiler_pass_standalone_vf2_layout_average` must be respected
/// for `circuit` and `target`.
#[deprecated(
    since = "2.3.0",
    note = "use `qk_dag_transpiler_pass_standalone_vf2_layout_average` instead"
)]
#[unsafe(no_mangle)]
#[cfg(feature = "cbinding")]
pub unsafe extern "C" fn qk_dag_transpiler_pass_standalone_vf2_layout(
    dag: *const DAGCircuit,
    target: *const Target,
    strict_direction: bool,
    call_limit: i64,
    ...
    // is constructed in safe code and lasts for the duration of this function.
    unsafe {
        qk_dag_transpiler_pass_standalone_vf2_layout_average(dag, target, &config, strict_direction)
    }
}
