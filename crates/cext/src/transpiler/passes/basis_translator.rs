...
use qiskit_transpiler::libraries::standard_equivalence_library::get_standard_equivalence_library;
...
pub unsafe extern "C" fn qk_transpiler_pass_standalone_basis_translator(
    ...
    let qargs_with_non_global_ops = {
        let mut map: HashMap<Qargs, HashSet<String>> = HashMap::default();
        let names = target
        ...
        for gate in names {
            for qarg in target[&gate].keys() {
                if let Some(val) = map.get_mut(qarg) {
                    val.insert(gate.clone());
                } else {
                    map.insert(qarg.clone(), HashSet::from_iter([gate.clone()]));
                }
            }
        }
        map
    ...
    let mut equiv_lib = get_standard_equivalence_library().clone();

    let result_dag = match run_basis_translator(
        dag,
        &mut equiv_lib,
        qargs_with_non_global_ops,
        min_qubits,
        None,
        Some(target),
        None,
