 ...
 * @ingroup QkCircuitMethods
 *
 * Standard quantum circuit gates.
 */
enum QkGate
#ifdef __cplusplus
    : uint8_t
 ...
 * routine that can be run, one shot at a time, on a quantum processing unit (QPU).  A circuit will
 * act on a predefined amount of quantum data (in Qiskit, we only directly support qubits) with
 * unitary operations (gates), measurements and resets.  In addition, a quantum circuit can contain
 * operations on classical data, including real-time computations and control-flow constructs,
 * which are executed by the controllers of the QPU. The ``QkCircuit`` struct  exposes a low level
 * interface to Qiskit's quantum circuit data structure and exposes only what is defined in the
 * inner data model of Qiskit. Therefore it is missing some functionality that is available in the
 * higher level Python :class:`.QuantumCircuit` class.
 *
 * Below is an example of a quantum circuit that makes a three-qubit Greenberger–Horne–Zeilinger
 * (GHZ) state defined as:
 *
 * .. math::
 *
 ...
 */
typedef struct {
    /* A array of size ``len`` containing ``OpCount`` objects for each type of operation in the
     * circuit. */
    QkOpCount *data;
 ...
 * Behavior is undefined if ``circuit`` is not a valid, non-null pointer to a ``QkCircuit``.
 */
QkExitCode qk_circuit_barrier(QkCircuit *circuit, uint32_t num_qubits, const uint32_t *qubits);
