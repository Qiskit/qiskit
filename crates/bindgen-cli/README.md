...
You can compare two listings of slots (as generated by `qiskit-bindgen-cli show-slots`) for ABI
compatibility using the `check-abi` command, such as

```bash
cargo run -p qiskit-bindgen-cli -- check-abi previous_slots.txt [new_slots.txt]
```

If a "new" file is not given, the current working vtables are compared instead (which is most likely
usually what you want).  The ABI versions are included in the slots listings.  If the working slots
are used, the ABI version is read from the Cargo metadata.

The comparison is semver-aware: if the version-number change corresponds to:

* a major version, no warnings are emitted since all breakages are allowed;
* a minor version, all filled slots in the previous version must be valid with the same names at the
  same offsets the new version, but filling in more slots is allowed;
* a patch version, no changes are permitted.

Note that this check doesn't account for potential changes in the types (or behaviours!) of
functions; those are not encoded in the slots lists.
