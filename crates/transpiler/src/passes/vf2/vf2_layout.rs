...
struct VirtualInteractions<T> {
    ...
    /// The qubits that have only single-qubit operations on them, mapped to the
    /// interaction summary associated with them.  We iterate through this at the end, so need a
    /// consistent order.
    free: IndexMap<VirtualQubit, T>,
}
impl<T: Default> VirtualInteractions<T> {
        ...
        W: Fn(&mut T, &PackedInstruction, usize) -> bool,
    {
        for (_, inst) in dag.op_nodes(false) {
            ...
            if inst.op.control_flow() {
                let can_continue = Python::with_gil(|py| -> PyResult<bool> {
            ...
            match qubits {
                ...
                [q] => {
                    ...
                    if let Some(index) = self.nodes.get_index_of(&q) {
                        ...
                        let weight = self.free.entry(q).or_default();
    ...
    fn ensure_1q_in_graph(&mut self, q: VirtualQubit) -> NodeIndex {
        ...
        self.graph
            .add_node(self.free.swap_remove(&q).unwrap_or_default())
    }

    /// Move all the free qubits into the graph.
    ///
    ...
    fn move_all_free_to_graph(&mut self) {
        for (qubit, interactions) in self.free.drain(..) {
...
fn map_free_qubits(
    ...
    if interactions.free.is_empty() {
        return Some(partial_layout);
    }
    if num_physical_qubits < partial_layout.len() + interactions.free.len() {
        return None;
    }
    ...
    let mut free_virtual = interactions.free.into_iter().collect::<Vec<_>>();
    free_virtual.par_sort_by_key(|(_, interactions)| *interactions);

    // The best remaining physical qubits are first, the most used virtual qubits are last.
    for ((virt, _), phys) in free_virtual.into_iter().rev().zip(free_physical) {
        partial_layout.insert(virt, phys);
    }
    Some(partial_layout)
}
...
pub fn vf2_layout_pass_exact(
    ...
    interactions.move_all_free_to_graph();
    ...
    let best_score = if config.score_initial_layout {
        score_identity_layout(&interactions.graph, &coupling_graph, score)
    } else {
        None
    };
    let num_physical_qubits = coupling_graph.node_count();
