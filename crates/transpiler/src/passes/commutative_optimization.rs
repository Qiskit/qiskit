...
fn canonicalize(
    ...
    let rotation = match inst.op.view() {
        ...
        | OperationRef::StandardGate(StandardGate::U1) => Some((
            ...
            inst.params_view()[0].mul_f64(0.5),
        )),
...
fn try_merge(
        ...
        (inst1.op.view(), inst2.op.view())
    {
        ...
        if gate1 == gate2 && MERGEABLE_ROTATION_GATES.contains(&gate1) {
            let merged_param = params1[0].add_scalar(&params2[0]);
    ...
    for idx1 in 0..num_nodes {
        ...
        for idx2 in (0..idx1).rev() {
            ...
            if can_be_merged {
                ...
                new_global_phase = new_global_phase
                    .add_f64(phase_update)
                    .add_scalar(&extraphase1)
                    .add_scalar(&extraphase2);
                modified = true;
                break;
            }
