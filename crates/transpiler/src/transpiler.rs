...
fn trivial_layout(dag: &DAGCircuit) -> NLayout {
    NLayout::generate_trivial_layout(dag.num_qubits() as u32)
}
...
pub fn transpile(
    ...
    let mut transpile_layout: TranspileLayout = TranspileLayout::new_empty(dag.qubits().clone());
    ...
    if optimization_level == 1 {
        ...
        if let Some(result) = result {
            dag = result.0;
            let split_2q_permutation = result.1;
