...
impl PyTarget {
    ...
    #[pyo3(signature = (instruction, name=None, properties=None, *, angle_bounds=None))]
    fn add_instruction(
        &mut self,
        py: Python,
        instruction: TargetOperation,
    ...
    fn __setstate__(&mut self, state: Bound<PyDict>) -> PyResult<()> {
        ...
        let mut gate_map: IndexMap<String, TargetProperties> = IndexMap::default();
        ...
        for (gate, bounds) in bounds_map {
            self.inner
                .add_owned_angle_bound(gate, bounds)
