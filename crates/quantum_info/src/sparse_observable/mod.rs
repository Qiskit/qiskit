...
/// not allowing the stored values to be outside the valid `BitTerm` objects, but doing so in Python would
...
/// `BitTerm` objects.  For interop with Python space, we accept Numpy arrays of `u8` to represent this,
