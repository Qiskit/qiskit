...
/// not allowing the stored values to be outside the valid `BitTerm` instances, but doing so in Python would
...
/// `BitTerm` instances.  For interop with Python space, we accept Numpy arrays of `u8` to represent this,
...
///     with :class:¬SparseObservable` instances and the :class:`BitTerm` name is not ambiguous, you
