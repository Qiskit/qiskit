...
impl SparseObservable {
    ...
    pub fn to_matrix_old(&self) -> Result<Array2<Complex64>, MatrixError> {
        ...
        for (start_from_col, row) in matrix.chunks_mut(dim).enumerate() {
            self.to_matrix_row(start_from_col, row);
        }

        let matrix = Array2::from_shape_vec((dim, dim), matrix).expect("len fits in shape");
        Ok(matrix)
    }

    fn to_matrix_row(&self, start_from_col: usize, row: &mut [Complex64]) {
        for term in self.iter() {
            let mut coeff = MatrixCoeff {
                value: term.coeff,
                col: start_from_col,
                is_discarded: false,
            };

            let masks = term.indices.iter().map(|block| 1usize << block);
            for (&operation, mask) in term.bit_terms.iter().zip(masks) {
                accumulate_matrix_coeff(&mut coeff, operation, mask);
            }

            if !coeff.is_discarded {
                row[coeff.col] += coeff.value;
            }
        }
    }
}
...
fn accumulate_matrix_coeff(coeff: &mut MatrixCoeff, operation: BitTerm, mask: usize) {
    let is_col_on = || coeff.col & mask != 0;
    let flip_col = || coeff.col ^ mask;

    match operation {
        BitTerm::X => {
            coeff.col = flip_col();
        }
        BitTerm::Y if is_col_on() => {
            coeff.value *= Complex64::i();
            coeff.col = flip_col();
        }
        BitTerm::Y => {
            coeff.value *= -Complex64::i();
        ...
        BitTerm::Z if is_col_on() => {
            coeff.value = -coeff.value;
        }
        BitTerm::Plus | BitTerm::Minus => {
            coeff.value /= Complex64::from(2.0_f64.sqrt());
        }
        BitTerm::Right if is_col_on() => {
            coeff.value *= Complex64::i();
        }
        BitTerm::Right => {
            coeff.value *= -Complex64::i();
        }
        BitTerm::Left if is_col_on() => {
            coeff.value *= -Complex64::i();
        }
        BitTerm::Left => {
            coeff.value *= Complex64::i();
        }
        BitTerm::Zero if is_col_on() => {
            coeff.is_discarded = true;
            return;
        }
        BitTerm::One if !is_col_on() => {
            coeff.is_discarded = true;
            return;
        }
        _ => (),
