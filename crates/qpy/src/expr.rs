...
use qiskit_circuit::classical::expr::{Cast, Expr, Index, Unary, Value, Var, Binary, UnaryOp, BinaryOp};
use qiskit_circuit::classical::types::Type;
use qiskit_circuit::Clbit;
use qiskit_circuit::duration::Duration;
use std::io::{Read, Seek, Write};
use crate::formats::{ExpressionDurationPack, ExpressionElementPack, ExpressionTypePack, ExpressionValueElementPack, ExpressionVarElementPack, ExpressionVarRegisterPack, pack_biguint, unpack_biguint};
use crate::value::{QPYWriteData, QPYReadData};
...
pub fn pack_expression_var(var: &Var, qpy_data: &QPYWriteData) -> ExpressionElementPack {
    let (ty, value_pack) = match var {
        Var::Bit { bit } => {
            (&Type::Bool, ExpressionVarElementPack::Clbit(qpy_data.clbits.find(bit).unwrap().0))
        }
        Var::Register { register, ty } => (ty, ExpressionVarElementPack::Register(ExpressionVarRegisterPack{name: register.name().to_string()})),
        Var::Standalone { uuid, name: _, ty } => (ty, ExpressionVarElementPack::UUID(*qpy_data.standalone_var_indices.get(uuid).unwrap()))
    };
    ExpressionElementPack::Var(pack_expression_type(ty), value_pack)
}

pub fn unpack_expression_var(var_type_pack: ExpressionTypePack, var_element_pack: ExpressionVarElementPack, qpy_data: &QPYReadData) -> Var {
    let ty = unpack_expression_type(var_type_pack);
    match var_element_pack {
        ExpressionVarElementPack::Clbit(index) => Var::Bit { bit: qpy_data.circuit_data.clbits().get(Clbit(index)).unwrap().clone() }, // TODO: error handling?
        ExpressionVarElementPack::Register(packed_register) => Var::Register {register: qpy_data.circuit_data.cregs_data().get(packed_register.name.as_str()).unwrap().clone(), ty }, // TODO: can we avoid cloning?
...
pub fn write_expression<W: Write + Seek>(
    ...
    match exp {
        ...
        Expr::Stretch(stretch) => {
            ExpressionElementPack::Stretch(ExpressionTypePack::Duration, qpy_data.standalone_var_indices[&stretch.uuid]).write_options(writer, endian, ())?;
        }
        Expr::Index(index_node) => {
            ExpressionElementPack::Index(pack_expression_type(&index_node.ty)).write_options(writer, endian, ())?;
            write_expression(&index_node.target, writer, endian, (qpy_data,))?;
            write_expression(&index_node.index, writer, endian, (qpy_data,))?;
        }
        Expr::Cast(cast_node) => {
            ExpressionElementPack::Cast(pack_expression_type(&cast_node.ty), cast_node.implicit as u8).write_options(writer, endian, ())?;
            write_expression(&cast_node.operand, writer, endian, (qpy_data,))?;
        }
        Expr::Unary(unary_node) => {
            ExpressionElementPack::Unary(pack_expression_type(&unary_node.ty), unary_node.op as u8).write_options(writer, endian, ())?;
            write_expression(&unary_node.operand, writer, endian, (qpy_data,))?;
        }
        Expr::Binary(binary_node) => {
            ExpressionElementPack::Binary(pack_expression_type(&binary_node.ty), binary_node.op as u8).write_options(writer, endian, ())?;
...
pub fn read_expression<R: Read + Seek>(
    ...
    match exp_element {
        ...
        ExpressionElementPack::Var(var_type_pack, var_element_pack) => Ok(Expr::Var(unpack_expression_var(var_type_pack, var_element_pack, qpy_data))),
        ExpressionElementPack::Stretch(_stretch_type_pack, key ) => {
            let stretch = qpy_data.standalone_stretches.get(&key).unwrap();
            Ok(Expr::Stretch(qpy_data.circuit_data.get_stretch(*stretch).unwrap().clone())) // TODO: can we avoid cloning?
        }
