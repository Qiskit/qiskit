...
fn is_python_gate(py: Python, op: &PackedOperation, python_gate: &Bound<PyAny>) -> Result<bool, QpyError> {
    match op.view() {
...
pub(crate) fn get_python_gate_class<'a>(
    ...
    if library.hasattr(gate_class_name)? {
        ...
        control_flow.getattr(gate_class_name).map_err(QpyError::from)
...
fn pack_sparse_pauli_op(
    ...
    if operator.is_instance_of::<PySparseObservable>() {
        let py_sparse_observable: PyRef<PySparseObservable> = operator.extract().map_err(|e| QpyError::from(PyErr::from(e)))?;
        let sparse_observable = py_sparse_observable
            .inner
            .read()
            .map_err(|_| QpyError::ConversionError("Can't extract sparse observable data".to_string()))?;
...
pub(crate) fn py_pack_pauli_evolution_gate(
    ...
    let operator_list: Bound<PyList> = if !operators.is_instance_of::<PyList>() {
        ...
        operators.cast().map_err(|e| QpyError::from(PyErr::from(e)))?.clone()
    };
...
pub(crate) fn py_convert_to_generic_value(py_object: &Bound<PyAny>) -> Result<GenericValue, QpyError> {
    ...
    match type_key {
        ...
        ValueType::Parameter => Ok(GenericValue::ParameterExpressionSymbol(
            py_object.extract::<PyParameter>().map_err(|e| QpyError::from(PyErr::from(e)))?.symbol().clone(),
        )),
        ValueType::ParameterVector => Ok(GenericValue::ParameterExpressionVectorSymbol(
            py_object
                .extract::<PyParameterVectorElement>().map_err(|e| QpyError::from(PyErr::from(e)))?
                .symbol()
                .clone(),
        )),
        ValueType::ParameterExpression => Ok(GenericValue::ParameterExpression(Arc::new(
            py_object.extract::<PyParameterExpression>().map_err(|e| QpyError::from(PyErr::from(e)))?.inner,
        ))),
        ...
        ValueType::Register => {
            if let Ok(clbit) = py_object.extract::<ShareableClbit>() {
                ...
                Err(QpyError::InvalidRegister("Could not read python register".to_string()))
            }
        }
    }
}
...
pub(crate) fn py_convert_from_generic_value(value: &GenericValue) -> Result<Py<PyAny>, QpyError> {
    Python::attach(|py| -> Result<Py<PyAny>, QpyError> { match value {
        ...
        GenericValue::ParameterExpressionSymbol(symbol) => Ok(symbol.clone().into_py_any(py)?),
        GenericValue::ParameterExpressionVectorSymbol(symbol) => Ok(symbol.clone().into_py_any(py)?),
...
pub(crate) fn py_pack_modifier(modifier: &Py<PyAny>) -> Result<formats::ModifierPack, QpyError> {
    Python::attach(|py| {
        ...
        if modifier.is_instance(&module.getattr("InverseModifier")?)? {
            ...
            Err(QpyError::ConversionError("Unsupported modifier".to_string()))
        }
    })
}

pub(crate) fn py_unpack_modifier(packed_modifier: &formats::ModifierPack) -> Result<Py<PyAny>, QpyError> {
