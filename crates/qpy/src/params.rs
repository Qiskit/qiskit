...
use crate::formats::{
    ...
    ParameterPack, SerializableParam,
};
...
use pyo3::intern;
use pyo3::prelude::*;
use pyo3::types::{PyAny, PyComplex, PyDict, PyFloat, PyInt, PyString};
...
fn write_parameter_expression_elements(py: Python, py_object: &Bound<PyAny>) -> PyResult<Bytes> {
    ...
    let result_data: Bytes = qpy_replay
        .iter()
        .flat_map(|replay_obj| {
            serialize_parameter_expression_element(py, &replay_obj.bind(py)).unwrap()
        })
        .collect();
    Ok(result_data)
}

fn write_symbol_table(py: Python, py_object: &Bound<PyAny>) -> PyResult<(u64, Bytes)> {
...
fn write_parameter_expression(py: Python, py_object: &Bound<PyAny>) -> PyResult<Bytes> {
...
fn serialize_parameter_expression(
    py: Python,
    py_object: &Bound<PyAny>,
) -> PyResult<SerializableParam> {
    let data = write_parameter_expression(py, py_object)?;
    let data_len = data.len() as u64;
    Ok(SerializableParam {
        type_key: (tags::PARAMETER_EXPRESSION),
        data_len: (data_len),
        data: (data),
    })
}

fn write_parameter(py: Python, py_object: &Bound<PyAny>) -> PyResult<Bytes> {
    let name = py_object
...
fn serialize_object(py: Python, py_object: &Py<PyAny>) -> SerializableParam {
    ...
    match type_key {
        tags::CIRCUIT => SerializableParam {
            type_key: (tags::NULL),
            data_len: (0),
            data: (Vec::new()),
        },
...
pub fn param_to_serializable(py: Python, param: &Param) -> SerializableParam {
    match param {
        Param::Float(val) => SerializableParam {
            type_key: (tags::FLOAT),
            data_len: (8),
            data: (val.to_le_bytes().to_vec()),
        },
        Param::ParameterExpression(py_object) => {
            serialize_parameters(py, py_object.bind(py)).unwrap()
        }
