...
fn pack_custom_instruction(
    ...
    if gate_type == CircuitInstructionType::PauliEvolutionGate {
        if let OperationRef::Gate(gate) = operation.op.view() {
            has_definition = true;
        ...
        if let OperationRef::Gate(pygate) = operation.op.view() {
            has_definition = true;
