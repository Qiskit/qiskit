...
pub struct DAGCircuit {
    ...
    // Indexed by DAGVarType as usize
    vars_by_type: [Py<PySet>; 3],

    captured_stretches: Py<PySet>,
    declared_stretches: Py<PyList>,
}
...
impl DAGCircuit {
    ...
    pub fn new(py: Python<'_>) -> PyResult<Self> {
        Ok(DAGCircuit {
            ...
            captured_stretches: PySet::empty(py)?.unbind(),
            declared_stretches: PyList::empty(py).unbind(),
        })
    }
    ...
    fn __getstate__(&self, py: Python) -> PyResult<Py<PyDict>> {
        ...
        out_dict.set_item("captured_stretches", self.captured_stretches.clone())?;
        out_dict.set_item("declared_stretches", self.declared_stretches.clone())?;
    ...
    pub fn copy_empty_like(&self, py: Python, vars_mode: &str) -> PyResult<Self> {
        ...
        if vars_mode == "alike" {
            ...
            for stretch in self.captured_stretches.bind(py).iter() {
    ...
    fn add_captured_stretch(&mut self, py: Python, stretch: &Bound<PyAny>) -> PyResult<()> {
        ...
        let name: String = stretch.getattr("name")?.extract::<String>()?;
        match self.identifier_info.get(&name) {
            Some(DAGIdentifierInfo::Stretch(info)) if stretch.eq(&info.stretch)? => {
    ...
    fn iter_stretches(&self, py: Python) -> PyResult<Py<PyIterator>> {
        let out_set = PySet::empty(py)?;
        for s in self.captured_stretches.bind(py).iter() {
            out_set.add(s)?;
        }
        for s in self.declared_stretches.bind(py).iter() {
            out_set.add(s)?;
        }
        Ok(out_set.into_any().try_iter()?.unbind())
    }
