...
impl DAGCircuit {
    ...
    fn additional_wires(&self, op: OperationRef) -> PyResult<(Vec<Clbit>, Vec<PyObject>)> {
    ...
    pub(crate) fn from_circuit(
        ...
        let mut new_dag =
            DAGCircuit::with_capacity(num_qubits, num_clbits, Some(num_vars), Some(num_ops), None)?;
