...
impl PyDAGCircuit {
    ...
    #[getter]
    fn get_qregs(&self, py: Python) -> PyResult<Py<PyDict>> {
        Ok(self.try_read()?.qregs.cached(py).clone_ref(py))
    }
    ...
    pub fn get_qubit_locations(&self, py: Python) -> PyResult<Py<PyDict>> {
        Ok(self.try_read()?.qubit_locations.cached(py).clone_ref(py))
    }

    /// Returns the dict containing the ClassicalRegisters in the circuit
    #[getter]
    fn get_cregs(&self, py: Python) -> PyResult<Py<PyDict>> {
        Ok(self.try_read()?.cregs.cached(py).clone_ref(py))
    }
    ...
    pub fn get_clbit_locations(&self, py: Python) -> PyResult<Py<PyDict>> {
        Ok(self.try_read()?.clbit_locations.cached(py).clone_ref(py))
    }
    ...
    fn input_map(&self, py: Python) -> PyResult<Py<PyDict>> {
        let borrowed = self.try_read()?;
        let out_dict = PyDict::new(py);
        for (qubit, indices) in borrowed
            .qubit_io_map
            .iter()
            .enumerate()
        ...
        {
            out_dict.set_item(
                borrowed.qubits.get(qubit).unwrap(),
                borrowed.get_node(py, indices[0])?,
            )?;
        }
        for (clbit, indices) in borrowed
            .clbit_io_map
            .iter()
            .enumerate()
        ...
        {
            out_dict.set_item(
                borrowed.clbits.get(clbit).unwrap(),
                borrowed.get_node(py, indices[0])?,
            )?;
        }
        for (var, indices) in borrowed
            .var_io_map
            .iter()
            .enumerate()
    ...
    fn output_map(&self, py: Python) -> PyResult<Py<PyDict>> {
            ...
            .map(|(idx, indices)| (Qubit::new(idx), indices))
        {
            out_dict.set_item(
                borrowed.qubits.get(qubit).unwrap(),
                borrowed.get_node(py, indices[1])?,
            )?;
        }
        for (clbit, indices) in borrowed
            .clbit_io_map
    ...
    fn __getstate__(&self, py: Python) -> PyResult<Py<PyDict>> {
        ...
        out_dict.set_item("qregs", borrowed.qregs.cached(py))?;
        out_dict.set_item("cregs", borrowed.cregs.cached(py))?;
        out_dict.set_item("global_phase", borrowed.global_phase.clone())?;
        out_dict.set_item(
            "qubit_io_map",
            borrowed
                .qubit_io_map
        ...
        out_dict.set_item("op_name", borrowed.op_names.clone())?;
        out_dict.set_item("qubits", borrowed.qubits.objects())?;
        out_dict.set_item("clbits", borrowed.clbits.objects())?;
        ...
        let mut nodes: Vec<Py<PyAny>> = Vec::with_capacity(borrowed.dag.node_count());
        for node_idx in borrowed.dag.node_indices() {
            let node_data = borrowed.get_node(py, node_idx)?;
            nodes.push((node_idx.index(), node_data).into_py_any(py)?);
        }
        out_dict.set_item("nodes", nodes)?;
        out_dict.set_item(
            "nodes_removed",
            borrowed.dag.node_count() != borrowed.dag.node_bound(),
        )?;
        let mut edges: Vec<Py<PyAny>> = Vec::with_capacity(borrowed.dag.edge_bound());
        // edges are saved with none (deleted edges) instead of their index to save space
        for i in 0..borrowed.dag.edge_bound() {
            let idx = EdgeIndex::new(i);
            let edge = match borrowed.dag.edge_weight(idx) {
                Some(edge_w) => {
                    let endpoints = borrowed.dag.edge_endpoints(idx).unwrap();
                    (
                        endpoints.0.index(),
                        endpoints.1.index(),
                        edge_w.to_pickle(py)?,
    ...
    fn __setstate__(&mut self, py: Python, state: Py<PyAny>) -> PyResult<()> {
        ...
        self.try_write()?.__setstate__(py, state)
    }

    pub fn __copy__(&self) -> PyResult<Self> {
        Ok(Self {
            inner: RwLock::new(self.try_read()?.clone()).into(),
            ..Clone::clone(self)
        })
    }
    ...
    pub fn __deepcopy__<'py>(
        ...
        out.duration = out
            ...
            .map(|dict| deepcopy.call1((dict, memo)).map(|ob| ob.unbind()))
            .transpose()?;
        let mut out_mut = out.try_write()?;
        for node in out_mut.dag.node_weights_mut() {
            if let NodeType::Operation(inst) = node {
                inst.py_deepcopy_inplace(py, memo)?;
            };
        }
        // Manually drop mutable borrow of out since we will no longer use it.
        drop(out_mut);
        Ok(out)
    }
    ...
    pub fn py_qubits(&self, py: Python<'_>) -> PyResult<Py<PyList>> {
        Ok(self.try_read()?.qubits.cached(py).clone_ref(py))
    }
    ...
    pub fn py_clbits(&self, py: Python<'_>) -> PyResult<Py<PyList>> {
        Ok(self.try_read()?.clbits.cached(py).clone_ref(py))
    }

    /// Return a list of the wires in order.
    #[getter]
    fn get_wires(&self, py: Python<'_>) -> PyResult<Py<PyList>> {
        self.try_read()?.get_wires(py)
    }

    /// Returns the number of nodes in the dag.
    #[getter]
    fn get_node_counter(&self) -> PyResult<usize> {
        Ok(self.try_read()?.dag.node_count())
    }

    /// Return the global phase of the circuit.
    #[getter]
    pub fn get_global_phase(&self) -> PyResult<Param> {
        Ok(self.try_read()?.global_phase.clone())
    }
    ...
    pub fn set_global_phase(&mut self, angle: Param) -> PyResult<()> {
        self.try_write()?
            .set_global_phase_param(angle)
            .map(|_| ())
            .map_err(Into::into)
    }

    /// Remove all operation nodes with the given name.
    fn remove_all_ops_named(&mut self, opname: &str) -> PyResult<()> {
    ...
    fn add_qubits(&mut self, qubits: Vec<Bound<PyAny>>) -> PyResult<()> {
        ...
        for bit in qubits.into_iter() {
            ...
            if borrowed.qubits.find(&bit).is_some() {
                return Err(DAGCircuitError::new_err(format!(
                    "duplicate qubits {bit:?}"
                )));
            }
            borrowed.add_qubit_unchecked(bit)?;
        }
        Ok(())
    }
    ...
    fn add_clbits(&mut self, clbits: Vec<Bound<'_, PyAny>>) -> PyResult<()> {
        ...
        for bit in clbits.into_iter() {
            ...
            if borrowed.clbits.find(&bit).is_some() {
                return Err(DAGCircuitError::new_err(format!(
                    "duplicate clbits {bit:?}"
                )));
            }
            borrowed.add_clbit_unchecked(bit)?;
        }
        Ok(())
    }
    ...
    pub fn add_qreg(&mut self, qreg: QuantumRegister) -> PyResult<()> {
        self.try_write()?.add_qreg(qreg).map_err(Into::into)
    }

    /// Add all wires in a classical register.
    pub fn add_creg(&mut self, creg: ClassicalRegister) -> PyResult<()> {
        self.try_write()?.add_creg(creg).map_err(Into::into)
    }
    ...
    fn py_remove_clbits(&mut self, clbits: Vec<ShareableClbit>) -> PyResult<()> {
        let mut borrowed = self.try_write()?;
        let bit_iter = match borrowed.clbits.map_objects(clbits.iter().cloned()) {
        ...
        borrowed.remove_clbits(bit_iter).map_err(Into::into)
    }
    ...
    fn py_remove_cregs(&mut self, cregs: Vec<ClassicalRegister>) -> PyResult<()> {
        self.try_write()?.remove_cregs(cregs).map_err(Into::into)
    }
    ...
    pub fn py_remove_qubits(&mut self, qubits: Vec<ShareableQubit>) -> PyResult<()> {
        let mut borrowed = self.try_write()?;
        let bit_iter = match borrowed.qubits.map_objects(qubits.iter().cloned()) {
            Ok(bit_iter) => bit_iter,
            Err(_) => {
                return Err(DAGCircuitError::new_err(format!(
        ...
        borrowed.remove_qubits(bit_iter).map_err(Into::into)
    }
    ...
    fn py_remove_qregs(&mut self, qregs: Vec<QuantumRegister>) -> PyResult<()> {
        self.try_write()?.py_remove_qregs(qregs)
    }
    ...
    fn _check_condition(&self, py: Python, name: &str, condition: &Bound<PyAny>) -> PyResult<()> {
        ...
        for bit in resources.clbits.bind(py) {
            ...
            if borrowed.clbits.find(&bit).is_none() {
                return Err(DAGCircuitError::new_err(format!(
    ...
    pub fn py_copy_empty_like(&self, vars_mode: VarsMode) -> PyResult<Self> {
        Ok(PyDAGCircuit {
            ...
            inner: RwLock::new(self.try_read()?.copy_empty_like_with_capacity(
                0,
                0,
                vars_mode,
                BlocksMode::Drop,
    ...
    pub fn py_make_physical(&mut self, num_qubits: Option<u32>) -> PyResult<()> {
        ...
        let num_qubits = match num_qubits {
            Some(num_qubits) => {
                if (num_qubits as usize) < borrowed.num_qubits() {
            ...
            None => borrowed.num_qubits(),
        };
        borrowed.make_physical(num_qubits);
        Ok(())
    }
    ...
    fn _apply_op_node_back(
        ...
        let mut borrowed = self.try_write()?;
        if let NodeType::Operation(inst) = borrowed.pack_into(py, node)? {
            if check {
                borrowed.check_op_addition(&inst)?;
            }

            borrowed.push_back(inst)?;
            Ok(())
        } else {
    ...
    pub fn py_apply_operation_back(
        ...
        self.try_write()?
            .py_apply_operation_back(py, op, qargs, cargs, check)
    }
    ...
    fn py_apply_operation_front(
        ...
        self.try_write()?
            .py_apply_operation_front(py, op, qargs, cargs, check)
    }
    ...
    fn py_compose(
        ...
        self.try_write()?
            ...
            .map(|dag| PyDAGCircuit::from(dag).into_py_any(py))
            .transpose()
    }
    ...
    pub fn size(&self, recurse: bool) -> PyResult<usize> {
        self.try_read()?.size(recurse).map_err(Into::into)
    }
    ...
    pub fn depth(&self, recurse: bool) -> PyResult<usize> {
        self.try_read()?.depth(recurse).map_err(Into::into)
    }
    ...
    pub fn num_qubits(&self) -> PyResult<usize> {
        Ok(self.try_read()?.num_qubits())
    }

    /// Return the total number of classical bits used by the circuit.
    pub fn num_clbits(&self) -> PyResult<usize> {
        Ok(self.try_read()?.num_clbits())
    }

    /// Return the number of basic blocks in this circuit.
    pub fn num_blocks(&self) -> PyResult<usize> {
        Ok(self.try_read()?.num_blocks())
    }

    /// Get the number of op nodes in the DAG.
    #[inline]
    pub fn num_ops(&self) -> PyResult<usize> {
        Ok(self.try_read()?.num_ops())
    }

    /// Compute how many components the circuit can decompose into.
    fn num_tensor_factors(&self) -> PyResult<usize> {
        Ok(self.try_read()?.num_tensor_factors())
    }
    ...
    fn __eq__(&self, py: Python, other: &PyDAGCircuit) -> PyResult<bool> {
        ...
        let borrowed = self.try_read()?;
        let other_borrowed = other.try_read()?;
        if borrowed.num_qubits() != other_borrowed.num_qubits()
            || borrowed.num_clbits() != other_borrowed.num_clbits()
        {
            return Ok(false);
        }
        let slf_qubit_map: HashMap<Qubit, Qubit> = (0..borrowed.num_qubits())
            .map(|i| (Qubit::new(i), Qubit::new(i)))
            .collect();
        let slf_clbit_map: HashMap<Clbit, Clbit> = (0..borrowed.num_clbits())
    ...
    fn structurally_equal(&self, other: &PyDAGCircuit) -> PyResult<bool> {
        let borrowed = self.try_read()?;
        let other_borrowed = other.try_read()?;
        if borrowed.qubits != other_borrowed.qubits {
            return Ok(false);
        }
        if borrowed.clbits != other_borrowed.clbits {
            return Ok(false);
        }
        if !borrowed
            .vars_stretches
            .structurally_equal(&other_borrowed.vars_stretches)
        {
            return Ok(false);
        }
        if borrowed.qregs != other_borrowed.qregs {
            return Ok(false);
        }
        if borrowed.cregs != other_borrowed.cregs {
            return Ok(false);
        }
        ...
        if !param_eq(&borrowed.global_phase, &other_borrowed.global_phase)? {
            return Ok(false);
        }
        ...
        let inst_eq = |from_self: &PackedInstruction,
         ...
         -> PyResult<bool> {
            ...
            if borrowed.qargs_interner.get(from_self.qubits)
                != other_borrowed.qargs_interner.get(from_other.qubits)
            {
                return Ok(false);
            }
            if borrowed.cargs_interner.get(from_self.clbits)
                != other_borrowed.cargs_interner.get(from_other.clbits)
            {
                return Ok(false);
            }
        ...
        if borrowed.dag.node_count() != other_borrowed.dag.node_count()
            || borrowed.dag.edge_count() != other_borrowed.dag.edge_count()
        {
            return Ok(false);
        }
        for (self_index, other_index) in borrowed
            .dag
            .node_indices()
            .zip(other_borrowed.dag.node_indices())
        {
            ...
            if !borrowed.dag[self_index].equal_with(&other_borrowed.dag[other_index], inst_eq)? {
                return Ok(false);
            }
            for pair in borrowed
                .dag
    ...
    fn py_topological_nodes(
        ...
        let nodes: PyResult<Vec<_>> = if let Some(key) = key {
            borrowed
                ...
                .map(|node| borrowed.get_node(py, node))
                .collect()
        } else {
    ...
    fn py_topological_op_nodes(
        ...
        let nodes: PyResult<Vec<_>> = if let Some(key) = key {
            borrowed
                .topological_key_sort(py, &key, reverse)?
                .filter_map(|node| match borrowed.dag.node_weight(node) {
                    Some(NodeType::Operation(_)) => Some(borrowed.get_node(py, node)),
                    _ => None,
                })
                .collect()
        } else {
    ...
    fn replace_block_with_op(
        ...
        self.try_write()?
            .replace_block_with_op(py, node_block, op, wire_pos_map, cycle_check)
    }
    ...
    pub fn py_substitute_node_with_dag(
        ...
        self.try_write()?
            .py_substitute_node_with_dag(py, node, &*input_dag.try_read()?, wires)
    }
    ...
    pub fn py_substitute_node(
        ...
        // Acquire mutable lock just once to substitute the node.
        self.try_write()?
            .substitute_node_with_py_op(node_index, op)?;

        // Acquire read lock to retrieve the node.
        let borrowed = self.try_read()?;
        if inplace {
            ...
            borrowed.get_node(py, node_index)
        }
    }
    ...
    fn separable_circuits(
        ...
        let connected_components =
            rustworkx_core::connectivity::connected_components(&borrowed.dag);
        let dags = PyList::empty(py);

        for comp_nodes in connected_components.iter() {
            let mut new_dag = borrowed.copy_empty_like(vars_mode, BlocksMode::Drop);
            ...
            for node in comp_nodes {
                match borrowed.dag.node_weight(*node) {
                    Some(w) => match w {
                        NodeType::ClbitIn(b) => {
                        ...
                        NodeType::Operation(pi) => {
                            let pi = block_map.map_instruction(pi, |b| {
                                new_dag.add_block(borrowed.blocks[b].clone())
                            });
            ...
            let filtered = NodeFiltered(&borrowed.dag, node_filter);
            ...
            // Manually drop the mutable borrow
            let new_py_dag = PyDAGCircuit::from_dagcircuit_with_cloned_metadata(new_dag, self);
    ...
    fn swap_nodes(&mut self, node1: &DAGNode, node2: &DAGNode) -> PyResult<()> {
        ...
        let wires: Vec<Wire> = borrowed_mut
            .dag
            .edges(node1)
        ...
        let directed_edge_for_wire = |node: NodeIndex, direction: Direction, wire: Wire| {
            for edge in borrowed_mut.dag.edges_directed(node, direction) {
                if wire == *edge.weight() {
                    match direction {
        ...
        for (wire, (node1_to_node2, _), (parent_to_node1, parent), (node2_to_child, child)) in
            relevant_edges
        {
            borrowed_mut.dag.remove_edge(parent_to_node1);
            borrowed_mut.dag.add_edge(parent, node2, wire);
            borrowed_mut.dag.remove_edge(node1_to_node2);
            borrowed_mut.dag.add_edge(node2, node1, wire);
            borrowed_mut.dag.remove_edge(node2_to_child);
            borrowed_mut.dag.add_edge(node1, child, wire);
        }
        Ok(())
    }
    ...
    fn node(&self, py: Python, node_id: isize) -> PyResult<Py<PyAny>> {
        self.try_read()?
            .get_node(py, NodeIndex::new(node_id as usize))
    }
    ...
    fn nodes(&self, py: Python) -> PyResult<Py<PyIterator>> {
        let borrowed = self.try_read()?;
        let result: PyResult<Vec<_>> = borrowed
            .dag
            .node_references()
            .map(|(node, weight)| borrowed.unpack_into(py, node, weight))
            .collect();
    ...
    fn edges(&self, py: Python, nodes: Option<Bound<PyAny>>) -> PyResult<Py<PyIterator>> {
        ...
        let actual_nodes: Vec<_> = match nodes {
            None => borrowed.dag.node_indices().collect(),
            Some(nodes) => {
                let mut out = Vec::new();
        ...
        for node in actual_nodes {
            for edge in borrowed.dag.edges_directed(node, Outgoing) {
                edges.push((
                    borrowed.get_node(py, edge.source())?,
                    borrowed.get_node(py, edge.target())?,
                    match edge.weight() {
                        Wire::Qubit(qubit) => {
                            borrowed.qubits.get(*qubit).unwrap().into_bound_py_any(py)?
                        }
                        Wire::Clbit(clbit) => {
                            borrowed.clbits.get(*clbit).unwrap().into_bound_py_any(py)?
                        }
                        Wire::Var(var) => borrowed
                            .vars_stretches
                            .vars()
                            .get(*var)
                            .unwrap()
    ...
    fn py_op_nodes(
        ...
        for (node, weight) in borrowed.dag.node_references() {
            if let NodeType::Operation(packed) = &weight {
                ...
                if let Some(op_type) = op {
                    ...
                    {
                        nodes.push(borrowed.unpack_into(py, node, weight)?);
                    }
                } else {
                    nodes.push(borrowed.unpack_into(py, node, weight)?);
                }
            ...
            .filter_map(|(node_index, node_type)| match node_type {
                NodeType::Operation(node) => {
                    if node.op.try_control_flow().is_some_and(|control_flow| {
                        ...
                        Some(borrowed.unpack_into(py, node_index, node_type))
                    } else {
                        None
                    }
                }
        ...
        borrowed
            ...
            .filter_map(|(node, weight)| match weight {
                NodeType::Operation(packed) => match packed.op.view() {
                    ...
                    | OperationRef::StandardGate(_) => Some(borrowed.unpack_into(py, node, weight)),
                    _ => None,
                },
                _ => None,
            })
    ...
    pub fn py_two_qubit_ops(&self, py: Python) -> PyResult<Vec<Py<PyAny>>> {
        ...
        borrowed
            .two_qubit_ops()
            .map(|(index, _)| borrowed.unpack_into(py, index, &borrowed.dag[index]))
            .collect()
    }
    ...
    fn multi_qubit_ops(&self, py: Python) -> PyResult<Vec<Py<PyAny>>> {
        ...
        for (node, weight) in borrowed.dag.node_references() {
            if let NodeType::Operation(packed) = weight {
                ...
                let qargs = borrowed.qargs_interner.get(packed.qubits);
                if qargs.len() >= 3 {
                    nodes.push(borrowed.unpack_into(py, node, weight)?);
                }
            }
    ...
    fn longest_path(&self, py: Python) -> PyResult<Vec<Py<PyAny>>> {
        ...
        match rustworkx_core::dag_algo::longest_path(&borrowed.dag, weight_fn).unwrap() {
            Some(res) => res.0,
            None => panic!("not a DAG"),
        }
        .into_iter()
        .map(|node_index| borrowed.get_node(py, node_index))
        .collect()
    }
    ...
    fn py_successors(&self, py: Python, node: &DAGNode) -> PyResult<Py<PyIterator>> {
        let borrowed = self.try_read()?;
        let successors: PyResult<Vec<_>> = borrowed
            .successors(node.node.unwrap())
            .map(|i| borrowed.get_node(py, i))
            .collect();
    ...
    fn py_predecessors(&self, py: Python, node: &DAGNode) -> PyResult<Py<PyIterator>> {
        let borrowed = self.try_read()?;
        let predecessors: PyResult<Vec<_>> = borrowed
    ...
    fn op_successors(&self, py: Python, node: &DAGNode) -> PyResult<Py<PyIterator>> {
        ...
        let predecessors: PyResult<Vec<_>> = borrowed
            ...
            .unique()
            .filter_map(|i| match borrowed.dag[i] {
                NodeType::Operation(_) => Some(borrowed.get_node(py, i)),
                _ => None,
            })
            .collect();
        Ok(PyTuple::new(py, predecessors?)?
    ...
    fn is_successor(&self, node: &DAGNode, node_succ: &DAGNode) -> PyResult<bool> {
        let borrowed = self.try_read()?;
        Ok(borrowed
            .dag
    ...
    fn is_predecessor(&self, node: &DAGNode, node_pred: &DAGNode) -> PyResult<bool> {
        let borrowed = self.try_read()?;
        Ok(borrowed
            .dag
    ...
    fn classical_predecessors(&self, py: Python, node: &DAGNode) -> PyResult<Py<PyIterator>> {
        let borrowed = self.try_read()?;
        let edges = borrowed.dag.edges_directed(node.node.unwrap(), Incoming);
    ...
    fn py_ancestors(&self, py: Python, node: &DAGNode) -> PyResult<Py<PySet>> {
        let borrowed = self.try_read()?;
        let ancestors: PyResult<Vec<Py<PyAny>>> = borrowed
    ...
    fn py_descendants(&self, py: Python, node: &DAGNode) -> PyResult<Py<PySet>> {
        let borrowed = self.try_read()?;
        let descendants: PyResult<Vec<Py<PyAny>>> = borrowed
    ...
    fn py_bfs_successors(&self, py: Python, node: &DAGNode) -> PyResult<Py<PyIterator>> {
        ...
        let borrowed = self.try_read()?;
        let successor_index: PyResult<PyIteratorVec> = borrowed
            .bfs_successors(node.node.unwrap())
            .map(|(node, nodes)| -> PyResult<(Py<PyAny>, Vec<Py<PyAny>>)> {
                Ok((
                    borrowed.get_node(py, node)?,
                    nodes
                        .iter()
                        .map(|sub_node| borrowed.get_node(py, *sub_node))
    ...
    fn classical_successors(&self, py: Python, node: &DAGNode) -> PyResult<Py<PyIterator>> {
        let borrowed = self.try_read()?;
        let edges = borrowed.dag.edges_directed(node.node.unwrap(), Outgoing);
        let filtered = edges.filter_map(|e| match e.weight() {
            Wire::Qubit(_) => None,
            _ => Some(e.target()),
        });
    ...
    fn py_remove_op_node(&mut self, node: &Bound<PyAny>) -> PyResult<()> {
        ...
        if borrowed.dag.node_weight(index).is_none() {
            return Err(DAGCircuitError::new_err("Node not in DAG"));
        }
        borrowed.remove_op_node(index);
        Ok(())
    }
    ...
    fn remove_ancestors_of(&mut self, node: &DAGNode) -> PyResult<()> {
        let mut borrowed = self.try_write()?;
        let ancestors: Vec<_> = core_ancestors(&borrowed.dag, node.node.unwrap())
            .filter(|next| {
                next != &node.node.unwrap()
                    && matches!(
                        borrowed.dag.node_weight(*next),
                        Some(NodeType::Operation(_))
                    )
            })
            .collect();
        for a in ancestors {
            borrowed.dag.remove_node(a);
        }
        Ok(())
    }
    ...
    fn remove_descendants_of(&mut self, node: &DAGNode) -> PyResult<()> {
        let mut borrowed = self.try_write()?;
        let descendants: Vec<_> = core_descendants(&borrowed.dag, node.node.unwrap())
            .filter(|next| {
                next != &node.node.unwrap()
                    && matches!(
                        borrowed.dag.node_weight(*next),
    ...
    fn remove_nonancestors_of(&mut self, node: &DAGNode) -> PyResult<()> {
        let mut borrowed = self.try_write()?;
        let ancestors: HashSet<_> = core_ancestors(&borrowed.dag, node.node.unwrap())
            .filter(|next| {
                next != &node.node.unwrap()
                    && matches!(
                        borrowed.dag.node_weight(*next),
    ...
    fn remove_nondescendants_of(&mut self, node: &DAGNode) -> PyResult<()> {
        let mut borrowed = self.try_write()?;
        let descendants: HashSet<_> = core_descendants(&borrowed.dag, node.node.unwrap())
            .filter(|next| {
                next != &node.node.unwrap()
                    && matches!(
                        borrowed.dag.node_weight(*next),
    ...
    fn py_front_layer(&self, py: Python) -> PyResult<Py<PyList>> {
        let borrowed = self.try_read()?;
        let native_front_layer = borrowed.front_layer();
        let front_layer_list = PyList::empty(py);
        for node in native_front_layer {
            front_layer_list.append(borrowed.get_node(py, node)?)?;
        }
    ...
    fn layers(&self, py: Python, vars_mode: VarsMode) -> PyResult<Py<PyIterator>> {
        ...
        let mut graph_layers = borrowed.multigraph_layers();
        ...
        for graph_layer in graph_layers {
            ...
            let mut new_layer = borrowed.copy_empty_like(vars_mode, BlocksMode::Drop);
            ...
            let data: Vec<_> = op_nodes
                ...
                .map(|(inst, _)| {
                    block_map
                        .map_instruction(inst, |b| new_layer.add_block(borrowed.blocks[b].clone()))
                })
                .collect();
            ...
            layer_dict.set_item(
                "graph",
                PyDAGCircuit::from_dagcircuit_with_cloned_metadata(new_layer, self),
            )?;
    ...
    fn serial_layers(&self, py: Python, vars_mode: VarsMode) -> PyResult<Py<PyIterator>> {
        let borrowed = self.try_read()?;
        let layer_list = PyList::empty(py);
        for next_node in borrowed.topological_op_nodes(false) {
            let retrieved_node: &PackedInstruction = match borrowed.dag.node_weight(next_node) {
            ...
            let qubits = PyTuple::new(
                ...
                borrowed
                    ...
                    .iter()
                    .map(|qubit| borrowed.qubits.get(*qubit)),
            )?
            .unbind();
            let inst = block_map.map_instruction(retrieved_node, |b| {
                new_layer.add_block(borrowed.blocks[b].clone())
            });
    ...
    fn py_multigraph_layers(&self, py: Python) -> PyResult<Py<PyIterator>> {
        let borrowed = self.try_read()?;
        let graph_layers = borrowed.multigraph_layers().map(|layer| -> Vec<Py<PyAny>> {
            layer
                .into_iter()
                .filter_map(|index| borrowed.get_node(py, index).ok())
                .collect()
        });
    ...
    fn py_collect_runs(&self, py: Python, namelist: &Bound<PyList>) -> PyResult<Py<PySet>> {
        ...
        for run in borrowed.collect_runs(name_list_set) {
            let run_tuple = PyTuple::new(
                py,
                run.into_iter()
                    .map(|node_index| borrowed.get_node(py, node_index).unwrap()),
            )?;
    ...
    fn py_collect_1q_runs(&self, py: Python) -> PyResult<Py<PyList>> {
        let borrowed = self.try_read()?;
        match borrowed.collect_1q_runs() {
            Some(runs) => {
    ...
    fn py_collect_2q_runs(&self, py: Python) -> PyResult<Py<PyList>> {
        let borrowed = self.try_read()?;
        match borrowed.collect_2q_runs() {
            Some(runs) => {
    ...
    fn py_nodes_on_wire(
        ...
        let wire = if wire.cast::<PyQubit>().is_ok() {
            ...
            borrowed.qubits.find(&wire).map(Wire::Qubit)
        } else if wire.cast::<PyClbit>().is_ok() {
            let wire = wire.extract::<ShareableClbit>()?;
            borrowed.clbits.find(&wire).map(Wire::Clbit)
        } else {
            let wire = wire.extract::<expr::Var>()?;
            borrowed.vars_stretches.vars().find(&wire).map(Wire::Var)
        }
        .ok_or_else(|| {
            DAGCircuitError::new_err(format!(
        ...
        let nodes = borrowed
            .nodes_on_wire(wire)
            .filter(|node| !only_ops || matches!(borrowed.dag[*node], NodeType::Operation(_)))
            .map(|n| borrowed.get_node(py, n))
            .collect::<PyResult<Vec<_>>>()?;
    ...
    fn py_count_ops(&self, py: Python, recurse: bool) -> PyResult<Py<PyAny>> {
        self.try_read()?.count_ops(recurse)?.into_py_any(py)
    }

    /// Count the occurrences of operation names on the longest path.
    ///
    /// Returns a dictionary of counts keyed on the operation name.
    fn count_ops_longest_path<'py>(&self, py: Python<'py>) -> PyResult<Bound<'py, PyAny>> {
        let borrowed = self.try_read()?;
        if borrowed.dag.node_count() == 0 {
        ...
        for node_index in &longest_path[1..longest_path.len() - 1] {
            if let NodeType::Operation(ref packed) = borrowed.dag[*node_index] {
        ...
        op_counts.into_bound_py_any(py)
    }
    ...
    fn quantum_causal_cone(&self, py: Python, qubit: &Bound<PyAny>) -> PyResult<Py<PySet>> {
        ...
        let output_qubit = borrowed.qubits.find(&qubit_nat).ok_or_else(|| {
            DAGCircuitError::new_err(format!(
                "The given qubit {qubit:?} is not present in the circuit"
            ))
        })?;
        let output_node_index = borrowed
            .qubit_io_map
        ...
        let mut queue: VecDeque<NodeIndex> =
            borrowed.quantum_predecessors(output_node_index).collect();
        ...
        while !queue.is_empty() {
            ...
            if let NodeType::Operation(packed) = borrowed.dag.node_weight(cur_index).unwrap() {
                if !packed.op.directive() {
                    ...
                    qubits_in_cone.extend(borrowed.qargs_interner.get(packed.qubits));
                    processed_non_directive_nodes.insert(cur_index);

                    for pred_index in borrowed.quantum_predecessors(cur_index) {
                        if let NodeType::Operation(_pred_packed) =
                            borrowed.dag.node_weight(pred_index).unwrap()
                        {
        ...
        let elements = borrowed.qubits.map_indices(&qubits_in_cone_vec);
    ...
    fn properties(&self, py: Python) -> PyResult<HashMap<&str, Py<PyAny>>> {
        ...
        Ok(HashMap::from_iter([
            ("size", borrowed.size(false)?.into_py_any(py)?),
            ("depth", borrowed.depth(false)?.into_py_any(py)?),
            ("width", borrowed.width().into_py_any(py)?),
            ("qubits", borrowed.num_qubits().into_py_any(py)?),
            ("bits", borrowed.num_clbits().into_py_any(py)?),
            ("factors", borrowed.num_tensor_factors().into_py_any(py)?),
    ...
    fn py_add_input_var(&mut self, var: expr::Var) -> PyResult<()> {
        self.try_write()?.add_var(var, VarType::Input)?;
        Ok(())
    }
    ...
    fn py_add_captured_var(&mut self, var: expr::Var) -> PyResult<()> {
        self.try_write()?.add_var(var, VarType::Capture)?;
        Ok(())
    }
    ...
    fn py_add_declared_var(&mut self, var: expr::Var) -> PyResult<()> {
        self.try_write()?.add_var(var, VarType::Declare)?;
        Ok(())
    }
    ...
    #[getter]
    fn num_vars(&self) -> PyResult<usize> {
        let borrowed = self.try_read()?;
        Ok(borrowed.num_input_vars() + borrowed.num_captured_vars() + borrowed.num_declared_vars())
    }

    /// Number of input classical variables tracked by the circuit.
    #[getter]
    fn num_input_vars(&self) -> PyResult<usize> {
        Ok(self.try_read()?.num_input_vars())
    }

    /// Number of captured classical variables tracked by the circuit.
    #[getter]
    fn num_captured_vars(&self) -> PyResult<usize> {
        Ok(self.try_read()?.num_captured_vars())
    }

    /// Number of declared local classical variables tracked by the circuit.
    #[getter]
    fn num_declared_vars(&self) -> PyResult<usize> {
        Ok(self.try_read()?.num_declared_vars())
    }

    /// Total number of stretches tracked by the circuit.
    #[getter]
    pub fn num_stretches(&self) -> PyResult<usize> {
        Ok(self.try_read()?.num_stretches())
    }

    /// Number of captured stretches tracked by the circuit.
    #[getter]
    fn num_captured_stretches(&self) -> PyResult<usize> {
        Ok(self.try_read()?.num_captured_stretches())
    }

    /// Number of declared local stretches tracked by the circuit.
    #[getter]
    fn num_declared_stretches(&self) -> PyResult<usize> {
        Ok(self.try_read()?.num_declared_stretches())
    }
    ...
    fn py_has_var(&self, var: &Bound<PyAny>) -> PyResult<bool> {
        ...
        if let Ok(name) = var.extract::<String>() {
            Ok(borrowed.vars_stretches.has_var(&name))
        } else {
            let var = var.extract::<expr::Var>()?;
            Ok(borrowed.vars_stretches.vars().contains(&var))
        }
    }
    ...
    fn py_has_stretch(&self, stretch: &Bound<PyAny>) -> PyResult<bool> {
        ...
        if let Ok(name) = stretch.extract::<String>() {
            Ok(borrowed.vars_stretches.has_stretch(&name))
        } else {
            let stretch = stretch.extract::<expr::Stretch>()?;
            Ok(borrowed.vars_stretches.stretches().contains(&stretch))
        }
    }
    ...
    fn py_has_identifier(&self, var: &Bound<PyAny>) -> PyResult<bool> {
        let borrowed = self.try_read()?;
        if let Ok(name) = var.extract::<String>() {
            Ok(borrowed.vars_stretches.has_identifier(&name))
    ...
    fn iter_captures(&self, py: Python) -> PyResult<Py<PyIterator>> {
        let borrowed = self.try_read()?;
        let out_set = PySet::empty(py)?;
        for var in borrowed.captured_vars() {
            out_set.add(var.clone())?;
        }
        for stretch in borrowed.captured_stretches() {
    ...
    fn iter_vars(&self, py: Python) -> PyResult<Py<PyIterator>> {
        let borrowed = self.try_read()?;
        let out_set = PySet::empty(py)?;
        for var in borrowed.vars_stretches.vars().objects() {
            out_set.add(var.clone())?;
        }
        Ok(out_set.into_any().try_iter()?.unbind())
    }
    ...
    fn iter_stretches(&self, py: Python) -> PyResult<Py<PyIterator>> {
        let borrowed = self.try_read()?;
        let out_set = PySet::empty(py)?;
        for stretch in borrowed.vars_stretches.stretches().objects() {
            out_set.add(stretch.clone())?;
        }
        Ok(out_set.into_any().try_iter()?.unbind())
    }

    fn _has_edge(&self, source: usize, target: usize) -> PyResult<bool> {
        Ok(self
            .try_read()?
            .dag
    ...
    fn _is_dag(&self) -> PyResult<bool> {
        Ok(rustworkx_core::petgraph::algo::toposort(&self.try_read()?.dag, None).is_ok())
    }

    fn _in_edges(&self, py: Python, node_index: usize) -> PyResult<Vec<Py<PyTuple>>> {
        let borrowed = self.try_read()?;
        Ok(borrowed
            .dag
            ...
            .map(|wire| {
                (
                    ...
                    match wire.weight() {
                        Wire::Qubit(qubit) => {
                            borrowed.qubits.get(*qubit).into_bound_py_any(py).unwrap()
                        }
                        Wire::Clbit(clbit) => {
                            borrowed.clbits.get(*clbit).into_bound_py_any(py).unwrap()
                        }
                        Wire::Var(var) => borrowed
                            .vars_stretches
                            .vars()
    ...
    fn _out_edges(&self, py: Python, node_index: usize) -> PyResult<Vec<Py<PyTuple>>> {
        let borrowed = self.try_read()?;
        Ok(borrowed
            .dag
    ...
    fn _in_wires(&self, py: Python, node_index: usize) -> PyResult<Vec<Py<PyAny>>> {
        let borrowed = self.try_read()?;
        Ok(borrowed
            .dag
            .edges_directed(NodeIndex::new(node_index), Incoming)
            .map(|wire| match wire.weight() {
                Wire::Qubit(qubit) => borrowed.qubits.get(*qubit).into_py_any(py).unwrap(),
                Wire::Clbit(clbit) => borrowed.clbits.get(*clbit).into_py_any(py).unwrap(),
                Wire::Var(var) => borrowed
                    .vars_stretches
                    .vars()
                    .get(*var)
    ...
    fn _out_wires(&self, py: Python, node_index: usize) -> PyResult<Vec<Py<PyAny>>> {
        let borrowed = self.try_read()?;
        Ok(borrowed
            .dag
            .edges_directed(NodeIndex::new(node_index), Outgoing)
    ...
    fn _find_successors_by_edge(
            ...
            .unique_by(|e| e.id())
        {
            let weight = match e.weight() {
                Wire::Qubit(qubit) => borrowed.qubits.get(*qubit).into_py_any(py)?,
                Wire::Clbit(clbit) => borrowed.clbits.get(*clbit).into_py_any(py)?,
                Wire::Var(var) => borrowed
                    .vars_stretches
                    .vars()
                    .get(*var)
            ...
            if edge_checker.call1((weight,))?.extract::<bool>()? {
                result.push(borrowed.get_node(py, e.target())?);
            }
        }
        Ok(result)
    }
    ...
    fn _edges(&self, py: Python) -> PyResult<Vec<Py<PyAny>>> {
        ...
        borrowed
            ...
            .edge_indices()
            .map(|index| {
                let wire = borrowed.dag.edge_weight(index).unwrap();
                match wire {
                    Wire::Qubit(qubit) => borrowed.qubits.get(*qubit).into_py_any(py),
                    Wire::Clbit(clbit) => borrowed.clbits.get(*clbit).into_py_any(py),
                    Wire::Var(var) => borrowed
...
impl DAGCircuit {
    ...
    fn py_substitute_node_with_dag(
        &mut self,
        py: Python,
        node: &Bound<PyAny>,
...
impl PyDAGCircuit {
    ...
    pub fn try_read<'a>(&'a self) -> PyResult<RwLockReadGuard<'a, DAGCircuit>> {
        self.inner
            .try_read()
    ...
    pub fn try_write<'a>(&'a mut self) -> PyResult<RwLockWriteGuard<'a, DAGCircuit>> {
        self.inner
            .try_write()
            .map_err(|e| PyRuntimeError::new_err(e.to_string()))
    }
