        ...
        .map(|dag_qubits| -> PyResult<DAGCircuit> {
            ...
            for index in dag.topological_op_nodes()? {
                ...
                if dag_qubits.is_superset(&qargs) {
                    ...
                    let mapped_qubits = new_dag.qubits().map_objects(qarg_bits)?.collect();
                    let qubits = new_dag.qargs_interner_mut().insert_owned(mapped_qubits);
                    new_dag.push_back(PackedInstruction {
                        op: node.op.clone(),
                        qubits,
                        clbits: node.clbits,
            ...
            let idle_clbits: Vec<ShareableClbit> = new_dag
                .clbit_io_map()
                .iter()
                .enumerate()
                .filter_map(|(index, [input, output])| {
                    if new_dag.dag().find_edge(*input, *output).is_some() {
                        Some(new_dag.clbits().get(Clbit::new(index)).unwrap().clone())
                    } else {
                        None
                    }
                })
...
pub fn disjoint_utils(m: &Bound<PyModule>) -> PyResult<()> {
