...
use crate::target_transpiler::{Target, TargetCouplingError};
...
impl Neighbors {
    /// Construct the neighbor adjacency table from a [Target].
    ///
    /// Returns the error state if the target either has all-to-all connectivity (in which case the
    /// neighbour adjacency lists would be both meaningless and excessively large), or if the target
    /// contains multi-qubit operations that cannot be represented in a coupling graph.
    pub fn from_target(target: &Target) -> Result<Self, TargetCouplingError> {
        let coupling = target.coupling_graph()?;
        // The incoming graph is undirected, but we duplicate and sort all the edges to have better
        // edge locality.
        let mut neighbors = Vec::with_capacity(coupling.edge_count() * 2);
        // The `+ 1` is to store an initial zero.
        let mut partition = Vec::with_capacity(coupling.node_count() + 1);
        partition.push(0);
        for node in coupling.node_indices() {
            for neighbor in coupling.neighbors(node) {
                neighbors.push(PhysicalQubit::new(neighbor.index() as u32));
            }
