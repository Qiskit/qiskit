...
use super::route::{swap_map, swap_map_trial, PyRoutingTarget, RoutingResult, RoutingTarget};
...
pub fn sabre_layout_and_routing(
    ...
    let Some(target) = target.0.as_ref() else {
        todo!("return trivial-layout ancilla-expanded DAG")
    };
    ...
    add_heuristic_layouts(&mut starting_layouts, dag, target, run_in_parallel);
    ...
    let seed_vec: Vec<u64> = outer_rng
        ...
        .collect();
    let result = CondIterator::new(seed_vec, run_in_parallel && starting_layouts.len() > 1)
        .enumerate()
        .map(|(index, seed)| {
            (
                index,
    ...
    result
        .rebuild(py)
        .map(|dag| (dag, result.initial_layout, result.final_layout))
}
...
fn layout_trial<'a>(
    ...
    let initial_layout = (0..max_iterations)
        ...
        .fold(initial_layout, |initial, sabre| {
            swap_map_trial(target, sabre, dag, heuristic, &initial, seed).final_layout
        });
    swap_map(
        target,
        sabre,
