...
use crate::target_transpiler::{exceptions::TranspilerError, Target, TargetCouplingError};
        ...
        for item in &self.order {
            ...
            match item.kind {
                ...
                RoutedItemKind::ControlFlow(num_blocks) => {
                    ...
                    let circuit = imports::QUANTUM_CIRCUIT.get_bound(py);
                    let blocks = blocks
                        .into_iter()
                        .map(|mut dag| {
                            let qubits = idle
                                .iter()
                                .map(|qubit| dag.qubits().get(*qubit).unwrap().clone())
                                .collect();
                            dag.remove_qubits(py, qubits)?;
                            let circuit_data = dag_to_circuit(py, &dag, false)?;
                            circuit.call_method1("_from_circuit_data", (circuit_data,))
                        })
                        .collect::<Result<Vec<_>, _>>()?;
...
impl RoutingTarget {
    pub fn from_target(target: &Target) -> Result<Self, TargetCouplingError> {
        let neighbors = Neighbors::from_target(target)?;
        Ok(Self {
...
impl<'a> RoutingState<'a> {
    ...
    fn route_control_flow_block(
        ...
        let mut result = swap_map_trial(self.target, sabre, dag, self.heuristic, layout, self.seed);
    ...
    fn force_enable_closest_node(
        ...
        let shortest_path = {
            ...
            shortest_paths.swap_remove(&qubits[1]).unwrap()
        };
...
pub fn sabre_routing(
    ...
    result.rebuild(py).map(|dag| (dag, result.final_layout))
}
...
pub fn swap_map<'a>(
    ...
    .map(|seed| swap_map_trial(target, sabre, dag, heuristic, initial_layout, seed))
    .enumerate()
...
pub fn swap_map_trial<'a>(
    ...
    let mut state = RoutingState {
        ...
        dag,
        heuristic,
        order: Vec::with_capacity(sabre.dag.node_count()),
