...
fn get_target_basis_set(target: &Target, qubit: PhysicalQubit) -> EulerBasisSet {
    ...
    let target_basis_list = target.operation_names_for_qargs(&[qubit], false);
...
fn get_2q_decomposers_from_target(
    ...
    match target.operation_names_for_qargs(qubits, false) {
        Ok(direct_keys) => {
            qubit_gate_map.insert(qubits, direct_keys);
            if let Ok(reverse_keys) = target.operation_names_for_qargs(&reverse_qubits, false) {
...
fn synth_error(
    ...
    let mut score_instruction = |inst_name: &str,
        ...
        if let Ok(names) = target.operation_names_for_qargs(inst_qubits, false) {
            for name in names {
