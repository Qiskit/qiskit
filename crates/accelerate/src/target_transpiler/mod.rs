...
use pyo3::{
    ...
    prelude::*,
    pyclass,
    types::{PyDict, PyIterator, PyList, PySet},
    IntoPyObjectExt,
};
    ...
    fn add_instruction(
        ...
        match &instruction {
            ...
            TargetOperation::Normal(_) => {
                let properties =
                    properties.unwrap_or(IndexMap::from_iter([(Qargs::Global, None)]).into());
            ...
            if let QargsRef::Concrete(qargs_as_vec) = qargs {
                ...
                if let Some(gate_prop_name) = self.gate_map.get(operation_name) {
                    ...
                    if gate_prop_name.contains_key(&Qargs::Global) {
                        ...
                        match obj {
                            TargetOperation::Variadic(_) => {
                                return qargs_as_vec.iter().all(|qarg| {
