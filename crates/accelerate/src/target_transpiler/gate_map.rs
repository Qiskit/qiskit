...
use super::{
    ...
    nullable_index_map::{IntoIter, NullableIndexMap},
    Qargs,
};
...
#[derive(Debug, Default)]
pub struct PropsMap {
    map: NullableIndexMap<Qargs, Option<InstructionProperties>>,
    cached: OnceLock<Py<PyDict>>,
}
...
impl PropsMap {
    ...
    pub fn with_capacity(capacity: usize) -> Self {
        Self {
            map: NullableIndexMap::with_capacity(capacity),
            ..Default::default()
        }
    }
    ...
    pub fn cached_py_dict(&self, py: Python) -> PyResult<&Py<PyDict>> {
        ...
        if res_bound.len() == self.map.len() {
            ...
            for idx in res_bound_len..self.map.len() {
                ...
                let key = key.map(|key| PyTuple::new(py, key));
                res_bound.set_item(key.transpose()?, val.clone())?;
            }
            Ok(res)
        } else {
    ...
    fn generate_py_dict(&self, py: Python) -> PyResult<Py<PyDict>> {
        ...
        for (qargs, inst_prop) in self.map.iter() {
            dict.set_item(
                qargs.map(|qargs| PyTuple::new(py, qargs)).transpose()?,
                inst_prop.clone(),
            )?;
        }
        Ok(dict.unbind())
...
impl IntoIterator for PropsMap {
    type Item = (Option<Qargs>, Option<InstructionProperties>);
...
impl From<NullableIndexMap<Qargs, Option<InstructionProperties>>> for PropsMap {
    fn from(value: NullableIndexMap<Qargs, Option<InstructionProperties>>) -> Self {
        Self {
            map: value,
            ..Default::default()
        }
...
impl<'py> FromPyObject<'py> for PropsMap {
    fn extract_bound(ob: &Bound<'py, PyAny>) -> PyResult<Self> {
        ...
        let mut map: NullableIndexMap<Qargs, Option<InstructionProperties>> =
            NullableIndexMap::with_capacity(ob.len() - 1);
        for (key, val) in ob.iter() {
            let key: Option<Qargs> = key.extract()?;
...
impl Deref for PropsMap {
    type Target = NullableIndexMap<Qargs, Option<InstructionProperties>>;
