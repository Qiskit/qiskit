...
impl PropsMap {
    ...
    pub fn cached_py_dict(&self, py: Python) -> PyResult<&Py<PyDict>> {
        ...
        if res_bound.len() == self.map.len() {
            ...
            for idx in res_bound_len..self.map.len() {
                ...
                let key = (!key.is_empty()).then_some(PyTuple::new(py, key)?);
                res_bound.set_item(key, val.clone())?;
            }
            Ok(res)
        } else {
    ...
    fn generate_py_dict(&self, py: Python) -> PyResult<Py<PyDict>> {
        ...
        for (qargs, inst_prop) in self.map.iter() {
            dict.set_item(
                (!qargs.is_empty()).then_some(PyTuple::new(py, qargs)?),
                inst_prop.clone(),
            )?;
        }
        Ok(dict.unbind())
...
impl<'py> FromPyObject<'py> for PropsMap {
    fn extract_bound(ob: &Bound<'py, PyAny>) -> PyResult<Self> {
        ...
        let mut map: IndexMap<Qargs, Option<InstructionProperties>> = IndexMap::new();
        for (key, val) in ob.iter() {
            let key: Qargs = if key.is_none() {
                smallvec![]
            } else {
                key.extract()?
            };
