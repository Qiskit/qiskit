...
use numpy::{
    npyffi, PyArray1, PyArray2, PyArrayDescr, PyArrayDescrMethods, PyArrayLike1, PyArrayMethods,
    PyReadonlyArray1, PyReadonlyArray2, PyReadwriteArray, PyUntypedArrayMethods,
};
...
///     :members:
///
///     .. autoattribute:: qiskit.quantum_info::SparseObservable.Term.num_qubits
///     .. autoattribute:: qiskit.quantum_info::SparseObservable.Term.coeff
///     .. autoattribute:: qiskit.quantum_info::SparseObservable.Term.bit_terms
///     .. autoattribute:: qiskit.quantum_info::SparseObservable.Term.indices
///
/// Construction
/// ============
///
...
impl SparseTerm {
    ...
    fn get_bit_terms(slf_: Bound<Self>) -> Bound<PyArray1<u8>> {
        ...
        make_array_readonly(out.readwrite());
        out
    }
...
// This method might be added to rust-numpy in a future release, at which point we can use that.
// See https://github.com/PyO3/rust-numpy/pull/462.
/// Clear the `WRITEABLE` flag of the underlying Numpy array.
fn make_array_readonly<T, D>(array: PyReadwriteArray<T, D>)
where
    T: ::numpy::Element,
    D: ::ndarray::Dimension,
{
    // SAFETY: we're clearing the WRITEABLE flag from a pointer that we know is non-null.  There
    // can't be any existing read-write views onto the array because we consume the only possible
    // extant `PyReadwriteArray` on entry.
    unsafe {
        (*array.as_array_ptr()).flags &= !npyffi::flags::NPY_ARRAY_WRITEABLE;
    }
}
