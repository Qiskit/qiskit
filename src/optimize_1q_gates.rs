...
#[pyfunction]
pub fn compose_u3_rust(theta1:f64, phi1:f64, lambda1:f64,
                   theta2:f64, phi2:f64, lambda2:f64) -> [f64; 3] { 

    let q = [(theta1/2.0).cos(), 0., (theta1/2.0).sin(), 0.];
    let r = [((lambda1+phi2)/2.0).cos(), 0., 0., ((lambda1+phi2)/2.0).sin()];
    ...
    if mat[3*2+2] < 1.0 {
        if mat[3*2+2] > -1.0 {
            euler[0] = mat[3*1+2].atan2(mat[3*0+2]);
            euler[1] = (mat[3*2+2]).acos();
            euler[2] = mat[3*2+1].atan2(-mat[3*2+0]);
        } else {
            euler[0] = -1.*(mat[3*1+0].atan2(mat[3*1+1]));
            euler[1] = PI;
        }
    } else {    
        euler[0] = mat[3*1+0].atan2(mat[3*1+1]);
    }
    for kk in 0..3 {
