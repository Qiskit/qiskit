    ...
    def read_config_file(self):
        ...
        if "default" in self.config_parser.sections():
            ...
            if min_qpy_version is not None and min_qpy_version.strip() != "":
                try:
                    parsed_version = parse_version(min_qpy_version)
                    self.settings["min_qpy_version"] = parsed_version
                except InvalidVersion as err:
                    raise exceptions.QiskitUserConfigError(
                        f"Value assigned to min_qpy_version is not a valid version string: {str(err)}"
                    )
