...
for m, reg in enumerate(regularizations):
    ...
    for k, e in enumerate(entanglements):
        ...
        for i, n_p in enumerate(set(num_params)):
            ...
            # plt.plot(num_time_steps_plot, true_error_plot, color=color, marker='x', linestyle='dashed',
    ...
    plt.savefig(os.path.join('..', 'output', 'real', reg + 'energy.png'))
    plt.close()
    plt.figure(2)
    plt.savefig(os.path.join('..', 'output', 'real', reg + 'fidelity.png'))
    plt.close()
    plt.figure(1)
    plt.savefig(os.path.join('..', 'output', 'real', reg + 'error.png'))
    plt.close()
