    ...
    for k, ode_solver in enumerate(ode_solvers):
        for d in depths:
            for j, reg in enumerate(regs):
                ...
                # observable = (Y ^ Y)
                observable = SummedOp([(Z ^ X), (X ^ Z), (Y ^ Y)]).reduce()
                # observable = (Y ^ I)
                ...
                print('---------------------------------------------------------------------')
                t0 = time.time()
                varqite_snapshot_dir = os.path.join('..', 'output_test_sum', 'imag',
                                                    str(nts),
                                                    reg_names[j],
                ...
                varqite_error_bounds, varqite_reverse_error_bounds = varqite.error_bound(
                    varqite_snapshot_dir, use_integral_approx=False, imag_reverse_bound=True,
                ...
                #                                                'error_bounds.npy')])
                varqite.print_results([varqite_snapshot_dir], [os.path.join(varqite_snapshot_dir,
                                                               'error_bounds.npy')],
                                      [os.path.join(varqite_snapshot_dir,
                                                    'reverse_error_bounds.npy')],
                                      use_integral_approx=False
                                      )

                print('run time', (time.time()-t0)/60)
                print('---------------------------------------------------------------------')
                varqrte_snapshot_dir = os.path.join('..', 'output_test_sum', 'real',
                                                    str(nts),
                                                    reg_names[j],
                                                    ode_solvers_names[k] + 'nat_grad')
                t0 = time.time()
                ...
                print('run time', (time.time()-t0)/60)
                varqrte.print_results([varqrte_snapshot_dir], [os.path.join(varqrte_snapshot_dir,
                                                                            'error_bounds.npy')],
                                      use_integral_approx=False)
