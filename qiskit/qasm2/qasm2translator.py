...
class Qasm2Translator:
    ...
    def apply_gate(
        ...
        param_dict : OrderedDict of ``{str: Parameter}`` (optional)
            The default is None.
            Ordered dictionary mapping names to Parameters for substitution
        ...
        try:
            ...
            print(
                "In Qasm2Translator.apply_gate gate {} params: {} targets: {} yielding paramlist {} targlist {} param_dict {}".format(
    ...
    def gate_def_to_gate(self, gate_def: dict) -> Gate:
        ...
        for _step in gate_def["definition"]:
            ...
            if _op in STANDARD_GATES:
                ...
                if _gatedef:
                    try:
                        ...
                        traceback.print_tb(ex.__traceback__)
                        raise QasmError(
                            "gate_def_to_gate: Translation of code step {} failed with Exception {}".format(
                                _step, ex
                            )
                        )
            ...
            if _gate:
                ...
                raise QasmError(
                    "gate_def_to_gate: Not a standard gate, not a Qasm2 keyword gate, and no def not found for {} in {}.".format(
                        _op, _step
                    )
                )
    ...
    def translate_code_entry(self, qc: QuantumCircuit, entry: dict) -> None:
        ...
        if isinstance(ctx, qasm2Parser.SubroutineCallContext):
            ...
            if _gate:
                ...
                raise QasmError(
                    "translate_code_entry: Not a standard gate, not a Qasm2 keyword gate, and no def not found for {} in {}.".format(
                        _op, entry
                    )
                )
            ...
            if _comparison_operator != "==":
                raise QasmError(
                    "translate_code_entry {} failed for an unsupported branch comparison operator {}".format(
    ...
    def translate(self) -> QuantumCircuit:
        ...
        try:
            ...
            traceback.print_tb(ex.__traceback__)
            raise QasmError("Error encountered in translation: {}".format(ex))
        return self.qc
