...
class Pauli:
    ...
    @classmethod
    def from_pauli_string(cls, pauli_string):
        """Take pauli string to construct pauli."""
        v = np.zeros(len(pauli_string), dtype=np.bool)
        w = np.zeros(len(pauli_string), dtype=np.bool)
        for i, label in enumerate(pauli_string):
            if label == 'X':
                w[i] = True
            elif label == 'Z':
                v[i] = True
    ...
    def insert_qubits(self, pos, pauli_labels):
        ...
        new_v = self._v
        new_w = self._w
        self._v = np.insert(new_v, pos, tmp._v)
        self._w = np.insert(new_w, pos, tmp._w)
    ...
    def append_qubits(self, pauli_labels):
        ...
        self._v = np.concatenate((self._v, tmp._v))
        self._w = np.concatenate((self._w, tmp._w))
    ...
    def generate_random_pauli(num_qubits):
        ...
        """
        v = np.random.randint(2, size=num_qubits).astype(bool)
        w = np.random.randint(2, size=num_qubits).astype(bool)
        return Pauli(v, w)
...
if __name__ == '__main__':
    p = Pauli.from_pauli_string('IIII')
