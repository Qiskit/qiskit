...
grover00_template = "./qoptimizer4grove/grover00_template.qasm"
grover01_template = "./qoptimizer4grove/grover01_template.qasm"
grover10_template = "./qoptimizer4grove/grover10_template.qasm"
grover11_template = "./qoptimizer4grove/grover11_template.qasm"
...
def makeArgs():
    ...
    parser.add_argument('--qasm_files', type=str, default="./qoptimizer4grove/amplify.qasm", #
    ...
    parser.add_argument('--mcmc_repeat', type=int, default=10,
    ...
    parser.add_argument('--mcmc_folder', type=str, default="./qoptimizer4grove/mcmc_mutants", #
                       help='folder for holding mutant files') # they are separated with

    parser.add_argument('--mcmc_cost_logs', type=str, default="./qoptimizer4grove/mcmc_cost_logs", #
...
def complete(template, addon):
    newFilePath = addon + ".into." + os.path.basename(template)
    with open(newFilePath, 'w') as newFile:
        templatelines = open(template).readlines()
        for line in templatelines:
            if not line.startswith("// amplify.qasm to be copied here"):
                newFile.write(line)
            else:
                addonlines=open(addon).readlines()
...
def correctness_cost_on_4grover(args, cexecutor, orig_file, new_file, disorder_penality):
    grover00_orig = complete(grover00_template, orig_file)
    grover00_new = complete(grover00_template, new_file)
    cexecutor.cache_execute(args, grover00_orig) # for getting the final state to be used by correctness score
    cexecutor.cache_execute(args, grover00_new) # for getting the final state to be used by correctness score
    cost_grover00 = correctness_cost(cexecutor, grover00_orig, grover00_new, disorder_penality)

    grover01_orig = complete(grover01_template,orig_file)
    grover01_new = complete(grover01_template,new_file)
    cexecutor.cache_execute(args, grover01_orig)
    cexecutor.cache_execute(args, grover01_new)
    cost_grover01 = correctness_cost(cexecutor, grover01_orig, grover01_new, disorder_penality)

    grover10_orig = complete(grover10_template,orig_file)
    grover10_new = complete(grover10_template,new_file)
    cexecutor.cache_execute(args, grover10_orig)
    cexecutor.cache_execute(args, grover10_new)
    cost_grover10 = correctness_cost(cexecutor, grover10_orig, grover10_new, disorder_penality)

    grover11_orig = complete(grover11_template, orig_file)
    grover11_new = complete(grover11_template,new_file)
    cexecutor.cache_execute(args, grover11_orig)
    cexecutor.cache_execute(args, grover11_new)
    cost_grover11 = correctness_cost(cexecutor, grover11_orig, grover11_new, disorder_penality)
    return cost_grover00 + cost_grover01 + cost_grover10 + cost_grover11
...
def mcmc(repeatI, args, oracle_file, cexecutor):
    ...
    for i in range(args.iterations):
        ...
        cexecutor.cache_execute(args, new_file) # for getting the circuit graph layer number needed by the perf score

        # accept it or not, this is a question for mcmc
        p = random.uniform(0,1)
        new_cost_perf = perf_cost(cexecutor, oracle_file, new_file)
        new_cost_correctness = correctness_cost_on_4grover(args, cexecutor, oracle_file, new_file, args.disorder_penality)
        ...
        current_cost_correctness = correctness_cost_on_4grover(args, cexecutor, oracle_file, current_file, args.disorder_penality)
        ...
        remove_useless(cexecutor, oracle_file, useless + ".into.grover00_template.qasm")
        remove_useless(cexecutor, oracle_file, useless + ".into.grover01_template.qasm")
        remove_useless(cexecutor, oracle_file, useless + ".into.grover10_template.qasm")
        remove_useless(cexecutor, oracle_file, useless + ".into.grover11_template.qasm")
