...
class BasisState:
    ...
    def __init__(self, nqubits=None, state_string=None, index=None,
        ...
        if state_string is not None:
            ...
            self._qubit_set = [i for i, j in enumerate(self._state_string[::-1])
                               if j == '1']
            if qubit_set is not None and self._qubits_set != qubit_set:
...
class EquivalenceChecker():
    ...
    def run(self, c1, c2,
            ...
            up_to_phase=False):
        if c1_inputs is not None or c2_inputs is not None or c1_outputs is not None or c2_outputs is not None:
            return self.output_comparision_check(c1, c2, c1_inputs, c2_inputs,
                                                 c1_outputs, c2_outputs,
                                                 up_to_phase)

        if len(c1.qubits) != len(c2.qubits):
            raise RuntimeError(
                "Qubit numbers are different ({} != {})".format(len(c1.qubits),
                                                                len(c2.qubits)))
        n = len(c1.qubits)
        ...
        for i in range(2 ** n):
            ...
            s = self.get_statevector(initialized_qobj)
            s_i = s[i]
            if up_to_phase:
                s_i = np.abs(s_i)
    ...
    def get_output_for_input(self, qobj, qubit_set, c_inputs, c_outputs):
        ...
        s = self.get_statevector(initialized_qobj)
        reduced_s = self.reduce_statevector(s, c_outputs)
        return reduced_s

    def normalize(self, s):
        v = max(s, key=lambda x: np.abs(x))
        return [x / v for x in s]

    def output_comparision_check(self, c1, c2,
                                 c1_inputs, c2_inputs,
                                 c1_outputs, c2_outputs,
                                 up_to_phase=False
                                 ):
        if len(c1_inputs) != len(c2_inputs):
            raise RuntimeError(
                "Qubit numbers are different ({} != {})".format(len(c1_inputs),
                                                                len(c2_inputs)))
        n = len(c1_inputs)
        if n > self.qubit_limit:
            raise RuntimeError(
        ...
        qobj1 = assemble(transpile(c1, self.backend))
        qobj2 = assemble(transpile(c2, self.backend))
