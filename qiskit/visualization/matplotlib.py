...
class MatplotlibDrawer:
    ...
    def _draw_ops(self, verbose=False):
        ...
        for key, qreg in self._qreg_dict.items():
            q_anchors[key] = Anchor(reg_num=self._n_lines, yind=qreg["y"], fold=self._fold)
        c_anchors = {}
        for key, creg in self._creg_dict.items():
            c_anchors[key] = Anchor(reg_num=self._n_lines, yind=creg["y"], fold=self._fold)
        #
        # draw the ops
        #
        prev_anc = -1
        ...
        for layer in self._ops:
            ...
            for op in layer:
                ...
                # cu1, rzz, and controlled rzz gates (sidetext gates)
                    ...
                    self._sidetext(
                        qreg_b, tc=tc, text="{}".format(stext) + " " + "({})".format(param)
                    )
                    self._line(qreg_b, qreg_t, lc=lc)
