        ...
        for i in range(self.img_width):
            for j in range(self.img_depth + 1):
                ...
                if "barrier" not in cell_str:
                    ...
                    cell_str = re.sub(
                        r"[-+]?\d*\.\d{2,}|\d{2,}", _truncate_float, cell_str
                    )
        ...
        for layer in self.ops:
            ...
            for op in layer:
                ...
                for arg in op.op.params:
                    arg_str = re.sub(
                        r"[-+]?\d*\.\d{2,}|\d{2,}", _truncate_float, str(arg)
                    )
        ...
        for layer in self.ops:
            ...
            for op in layer:
                ...
                if controlled_gate and op.name not in ["ccx", "cx", "cz", "cu1",
                                                       ...
                                                       "cswap"]:
                    qarglist = op.qargs
                    name = generate_latex_label(op.op.base_gate.name.upper()).replace(
                        " ", "\\,"
                    )
                    pos_array = []
                    ...
                    if num_qargs == 1:
                        for index, ctrl_item in enumerate(zip(ctrl_pos, ctrl_state)):
                            ...
                            if pos_array[index] > pos_array[-1]:
                                ...
                                while nxt not in pos_array:
                                    nxt += 1
                            self._latex[pos][column] = _generate_latex_gate(
                                "ctrlo", [], gap=nxt - pos_array[index], if_value=cond
                            )
                        if name == "Z":
                        ...
                        if any(ctrl_pos) in range(pos_start, pos_stop):
                            ...
                            name = generate_latex_label(op.name).replace(" ", "\\,")
                        else:
                            for index, ctrl_item in enumerate(zip(ctrl_pos, ctrl_state)):
                                pos = ctrl_item[0]
                                cond = ctrl_item[1]
                    ...
                    if len(qarglist) == 1:
                        ...
                        if op.condition:
                            ...
                            if_reg = cl_reg.register
                            pos_2 = self.img_regs[cl_reg]
                            self._latex[pos_1][column] = _generate_latex_gate(
                                nm, op.op.params
                            )
                            _assign_cregs(
                                self.cregbundle,
                        ...
                        if op.condition:
                            ...
                            assignment_result = _assign_cgate(
                                nm,
                                op.op.params,
                                [cond],
                                self._latex,
                        ...
                        if controlled_gate:
                            ctrl_state = "{0:b}".format(op.op.ctrl_state).rjust(2, "0")[
                                ::-1
                            ]
                        else:
                            crtl_state = [0, 0]
                        ...
                        pos_stop = max(pos_array)
                        _assign_multigate(
                            nbits, nm, self._latex, pos_array, column, not special_gate
                        )

                elif op.name == "measure":
                    if len(op.cargs) != 1 or len(op.qargs) != 1 or op.op.params:
...
def _assign_multigate(number, gate, latex_array, pos_array, column, special_gate,
    ...
    if special_gate and number >= 2:
        ...
        number = length
    if not special_gate or length != 3 or pos_stop - pos_start >= 3:
        latex_array[pos_start][column] = "\\multigate{%s}{%s}" % (number - 1, gate)
    if special_gate and pos_stop - pos_start < length and length != 3:
...
def _assign_cgate(gate, params, cond, latex_array, column, pos_1, pos_2, pos_3=0,
                  length=0):
    if (gate not in ["cx", "cz", "cy", "ch", "swap", "crx", "cry", "crz", "cu1",
                     "cu3", "rzz"] and length == 0):
        return -1
    if gate == "rzz":
        cond = [1]
    latex_array[pos_1][column] = _generate_latex_gate(
        "ctrlo", [], gap=pos_2 - pos_1, if_value=cond[0]
    )
    if length == 0:
        latex_array[pos_2][column] = _generate_latex_gate(
            gate, params, gap=pos_1 - pos_2
        )
    elif gate == "ccx":
        latex_array[pos_2][column] = _generate_latex_gate(
            "ctrlo", [], gap=pos_3 - pos_2, if_value=cond[1]
        )
    ...
    if gate == "swap":
        ...
        return 0
    if gate == "cu1":
        latex_array[min(pos_1, pos_2)][column + 1] = "\\dstick{%s}\\qw" % (
...
def _generate_latex_gate(gate, params, gap=0, if_value=""):
    ...
    if gate == "tdg":
        latex = "\\gate{T^\\dag}"
    if gate == "u0":
        latex = "\\gate{U_0(%s)}" % _parse_params(params[0])
    if gate == "u1":
        latex = "\\gate{U_1(%s)}" % _parse_params(params[0])
    if gate == "u2":
        latex = "\\gate{U_2\\left(%s,%s\\right)}" % (
            _parse_params(params[0]),
    ...
    if gate in ["cu1", "rzz"]:
        latex = "\\control \\qw"
    if gate == "swap":
        latex = "\\qswap \\qwx[" + str(gap) + "]"
    if gate == "reset":
        latex = "\\push{\\rule{.6em}{0em}\\ket{0}\\rule{.2em}{0em}} \\qw"
    if "control" in gate:
        latex = "\\" + gate + " \\cw \\cwx[-" + str(gap) + "]"
    if "ctrl" in gate:
        latex = "\\" + gate + "{" + str(gap) + "}"
    if "rx" in gate:
        latex = "\\gate{R_x(%s)}" % _parse_params(params[0])
    if "ry" in gate:
        latex = "\\gate{R_y(%s)}" % _parse_params(params[0])
    if "rz" in gate:
        latex = "\\gate{R_z(%s)}" % _parse_params(params[0])
    if "u3" in gate:
        latex = "\\gate{U_3(%s,%s,%s)}" % (
            _parse_params(params[0]),
            _parse_params(params[1]),
