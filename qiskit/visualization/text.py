    ...
    def wire_names(self, with_initial_state=False):
        ...
        if self.layout is None:
            for bit in self.qregs:
                ...
                qubit_labels.append(
                    label.format(name=bit.register.name, index=bit.index, physical="")
                )
        else:
            for bit in self.qregs:
                if self.layout[bit.index]:
                    ...
                    qubit_labels.append(
                        label.format(
                            name=self.layout[bit.index].register.name,
    ...
    def _set_multibox(
        ...
        if qubits is not None and clbits is not None:
            ...
            qargs = [
                ...
                for qbit in self.qregs
                if qbit in qubits
            ]
            cargs = [
                str(clbits.index(cbit)).ljust(wire_label_len, " ")
                for cbit in self.cregs
                if cbit in clbits
            ]
