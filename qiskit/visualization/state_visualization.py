...
def plot_bloch_multivector(state, title="", figsize=None, *, rho=None):
    ...
    for i in range(num):
        ...
        plot_bloch_vector(bloch_data[i], "qubit " + str(i), ax=ax, figsize=figsize)
    fig.suptitle(title, fontsize=16)
    if get_backend() in ["module://ipykernel.pylab.backend_inline", "nbAgg"]:
        plt.close(fig)
    return fig
...
def _repr_state_text(state):
    ...
    pad = len(prefix) * " "
    return "{}{},\n{}dims={})".format(
        prefix, np.array2string(state._data, separator=", ", prefix=prefix), pad, state._dims
    )
...
class TextMatrix:
    ...
    def __str__(self):
        ...
        data = np.array2string(self.state._data, prefix=self.prefix, threshold=threshold)
        if self.dims:
...
def state_drawer(state, output=None, max_size=None, dims=None, prefix=None, **drawer_args):
