...
def lower(program: pulse.Program, compiler: Optional[BaseCompiler] = None,) -> Any:
...
class QobjCompiler(Compiler):
    ...
    def default_pipelines(self):
        ...
        lowering_pm.append(LowerQobj(self.qobj_id, self.qobj_header, self.run_config,),)
...
class LowerQobj(LoweringPass):
    ...
    def lower(self, program,) -> qobj.PulseQobj:
        ...
        lo_converter = converters.LoConfigConverter(
            qobj.PulseQobjExperimentConfig, **self.run_config.to_dict(),
        )
        experiments, experiment_config = self._assemble_experiments(
            program, lo_converter,
        )
        qobj_config = self._assemble_config(lo_converter, experiment_config,)
    ...
    def _assemble_experiments(
        ...
        freq_configs = [
            ...
            for lo_dict in getattr(self.run_config, "schedule_los", [])
        ]
        ...
        instruction_converter = instruction_converter(
            qobj.PulseQobjInstruction, **self.run_config.to_dict(),
        )
        ...
        for idx, schedule in enumerate(program.schedules):
            ...
            qobj_experiment_header = qobj.QobjExperimentHeader(
                memory_slots=self.analysis.max_memory_slot_used[idx]
                + 1,  # Memory slots are 0 indexed
