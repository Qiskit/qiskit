...
class ScheduleBlock:
    ...
    which is a dictionary of schedule blocks keyed on unique referene id
    generated by the subroutine name and channels.
        ...
        .. jupyter-execute:
            ...
            with pulse.build(name="main_prog") as main_program:
                with pulse.align_sequential():
                    key1 = pulse.call(name="sub_prog", channels=[pulse.DriveChannel(0)])
                    key2 = pulse.call(name="sub_prog", channels=[pulse.DriveChannel(1)])
    ...
    class ReferenceManager:
        ...
        def add_refkey(self, refinst: Reference):
            """Add new reference instruction.

            Args:
                refinst: Reference instruction to manage.
            """
            if not refinst.ref_id in self:
                self._program_map[refinst.ref_id] = None
        ...
        def get_program(self, refinst: Reference) -> "ScheduleBlock":
            ...
            Returns:
                Assigned subtourine.
            """
            return self._program_map[refinst.ref_id]

        def assign_program(self, ref_id: str, program: "ScheduleBlock"):
            """Assign subroutine to reference instruction.

            Args:
                name: Unique reference ID of the subroutine.
            ...
            if self._program_map[ref_id] is not None:
                if self._program_map[ref_id] != program:
                    raise PulseError(
                        f"Subroutine {ref_id} has been already assigned. "
                        "Newly assigned schedule has conflict with the previous assignment."
                    )
            else:
                self._program_map[ref_id] = program
    ...
    @property
    def references(self) -> Dict[str, "ScheduleBlock"]:
        """Return all subroutines associated with this schedule."""
        return self._references._program_map
    ...
    def append(
        ...
        if isinstance(block, Reference):
            self._references.add_refkey(block)
    ...
    def assign_reference(
        ...
        Args:
            ref_id: Unique string to identify the subroutine.
        ...
        if not inplace:
            new_schedule = copy.deepcopy(self)
            return new_schedule.assign_reference(ref_id, schedule, inplace=True)

        self._references.assign_program(ref_id, schedule)
        return self
