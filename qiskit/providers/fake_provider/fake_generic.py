...
class GenericFakeBackend(BackendV2):
    """Generic :class:`~.BackendV2` fake backend. This class will instantiate a partially
    configurable, locally runnable, fake backend. One can configure the number of qubits,
    basis gates, coupling map, ability to run dynamic circuits (control flow instructions),
    instruction calibrations and dtm.
