...
class ansatz:
    ...
    def get_circ_3(self):
        ...
        num_par = (2*self.feature_dim*self.repitition) + (self.feature_dim-1)*self.repitition
        paravec = np.random.randn(num_par)
        circ = QuantumCircuit(self.feature_dim)
        arg_count = 0 
    ...
    def get_circ_5(self):
        ...
        ansatz = NLocal(self.feature_dim, rotation_blocks = rot_layer, entanglement_blocks=blocks, reps = self.repitition)
        return ansatz
    ...
    def get_circ_11(self):
        ...
        num_pars = (2 * self.feature_dim + int(self.feature_dim/2-1)*4) * self.repitition if self.feature_dim%2 ==0 else (4 *self.feature_dim - 2) * self.repitition
        paravec = np.random.randn( num_pars)
        circ = QuantumCircuit(self.feature_dim)
        arg_count = 0
        for _ in range(self.repitition):
