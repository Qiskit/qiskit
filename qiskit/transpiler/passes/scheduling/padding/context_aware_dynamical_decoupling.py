...
from qiskit.dagcircuit import DAGOpNode, DAGInNode, DAGNode
...
class ContextAwareDynamicalDecoupling(TransformationPass):
    ...
    def run(self, dag: DAGCircuit) -> DAGCircuit:
        ...
        for block in adjacent_delay_blocks:
            merged_delays_in_block = _split_adjacent_block(block, bit_idx_locs, coupling_map)
        ...
        for merged_delay in merged_delays:
            ...
            coloring = _get_wire_coloring(
                dag,
                merged_delay,
                self.property_set["node_start_time"],
                coupling_map,
                self._coloring_strategy,
            )
            ...
            for op in merged_delay.ops:
                dd_circuit, start_times = _get_dd_sequence(
                    coloring[op.index],
                    op.index,
                    duration,
...
def _get_sorted_delays(
    ...
    eligible_delays = [
        ...
        for node, start_time in node_start_times.items()
        if (
            node.op.name == "delay"
            and not is_after_reset(node)
            and node.op.duration > min_joinable_duration
        )
...
def _collect_adjacent_delay_blocks(
    ...
    Args:
        ...
        cmap: The device's coupling map.
        bit_idx_locs: A map from qubit instance to qubit index.
    ...
    def _open_delay_block(delay_event):
        open_delay_blocks.append(
            AdjacentDelayBlock(events=[delay_event], active_qubits=set(delay_event.op_node.qargs))
        )
...
def _split_adjacent_block(
    ...
    for window in windows:
        ...
        for start_index in active:
            ...
            _dfs(start_index, cmap, active_neighbors, active)
...
def _get_wire_coloring(
    dag: DAGCircuit,
    merged_delay: MultiDelayOp,
    node_start_times: dict[DAGNode, int],
    coupling_map: CouplingMap,
    strategy: rx.ColoringStrategy,
) -> dict[int, int]:
    ...
    for delay_op in merged_delay.ops:
        # use coupling_map.graph.neighbors_undirected once Qiskit/rustworkx#1254 is in a release
        new_neighbors = {
            i for i in range(dag.num_qubits()) if coupling_map.distance(i, delay_op.index) == 1
        }
    ...
    subgraph = coupling_map.graph.subgraph(list(wires)).to_undirected()
    ...
    for wire in neighbors:
        for op_node in dag.nodes_on_wire(dag.qubits[wire], only_ops=True):
            ...
            op_start = node_start_times[op_node]
...
def _get_dd_sequence(
    ...
    if index not in checked_durations_cache:
        ...
        if x_duration % pulse_alignment != 0:
            raise TranspilerError(
                f"X gate length on qubit with index {index} is {x_duration} which is not "
                f"an integer multiple of the pulse alignment {pulse_alignment}."
            )
    ...
    if 1 - slack_fraction >= skip_threshold:  # dd doesn't fit
    ...
    taus = _constrain_spacing(spacing, slack, pulse_alignment)
...
def _constrain_spacing(spacing, slack, pulse_alignment):
    def _constrained_length(values):
        return pulse_alignment * np.floor(values / pulse_alignment)
