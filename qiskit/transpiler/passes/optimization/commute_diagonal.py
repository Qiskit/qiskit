...
from qiskit.circuit import Barrier, Qubit, Instruction
...
class CommuteDiagonal(TransformationPass):
        ...
        for block_qubits, block_list in candidate_blocks.items():
            ...
            for block_ind0, block_ind1 in zip(range(num_blocks - 1), range(1, num_blocks)):
                ...
                if diagonality == Diagonality.TRUE:
                    ...
                    new_inter_nodes = (
                        local_predeccessor_nodes + [nonlocal_node] + local_successor_nodes
                    )

                    # UPDATE INTER BLOCK
                    node_map = self._replace_block_with_block(dag, inter_nodes, new_inter_nodes)
                    ...
                    node_map = self._replace_block_with_dag(dag, block0_local, qc2cx_dag)
                    ...
                    block1_local_diag_circ = _block_to_circuit(
                        dag, block1_local_diag, remove_idle_qubits=True
                    )
                    block1_local_diag_circ2 = self._weyl_decomp(
                        Operator(block1_local_diag_circ).data
                    )
                    block1_local_diag_dag = self._map_anonymous_circuit(
                        block1_local_diag_circ2, dag1
                    )
                    node_map = self._replace_block_with_dag(
                        dag, block1_local, block1_local_diag_dag
                    )
                    self._block_list[block_ind1] = block1_local_diag_dag.op_nodes()
                    self._update_block_lists_with_node(node_map)

                    # add in global_phase from inter block decomposition
    ...
    def _replace_block_with_block(self, dag, block_a, block_b, phase=None):
        ...
        placeholder_node = dag.replace_block_with_op(
            block_a, placeholder_op, self._global_index_map
        )
    ...
    def get_candidate_2q_blocks(self, blocks, dag):
        """
        Return dictionary of 2q blocks which share same qubits.
        ...
        Returns:
            Dict: key is two qubits of block. Value is the list of DAGNode blocks on
               the qubit pair.
        """
        common_blocks = {}
        for block in blocks:
    ...
    def evaluate_diagonal_commutation(self, inter_dag, partial_qubits, do_equiv_check=True):
        ...
        if do_equiv_check and not is_diagonal and num_active_qubits == 2:
            ...
            dag2q = _copy_dag_ops_like(inter_dag)
...
def _block_to_circuit(dag, block, remove_idle_qubits=False):
    """convert block of nodes to circuit"""
    block_dag = _block_to_dag(dag, block, remove_idle_qubits=remove_idle_qubits)
    return dag_to_circuit(block_dag)


def _copy_dag_ops_like(dag):
    """Return a copy of dag with same qubit and clbit instances but copies
    of operations.

    This is like DAGCircuit.copy_empty_like but also includes a copy
    of the operations from this dag. Also copies anything
    'copy_empty_like' copies, namely global_phase.
...
def _copy_dag_bit_like(dag):
    """Return an empty copy of dag with the same qubit and clbit instances
    only."""
