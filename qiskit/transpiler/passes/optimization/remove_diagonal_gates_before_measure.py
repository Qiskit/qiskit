...
class RemoveDiagonalGatesBeforeMeasure(TransformationPass):
    ...
    def run(self, dag):
        ...
        for measure in dag.op_nodes(Measure):
            ...
            if isinstance(predecessor, OpNode) and isinstance(predecessor.op, diagonal_1q_gates):
                nodes_to_remove.add(predecessor)

            if isinstance(predecessor, OpNode) and isinstance(predecessor.op, diagonal_2q_gates):
                successors = dag.quantum_successors(predecessor)
                if all(isinstance(s, OpNode) and isinstance(s.op, Measure) for s in successors):
                    nodes_to_remove.add(predecessor)
