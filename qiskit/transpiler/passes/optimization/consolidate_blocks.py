    ...
    def run(self, dag):
        ...
        for block in blocks:
            if len(block) == 1 and self._check_not_in_basis(dag, block[0].name, block[0].qargs):
                ...
                if len(block_qargs) > 2:
                    ...
                    unitary = UnitaryGate(Operator(qc), skip_check=True)
                else:
                    matrix = _block_to_matrix(block, block_index_map)
                    unitary = UnitaryGate(matrix, skip_check=True)
        ...
        for run in runs:
            ...
            if len(run) == 1 and not self._check_not_in_basis(dag, run[0].name, run[0].qargs):
                dag.substitute_node(run[0], UnitaryGate(run[0].op.to_matrix(), skip_check=True))
            else:
                qubit = run[0].qargs[0]
                ...
                if already_in_block:
                    continue
                unitary = UnitaryGate(operator, skip_check=True)
