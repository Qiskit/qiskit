...
class IndexCalculator:
    ...
    def __init__(self,
                 ...
                 dummy_time_steps):
        self.qc = add_dummy_time_steps(quantum_circuit, dummy_time_steps)
...
def create_cpx_model(quantum_circuit, hardware_topology, dummy_time_steps,
    ...
    """
    prob = cplex.Cplex()
    ic = IndexCalculator(quantum_circuit, hardware_topology,
                         dummy_time_steps)
    # Add w variables
...
def unset_cross_talk_constraint(prob, ic):
    ...
    prob.linear_constraints.delete([i for (i, c) in enumerate(prob.linear_constraints.get_names()) if c.startswith('mccormick')])
...
def solve_cpx_model(problem, index_calculator, time_limit=300,
            ...
            for (i, j) in index_calculator.ht.arcs:
                if (problem.solution.get_values(
                    ...
                    if (problem.solution.get_values(
                        ...
                        print('Qubits {:d} and {:d} not in the right position at time {:d}'.format(p, q, t))
    num_swaps = 0
    num_merged_swaps = 0
    depth = 0
    for t in range(index_calculator.depth-1):
        ...
        if (problem.solution.get_values(index_calculator.wd_index(t)) > 0.5):
            depth += 1
    print('Num swaps:', num_swaps//2, 'num merged swaps:', num_merged_swaps//2, 'total depth:', depth)
