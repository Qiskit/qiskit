...
def _map_free_gates(layout, gates, coupling_map):
    ...
    for gate in gates:
        ...
        if not gate["partition"]:
            qubits = [n for n in gate["graph"].nodes() if isinstance(n, OpNode)][0].qargs
...
def _transform_gate_for_layout(gate, layout):
    ...
    mapped_op_node = deepcopy([n for n in gate["graph"].nodes() if isinstance(n, OpNode)][0])
...
def _swap_ops_from_edge(edge, layout):
    ...
    return [OpNode(op=SwapGate(), qargs=qreg_edge, cargs=[])]
