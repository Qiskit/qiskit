...
def compute_block_interior(pauli_block: PauliEvolutionGate) -> list:
    block_interior = non_identity_positions(
        [str(pauli_block.operator.paulis[0]), pauli_block.operator.coeffs[0]]
    )
    ...
    if len(pauli_block.operator.coeffs) > 1:
        for pauli_str in pauli_block.operator.paulis[1:]:
            block_interior = list(
                set(block_interior)
                & set(non_identity_positions([str(pauli_str), pauli_block.operator.coeffs[0]]))
            )
