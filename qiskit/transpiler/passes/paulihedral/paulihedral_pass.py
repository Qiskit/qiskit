...
class PaulihedralPass(BasePass):
    """
    Note:
        WARNING!!
        This is a very high-level pass. It uses many other passes in the
    ...
    def __init__(
        ...
        self.do_max_iteration = do_max_iteration
        self.property_set["layout"] = None
        if backend == None:
        ...
        if coupling_map == None:
            ...
            self.coupling_map = backend_processor.get_coupling_map()

    def run(self, input_dag):
        ...
        do_max_iteration = self.do_max_iteration
        ...
        if block_list == []:
            ...
            return None
        print("163", input_dag.qregs)
        # print(input_dag.qregs[0])
        print(input_dag.qregs["q"].size)
        qreg = None
        for idx, val in input_dag.qregs.items():
            qreg = val
        qubit_num = qreg.size  # circuit_to_dag(input_dag).qregs[0]._size
        ...
        dense_layout_pass = DenseLayout(
            coupling_map=coupling_map, backend_prop=backend.properties()
        )
        for layer in new_block_list:
            if layer[1] == True:
                ...
                print("792\n", layout_1)
                """
                for i in range(len(layout_1._p2v)):
        ...
        if len(new_block_list[0][2]._p2v) > total_qubit_num:
            raise TranspilerError(
                f"the virtual qubits number is more than physical qubits number, so it's impossible to run the function Paulihedral_on_mixed_circuit()"
            )
            return None
        ...
        # print(final_layout,"\n",final_result)
        self.property_set["layout"] = final_layout
        return circuit_to_dag(final_result)
