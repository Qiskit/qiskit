...
def qubit_occupation_template(qubit_occupied, qubit_num, latency):
    ...
    while start_index < qubit_num:
        ...
        if (end_index - start_index) >= 2:
            window = (
                [1] * start_index + [0] * (end_index - start_index) + [1] * (qubit_num - end_index)
            )
        ...
        while len(current_template) > 0 and iteration_count < max_iteration:
            ...
            for template in current_template:  # list list
                ...
                for small_block in small_block_list.kernel:
                    ...
                    if small_latency <= template[1] and occupation_embedding_check(
                        template[0], small_occupation
                    ):
            ...
            for qubit_occupation in qubit_occupation_list:
                layer_qubit_occupation = qubit_occupation_combine(
                    qubit_occupation, layer_qubit_occupation
                )  # list
            current_template = qubit_occupation_template(
                layer_qubit_occupation, qubit_num, latency
            )  # list
