...
class Decompose(TransformationPass):
    ...
    def __init__(self, gate: Type[Gate] = None, gates: list = None):
        """Decompose initializer.

        Args:
            gate: gate to decompose.
            gates (list(str)): optional subset of gates to be decomposed,
    ...
    def run(self, dag: DAGCircuit) -> DAGCircuit:
        ...
        for node in dag.op_nodes(self.gate):
            if self.gates is None or node.name in self.gates:
