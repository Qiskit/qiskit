...
class StochasticMapper(TransformationPass):
    ...
    def _layer_permutation(self, layer_partition, layout, qubit_subset,
        ...
        dist = sum([coupling.distance(layout[g[0]][1], layout[g[1]][1])
                    for g in gates])
    ...
    def _layer_update(self, i, first_layer, best_layout, best_d,
                      best_circ, layer_list, coupling_graph):
        """Update the circuit with a new mapped layer.
    ...
    def _mapper(self, circuit_graph, coupling_graph,
        ...
        if initial_layout is not None:
            ...
            for k, v in initial_layout.items():
                ...
                if k not in circ_qubits:
                    raise TranspilerError("initial_layout qubit %s[%d] not"
                                          "in input DAGCircuit" % (k[0].name,
                                                                   k[1]))
        ...
        for i, layer in enumerate(layerlist):
            ...
            logger.debug("mapper: success_flag=%s,best_d=%s,trivial_flag=%s",
