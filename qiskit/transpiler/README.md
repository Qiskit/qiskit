...
- Registering passes in the pass manager pipeline is done by the `add` method.
...
```
self.passmanager.add([Pass()], do_x_times=lambda x : 3)
```
...
```
pm = PassManager()
pm.add(CxCancellation()) # requires: ToffoliDecompose / preserves: ToffoliDecompose
pm.add(RotationMerge())  # requires: ToffoliDecompose / preserves: ToffoliDecompose
pm.add(Mapper(coupling_map=coupling_map))         # requires: [] / preserves: []
pm.add(CxCancellation())
```
...
```
pm.add(Unroller(basis_gates=['id','u1','u2','u3','cx']))
pm.add(...)
pm.add(Unroller(basis_gates=['U','CX']))
```
...
```
pm = PassManager()
pm.add([CxCancellation(), RotationMerge(), CalculateDepth()],
...
```
pm.add(LayoutMapper(coupling_map))
pm.add(CheckIfMapped(coupling_map))
pm.add(BasicMapper(coupling_map),
