...
class DefaultLayoutPassManager(PassManagerStagePlugin):
    ...
    def pass_manager(self, pass_manager_config, optimization_level=None) -> PassManager:
        ...
        if optimization_level == 0:
            ...
            layout.append(
                [BarrierBeforeFinalMeasurements("qiskit.transpiler.internal.routing.protection.barrier"), choose_layout_2], condition=_vf2_match_not_found
            )
        elif optimization_level == 2:
            ...
            layout.append(
                [BarrierBeforeFinalMeasurements("qiskit.transpiler.internal.routing.protection.barrier"), choose_layout_1], condition=_vf2_match_not_found
            )
        elif optimization_level == 3:
            choose_layout_0 = VF2Layout(
                coupling_map=pass_manager_config.coupling_map,
...
class SabreLayoutPassManager(PassManagerStagePlugin):
    ...
    def pass_manager(self, pass_manager_config, optimization_level=None) -> PassManager:
        ...
        if optimization_level == 0:
            ...
            raise TranspilerError(f"Invalid optimization level: {optimization_level}")
        layout.append(
            [BarrierBeforeFinalMeasurements("qiskit.transpiler.internal.routing.protection.barrier"), layout_pass], condition=_choose_layout_condition
        )
        embed = common.generate_embed_passmanager(coupling_map)
        layout.append(
            [pass_ for x in embed.passes() for pass_ in x["passes"]], condition=_swap_mapped
