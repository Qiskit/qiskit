...
from typing import Iterator, Optional, Union, Callable, Tuple, Dict, List
import logging
import warnings
...
class SPSA(Optimizer):
    ...
    def minimize(
        ...
        if self.learning_rate is None and self.perturbation is None:
            get_eta, get_eps = self.calibrate(fun, x0)
        else:
        ...
        if self.blocking:
            ...
            if self.allowed_increase is None:
                self.allowed_increase = 2 * self.estimate_stddev(fun, x)
        ...
        for k in range(1, self.maxiter + 1):
            ...
            # compute update
            update = self._compute_update(fun, x, k, next(eps), lse_solver)
