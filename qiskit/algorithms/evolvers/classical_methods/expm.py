...
def expm_multiply(A, B, start=None, stop=None, num=None, endpoint=None, traceA=None, tol=None):
    """
    ...
    if all(arg is None for arg in (start, stop, num, endpoint)):
        X = _expm_multiply_simple(A, B, traceA=traceA, tol=tol)
    else:
        X, status = _expm_multiply_interval(A, B, start, stop, num, endpoint, traceA=traceA)
    return X


def _expm_multiply_simple(A, B, t=1.0, traceA=None, balance=False, tol=None):
    """
