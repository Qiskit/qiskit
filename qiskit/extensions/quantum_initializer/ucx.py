...
class UCX(UCRot):
    ...
    q_controls = list of k control qubits (or empty list if no controls). The control qubits are ordered according to
                ...
                For example if q_controls=[q[1],q[2]] (with q = QuantumRegister(2)), the rotation R_x(a_0)is
                performed if q[1] and q[2] are in the state zero, the rotation  R_x(a_1) is performed if
