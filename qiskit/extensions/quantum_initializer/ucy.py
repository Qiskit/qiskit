...
"""
Attach a uniformly controlled (also called multiplexed gates) Ry rotation gate to a circuit.
The decomposition was given by Shende et al. in https://arxiv.org/pdf/quant-ph/0406176.pdf. 
    ...
    Args:
        ...
        q_controls (QautnumRegister|list[(QuantumRegister,int)]): list of k control qubits (or empty list if
            ...
            For example if q_controls=[q[1],q[2]] (with q = QuantumRegister(2)), the rotation Ry(a_0)is
            performed if q[1] and q[2] are in the state zero, the rotation  Ry(a_1) is performed if
