...
This module is used for connecting to the Quantum Experience as a backend.
"""
import time
import logging
import pprint
...
class QeRemote(BaseBackend):
    ...
    def run(self, q_job):
        ...
        _, job_status, job_result = _wait_for_job(
            output['id'], self._api, wait=wait, timeout=timeout)

        return Result(output['id'], job_status, job_result, qobj)
...
def _wait_for_job(job_id, api, wait=5, timeout=60):
    ...
    Args:
        job_id (str): identifier of the job on the backend
    ...
    Returns:
        tuple(str, str, list):
            ...
            list: the list of results that correspond to the jobids.
    ...
    while job_result['status'] == 'RUNNING':
        if timer >= timeout:
            return job_id, ResultStatus.ERROR.value, 'Time Out'
        time.sleep(wait)
        timer += wait
        ...
        if (job_result['status'] == 'ERROR_CREATING_JOB' or
            ...
            return job_id, ResultStatus.ERROR.value, job_result['status']
    ...
    return job_id, job_result['status'], job_result_return
