...
"""Provider for aer python backends."""
...
class AerPyProvider(BaseProvider):
    """Provider for aer pyhton backends."""

    def __init__(self, *args, **kwargs):
        super().__init__(args, kwargs)

        # Populate the list of aer python backends.
        self._backends = self._verify_aer_py_backends()
    ...
    def _verify_aer_py_backends(self):
        """
        Return the aer python backends in `AERPY_STANDARD_BACKENDS` that are
        ...
        Returns:
            dict[str:BaseBackend]: a dict of aer  python backend instances for
                the backends that could be instantiated, keyed by backend name.
        """
        ret = OrderedDict()
        for backend_cls in AERPY_STANDARD_BACKENDS:
            try:
                ...
                logger.info('aer python backend %s is not available: %s',
                            backend_cls, str(err))
        return ret
