...
@deprecate_arg(
    "opt_a2",
    since="2.5.0",
    deprecation_description="The argument ``opt_a2`` of ``qs_decomposition()``",
    additional_msg="The optimization is now applied automatically when appropriate.",
)
@deprecate_arg(
    "opt_a1",
    since="2.5.0",
    deprecation_description="The argument ``opt_a1`` of ``qs_decomposition()``",
    additional_msg="The optimization is now applied automatically when appropriate.",
)
def qs_decomposition(
    ...
    *,
    ...
    """
    for arg_name, arg_value, expected_type in [
        ("decomposer_1q", decomposer_1q, OneQubitEulerDecomposer),
        ("decomposer_2q", decomposer_2q, TwoQubitBasisDecomposer),
    ]:
        if callable(arg_value) and not isinstance(arg_value, expected_type):
            warnings.warn(
                f"Passing a callable that is not an instance of ``{expected_type.__name__}`` as "
                f"the ``{arg_name}`` argument of ``qs_decomposition()`` is deprecated as of "
                "Qiskit 2.5.0. Support for arbitrary callables will be removed in Qiskit 3.0.",
