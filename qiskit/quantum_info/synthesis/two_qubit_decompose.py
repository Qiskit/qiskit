...
class TwoQubitWeylDecomposition:
    ...
    def __init__(self, unitary_matrix):
        ...
        for i in range(-50, 50):
            M2real = (i / 50) * M2.real + (i / 25) * M2.imag
            _, P = la.eigh(M2real)
...
class TwoQubitBasisDecomposer():
    ...
    def traces(self, target):
        ...
        return [4 * (np.cos(target.a) * np.cos(target.b) * np.cos(target.c) +
                ...
                4 * (np.cos(np.pi / 4 - target.a) * np.cos(self.basis.b - target.b) * np.cos(
                    target.c) +
                     1j * np.sin(np.pi / 4 - target.a) * np.sin(self.basis.b - target.b) * np.sin(
                            target.c)),
                4 * np.cos(target.c),
                4]
