...
In addition, wew payload MAP_ITEM is defined to implement the :ref:`qpy_mapping` block.
...
Note that different programs cannot be packed together in the same file.
...
which is followed by ``data_size`` bytes of numpy serialized complex ``ndarray`` in C-order,
representing the complex IQ data points played on a quantum device.
...
Dictionary parameters are first dumped in the :ref:`qpy_mapping` form, and then ``duraction``
