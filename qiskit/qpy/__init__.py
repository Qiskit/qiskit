...
expression, while in version 13 QPY now represents the sequence of API calls used to create the
ParameterExpression.

The first change is the :ref:`qpy_param_expr_v3` payload for the ``expr_size`` bytes following
the header is now comprised of an array of ``PARAM_EXPR_ELEM_V13`` structs. The intent is for the
array of elements to be read one struct at a time that describe the calls to make to reconstruct
the :class:`.ParameterExpression`.
...
The ``op_code`` field is used to define the operation used for the operation
added to the :class:`.ParameterExpression`. The value can be:
...
the operand types and can be one of the following utf8 encoded
characters:
...
the inner ``expr`` in ``final_expr``. When ``s`` is encountered this indicates that until
an ``e` struct is reached the next structs are used for a recursive definition. For both
``s`` and ``e`` types the data values are 0 and not used. The type value
of ``u`` is used to represent a substitution call. This only is used for ``lhs_type``
