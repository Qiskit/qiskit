...
class Instruction(Operation):
    ...
    def base_class(self) -> type[Instruction]:
        ...
        The "base class" of an instruction is the lowest class in its inheritance hierarchy that remains
        fully compatible with the instruction’s behavior in all circuit contexts.

        In most cases, subclasses exist only to provide convenience or additional structure for the
        programmer, while the base class represents the true behavioral definition of the instruction.

        You should *not* override :attr:`base_class` when creating a custom instruction that behaves
        differently at the hardware level. For example, this includes alternative measurement strategies
        or specialized versions of parameterized gates used to distinguish implementations within a
        :class:`.Target`.
