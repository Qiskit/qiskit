...
of a pair of cy and cx gates is only stored in standard_gates_commutations["cx", "cy"] and additionally
as standard_gates_commutations["cx", "cy"]. The order of the gates is derived from the number of qubits
in each gate and the value of the integer representation of the gate name.
...
commutation. The relative placement is defined by the arrangement of the second gate's qubits with
respect to the qubits of the first gate. In the second example, the zeroth qubits of cy overlaps with
qubit 1 of cx. Furthermore, non-overlapping qubits are specified as 'None' in the dictionary key, e.g.
example 3, there is no overlap on the zeroth qubit of the first cx but the zeroth qubit of the second
gate overlaps with qubit 1 of the first cx.
"""
