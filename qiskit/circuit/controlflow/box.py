...
class BoxOp(ControlFlowOp):
    ...
    def __init__(
        ...
        *,
        ...
        self._duration, self._unit = Delay._validate_arguments(duration, unit)
        self._duration = duration
        self._unit = unit

    # The `duration` and `unit` properties are to override deprecation warnings in `Instruction`.
    @property
    def duration(self):
    ...
    @unit.setter
    def unit(self, value: typing.Literal["dt", "s", "ms", "us", "ns", "ps", "expr"]):
        self._unit = value
    ...
    def replace_blocks(self, blocks):
        (body,) = blocks
        return BoxOp(body, duration=self.duration, label=self.label)
