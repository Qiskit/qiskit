...
def control(
    ...
    if operation.name == "x" or (
        ...
        for gate, qreg, _ in unrolled_gate.definition.data:
            if gate.name == "x":
                controlled_circ.mct(q_control, q_target[qreg[0].index], q_ancillae)
            elif gate.name == "rx":
                ...
                controlled_circ.mct(
                    q_control[:] + [q_target[qreg[0].index]], q_target[qreg[1].index], q_ancillae
                )
            elif gate.name == "u":
                ...
                if num_ctrl_qubits == 1:
                    if theta == 0 and phi == 0:
                        ...
                        controlled_circ.cu(
                            theta, phi, lamb, 0, q_control[0], q_target[qreg[0].index]
                        )
                else:
                    if phi == -pi / 2 and lamb == pi / 2:
                        controlled_circ.mcrx(
                            theta, q_control, q_target[qreg[0].index], use_basis_gates=True
                        )
                        ...
                        controlled_circ.mcrz(
                            lamb, q_control, q_target[qreg[0].index], use_basis_gates=True
                        )
                    else:
                        controlled_circ.mcrz(
                            lamb, q_control, q_target[qreg[0].index], use_basis_gates=True
                        ...
                        controlled_circ.mcrz(
                            phi, q_control, q_target[qreg[0].index], use_basis_gates=True
                        )
            elif gate.name == "z":
                controlled_circ.h(q_target[qreg[0].index])
                controlled_circ.mcx(q_control, q_target[qreg[0].index], q_ancillae)
                controlled_circ.h(q_target[qreg[0].index])
            else:
                raise CircuitError(
    ...
    if hasattr(operation, "definition") and operation.definition:
        for rule in operation.definition.data:
            if rule[0].name in {"id", "barrier", "measure", "snapshot"}:
                raise CircuitError(
                    "Cannot make controlled gate with {} instruction".format(rule[0].name)
                )
