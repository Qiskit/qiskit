...
class QuantumCircuit:
    ...
    def rccx(self, control_qubit1, control_qubit2, target_qubit, label=None):
        ...
        return self.append(RCCXGate(label=label), [control_qubit1, control_qubit2, target_qubit], [])
    ...
    def fredkin(self, control_qubit, target_qubit1, target_qubit2, label=None, ctrl_state=None):
        """Apply :class:`~qiskit.circuit.library.CSwapGate`."""
        return self.cswap(control_qubit, target_qubit1, target_qubit2, label=label, ctrl_state=ctrl_state)
    ...
    def mcx(self, control_qubits, target_qubit, ancilla_qubits=None, mode='noancilla', 
        ...
        available_implementations = {
            'noancilla': MCXGrayCode(num_ctrl_qubits, label=label, ctrl_state=ctrl_state),
            'recursion': MCXRecursive(num_ctrl_qubits, label=label, ctrl_state=ctrl_state),
            'v-chain': MCXVChain(num_ctrl_qubits, False, label=label, ctrl_state=ctrl_state),
            'v-chain-dirty': MCXVChain(num_ctrl_qubits, dirty_ancillas=True, label=label, ctrl_state=ctrl_state),
            # outdated, previous names
            'advanced': MCXRecursive(num_ctrl_qubits, label=label, ctrl_state=ctrl_state),
            'basic': MCXVChain(num_ctrl_qubits, dirty_ancillas=False, label=label, ctrl_state=ctrl_state),
            'basic-dirty-ancilla': MCXVChain(num_ctrl_qubits, dirty_ancillas=True, label=label, ctrl_state=ctrl_state)
        }
    ...
    def mct(self, control_qubits, target_qubit, ancilla_qubits=None, mode='noancilla', 
                label=None, ctrl_state=None):
        """Apply :class:`~qiskit.circuit.library.MCXGate`."""
        return self.mcx(control_qubits, target_qubit, ancilla_qubits, mode, label=label, ctrl_state=ctrl_state)
