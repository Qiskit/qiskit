...
class QuantumCircuit:
    ...
    def reverse_ops(self):
        ...
        Examples:
            ...
            q_1: ┤ RX(1.57) ├─────
                 └──────────┘
        """
        reverse_circ = QuantumCircuit(*self.qregs, *self.cregs, name=self.name + "_reverse")
    ...
    def reverse_bits(self):
        ...
        Examples:
            ...
            q_1: ┤ H ├─────■──────
                 └───┘
        """
        circ = QuantumCircuit(*reversed(self.qregs), *reversed(self.cregs), name=self.name)
    ...
    def inverse(self):
        ...
        inverse_circ = QuantumCircuit(
            *self.qregs, *self.cregs, name=self.name + "_dg", global_phase=-self.global_phase
        )
    ...
    def repeat(self, reps):
        ...
        repeated_circ = QuantumCircuit(
            *self.qregs, *self.cregs, name=self.name + "**{}".format(reps)
        )
    ...
    def control(self, num_ctrl_qubits=1, label=None, ctrl_state=None):
        ...
        controlled_circ = QuantumCircuit(control_qreg, *self.qregs, name="c_{}".format(self.name))
    ...
    def _bit_argument_conversion(bit_representation, in_array):
        ...
        try:
            ...
            raise CircuitError(
                "Type error handling %s (%s)" % (bit_representation, type(bit_representation))
            )
        return ret
    ...
    def qasm(self, formatted=False, filename=None):
        ...
        for instruction, qargs, cargs in self._data:
            ...
            # If instruction is a root gate or a root instruction (in that case, compositive)
                ...
                string_temp += "%s %s;\n" % (
                    instruction.qasm(),
                    ",".join(["%s[%d]" % (j.register.name, j.index) for j in qargs + cargs]),
                )
            else:
                string_temp += "%s %s;\n" % (
                    instruction.qasm(),
    ...
    def assign_parameters(self, param_dict, inplace=False):
        ...
        params_not_in_circuit = [
            param_key for param_key in unrolled_param_dict if param_key not in self.parameters
        ]
    ...
    def mcx(self, control_qubits, target_qubit, ancilla_qubits=None, mode="noancilla"):
        ...
        try:
            ...
            raise ValueError(
                "Unsupported mode ({}) selected, choose one of {}".format(mode, all_modes)
            )
