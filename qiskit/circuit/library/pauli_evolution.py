    ...
    def __init__(self, input: list):
        ...
        if isinstance(input, list):
            if len(input) > 0:
                ...
                label = _get_default_label(operator)
                num_qubits = (
                    operator[0].num_qubits if isinstance(operator, list) else operator.num_qubits
                )
                time = input[0].params[0]
                super().__init__(
                    name="PauliEvolutionKernel", num_qubits=num_qubits, params=[time], label=label
                )
            ...
            super().__init__(
                name="PauliEvolutionKernel", num_qubits=0, params=[0.0], label="with error"
            )
