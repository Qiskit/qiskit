...
class NLocal(BlueprintCircuit):
        ...
        if initial_state is not None:
            try:
                ...
                if not isinstance(initial_state, InitialState):
                    raise TypeError(
                        "initial_state must be of type InitialState, but is "
    ...
    def assign_parameters(
        self,
        param_dict: Union[dict, List[float], List[Parameter], ParameterVector],
        inplace: bool = False,
        ...
        if not isinstance(param_dict, dict):
            ...
            unbound_unique_params = [
                param for param in unbound_params if param not in used and (used.add(param) or True)
            ]
