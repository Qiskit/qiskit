...
def _reordered_indices(num_cost: int, num_mixer: list):
    ...
    if num_cost == 0:
        ...
        beta_indices = list(range(len(np.array(num_mixer)[np.array(num_mixer) > 0])))
...
def adapt_mixer_pool(
    ...
    add_multi: bool = True,
    pool_type: str = None,
    circuit_rep: bool = False,
    circ_params: Optional[Union[np.ndarray, List]] = None,
    num_params: int = 1,
) -> None:
    """
    ...
    Args:
        ...
        pool_type: Optional input overrides add_single and add_multi by respecifying
            ...
            'singular' and 'single'.
        circuit_rep: Boolean value when set to True will return circuit representation of
            mixer pool. Defaults to False.
        circ_params: List of indices that specify qubits to be parameterised in the circuit
            representation.
        num_params: The number of parameters to be optimised per single qubit operation.
    Returns:
        List[Union[OperatorBase, QuantumCircuit]]: List of all possible combinations of mixers.
    Raises:
        KeyError: If an unrecognisible mixer type has been provided.
        ValueError: If the number of parameters is greater than the number of qubits.
    """
    ...
    if circuit_rep:
        ...
        if circ_params is not None:
            ...
            if check_params:
                raise ValueError(
                    "Qubit parameter indices {} out of range for {} specified "
...
class AdaptQAOAAnsatz(QAOAAnsatz):
    ...
    References:
        ...
        [2]: Zhu et al., An adaptive quantum approximate optimization algorithm for solving
            combinatorial problems on a quantum computer.
            `arXiv:2005.10258 <https://arxiv.org/pdf/2005.10258>'_
    """
    ...
    def __init__(
        ...
        Args:
            ...
            initial_state (QuantumCircuit, optional): An optional initial state to use.
                ...
                to all qubits.
            mixer_operators (OperatorBase or QuantumCircuit, List[QuantumCircuit], optional):
                An optional operator, or an ordered list of operators, with a length equal to the
                circuit depth, that define the mixer operators at each layer of the ansatz.
            ...
            mixer_pool_type (str): An optional string representing the varied mixer pool types, with
                `single` creating the same mixer pool as the standard QAOA. `singular` creates a mixer
                pool including mixers in `single` as well as additional single qubit mixers. `multi`
                creates a mixer pool including mixers from `single`, `singular` as well as multi-qubit
                entangling mixers.
    ...
    def _check_configuration(self, raise_on_failure: bool = True) -> bool:
        valid = True
        if self._mixer_operators is not None and (not isinstance(self._mixer_operators, list)):
            self.mixer_operator = self._mixer_operators
            return super()._check_configuration()

        if not EvolvedOperatorAnsatz._check_configuration(self, raise_on_failure):
            return False
        ...
        if not self._config_check:
            ...
            self._mixer_op_pool, valid = self._check_mixers(
                self.mixer_pool
            )  # sets self._mixer_pool as operator representation
            if self._mixer_operators is not None:
                # if custom mixers have been specified, check that they aren't in the mixer_pool
                if np.any([mixer not in self._mixer_pool for mixer in self.mixer_operators]):
                    _, valid = self._check_mixers(
                        self.mixer_operators
                    )  # if not, then check the custom mixers
            self._config_check = True  # set class attribute to avoid doing this check twice
        return valid

    def _check_mixers(
        self, mixer_operators: Optional[Union[List, QuantumCircuit]], raise_on_failure: bool = True
    ):

        from qiskit.opflow import PrimitiveOp

        mixer_operators = (
            mixer_operators if isinstance(mixer_operators, list) else [mixer_operators]
        )
        valid = True
        ...
        for mixer in mixer_operators:
            ...
            ):  # For the purposes of efficient energy gradient computation
                nmix_params.append(mixer.num_parameters)
                if not nmix_params[-1]:  # if number parameters is 0,
                    mixer = PrimitiveOp(  # convert circuit to operator
                        Operator(mixer)
                    )  # (i.e. the commutator) we must convert mixer circuits
            else:
                nmix_params.append(0 if _is_pauli_identity(mixer) else 1)
            mixer_list.append(mixer)  # to operators here
        ...
        if bool(sum(_config_check)):
            ...
            if raise_on_failure:
                neq_num_qubits = [nmix_qubits[i] for i in _config_check]
                raise AttributeError(
                    "Operators at index location(s) {} in the specified mixer pool "
                    "have an unequal number of qubits {} to the cost operator {}.".format(
                        err_str, neq_num_qubits, self.num_qubits
                    )
                )
    ...
    def parameter_bounds(self) -> Optional[List[Tuple[Optional[float], Optional[float]]]]:
        ...
        Returns:
            ...
            parameter bounds specified in the supplementary material of
    ...
    def mixer_operators(self):
        ...
        the ansatz.
        Returns:
            List[Union[OperatorBase, QuantumCircuit]]: An ordered list of mixers to be used on
            ansatz layers.
        Raises:
        ...
        if isinstance(self._mixer_operators, list):
    ...
    def mixer_operators(self, mixer_operators) -> None:
        ...
        Args:
            mixer_operators (List[Union[QuantumCircuit, OperatorBase]], optional): a list
                 of operators or circuits to set.
        """
    ...
    def mixer_pool(self, mixer_pool) -> None:
        ...
        Args:
            mixer_pool (List[Union[QuantumCircuit, OperatorBase]], optional): a list
                of operators or circuits that define the mixer pool.
        """
    ...
    def _build(self):
        if self._is_built:
            return
        EvolvedOperatorAnsatz._build(self)
        ...
        gamma_index, beta_index = _reordered_indices(
            num_mixer=self._num_mixer, num_cost=self._num_cost
        )
        reordered = np.array([None] * (sum(self._num_mixer) + self._num_cost))
        reordered[gamma_index] = gammas
        reordered[beta_index] = betas
