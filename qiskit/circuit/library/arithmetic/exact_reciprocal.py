...
class ExactReciprocal(QuantumCircuit):
    ...
    def __init__(
        ...
        Args:
            ...
            scaling: Scaling factor of the reciprocal function, i.e. to compute :math:`scaling / x`.
            neg_vals: Whether x might represent negative values. In this case the first qubit is
                the sign, with :math:`|1\rangle` for negative and :math:`|0\rangle` for positive.
                For the negative case it is assumed that the remaining string represents
                :math:`1 - x`. This is because :math:`e^{-2 \pi i x} = e^{2 \pi i (1 - x)}`
                for :math:`x \in [0,1)`.
