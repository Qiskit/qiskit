...
def mcmt(
    ...
    *,
    ...
    """
    warnings.warn(
        "The multi-control multi-target gate has moved to the circuit library as of "
    ...
    deprecated_modes = {"no-ancilla": "noancilla", "basic": "v-chain"}

    if mode in deprecated_modes.keys():
        warnings.warn(
            "The mode supplied mode for ``QuantumCircuit.mcmt`` is deprecated, use "
    ...
    if mode == "v-chain":
        ...
        return self.append(
            mcmt_gate, control_qubits[:] + target_qubits[:] + ancilla_qubits[:num_ancillas], []
        )
