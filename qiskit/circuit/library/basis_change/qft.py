...
class QFTGate(Gate):
    ...
    def inverse(self, annotated: bool = False):
        """Return the inverse Quantum Fourier Transform gate.

        Args:
            annotated: if set to ``True``, the inverse is returned as an
                :class:`.AnnotatedOperation` instead of an explicit gate.
        """
        if annotated:
            return super().inverse(annotated=True)
        return _InverseQFTGate(self.num_qubits)


class _InverseQFTGate(Gate):
    r"""The inverse of :class:`.QFTGate`.

    Unlike the generic gate returned by :meth:`.Gate.inverse`, this gate defines its own
    unitary matrix, which the transpiler relies on to collect and re-synthesize runs of
    single-qubit gates.
    """
    ...
    def __init__(
        ...
        """
        Args:
            num_qubits: The number of qubits on which the inverse QFT acts.
        """
        super().__init__(name="qft_dg", num_qubits=num_qubits, params=[])

    def __array__(self, dtype=complex, copy=None):
        """Return a numpy array for the inverse QFTGate."""
        if copy is False:
            raise ValueError("unable to avoid copy while creating an array as requested")
        n = self.num_qubits
        nums = np.arange(2**n)
    ...
    def _define(self):
        """Provide a specific decomposition of the gate into a quantum circuit."""
        from qiskit.synthesis.qft import synth_qft_full

        self.definition = synth_qft_full(num_qubits=self.num_qubits, inverse=True)

    def inverse(self, annotated: bool = False):
        """Return the Quantum Fourier Transform gate.
        ...
        if annotated:
            return super().inverse(annotated=True)
        return QFTGate(self.num_qubits)
