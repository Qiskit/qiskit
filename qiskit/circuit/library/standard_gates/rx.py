...
class RXGate(Gate):
    ...
    def _define(self):
        ...
        self.definition = QuantumCircuit._from_circuit_data(
            StandardGate.RX._get_definition(self.params), legacy_qubits=True
        )
...
class CRXGate(ControlledGate):
    ...
    def _define(self):
        ...
        self.definition = QuantumCircuit._from_circuit_data(
            StandardGate.CRX._get_definition(self.params), legacy_qubits=True
        )
...
class ACRXGate(Gate):
    r"""Anti-controlled RX gate.

    Applies an RX rotation on the target qubit if the control is
    in the :math:`|0\rangle` state.

    Can be applied to a :class:`~qiskit.circuit.QuantumCircuit`
    with the :meth:`~qiskit.circuit.QuantumCircuit.acrx` method.
    ...
    This is equivalent to a controlled-RX gate with the control state
    ...
    .. math::
        ...
        ACRX(\theta)\ q_0, q_1 =
            RX(\theta) \otimes |0\rangle\langle 0| + I \otimes |1\rangle\langle 1| =
            \begin{pmatrix}
                \cos\left(\rotationangle\right) & 0
                    & -i\sin\left(\rotationangle\right) & 0 \\
                0 & 1 & 0 & 0 \\
                -i\sin\left(\rotationangle\right) & 0
                    & \cos\left(\rotationangle\right) & 0 \\
                0 & 0 & 0 & 1
    ...
    .. note::
        ...
        .. math::
            ...
            ACRX(\theta)\ q_1, q_0 =
                |0\rangle\langle 0| \otimes RX(\theta) +
                |1\rangle\langle 1| \otimes I =
                \begin{pmatrix}
                    \cos\left(\rotationangle\right) &
                        -i\sin\left(\rotationangle\right) & 0 & 0 \\
                    -i\sin\left(\rotationangle\right) &
    ...
    def __init__(self, theta: ParameterValueType, label: str | None = None):
        """Create new ACRX gate.

        Args:
            theta: The rotation angle of the gate.
            label: An optional label for the gate.
        """
        super().__init__("acrx", 2, [theta], label=label)

    def _define(self):
        """Decomposition: X on control, CRX, X on control."""
        from qiskit.circuit import QuantumCircuit

        q = QuantumCircuit(2, name=self.name)
        q.x(0)
        q.crx(self.params[0], 0, 1)
        q.x(0)
        self.definition = q

    def inverse(self, annotated: bool = False):
        r"""Return inverse ACRX gate (i.e. with the negative rotation angle).

        Args:
            annotated: when set to ``True``, this is typically used to return an
                :class:`.AnnotatedOperation` with an inverse modifier set instead of a concrete
                :class:`.Gate`. However, for this class this argument is ignored as the inverse
                of this gate is always a :class:`.ACRXGate` with an inverted parameter value.

        Returns:
            ACRXGate: inverse gate.
        """
        return ACRXGate(-self.params[0])

    def __array__(self, dtype=None, copy=None):
        """Return a numpy.array for the ACRX gate."""
        if copy is False:
            raise ValueError("unable to avoid copy while creating an array as requested")
        half_theta = float(self.params[0]) / 2
        cos = math.cos(half_theta)
