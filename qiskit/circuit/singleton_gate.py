...
SINGLETOCONTROLED_NGATE_ATTR_SET = frozenset(
    (
        "definition",
        "unit",
        "duration",
...
class SingletonControlledGate(ControlledGate):
    ...
    def __new__(cls, *args, **kwargs):
        if args or (  # pylint: disable=too-many-boolean-expressions
            kwargs
            and (
                kwargs.get("label", None) is not None
                or kwargs.get("_base_label", None) is not None
                or kwargs.get("_condition", None) is not None
                or kwargs.get("duration", None) is not None
                or kwargs.get("unit", None) is not None
                or kwargs.get("ctrl_state", None) is not None
            )
        ):
            return super().__new__(cls)
        if cls._instance is None:
    ...
    def to_mutable(self):
        if not self.mutable:
            instance = super().__new__(type(self))
            instance.__init__(ctrl_state=self._ctrl_state)
            return instance
        else:
            return copy.deepcopy(self)
    ...
    def __setattr__(self, name, value):
        if self.mutable:
            ...
            if name not in SINGLETOCONTROLED_NGATE_ATTR_SET:
                raise NotImplementedError(
                    "Setting custom attributes is not allowed on a singleton gate"
                )
            super().__setattr__(name, value)
    ...
    def num_ctrl_qubits(self, num_ctrl_qubits):
        if not self.mutable:
            raise NotImplementedError(
                "Setting a custom number of control qubits is not allowed on a singleton gate, "
                "use ``.to_mutable()`` to make a mutable copy first"
            )
