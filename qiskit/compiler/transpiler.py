        ...
        if getattr(backend, "configuration", None):
            ...
            if hasattr(configuration, "coupling_map") and configuration.coupling_map:
                ...
                if faulty_map:
                    ...
                    functional_gates = [edge for edge in configuration.coupling_map if
                    ...
                    if configuration.n_qubits != coupling_map.size():
                        warnings.warn("The backend has currently some qubits/edges out of service."
                                      " This temporarily reduces the backend size from "
                                      f"{configuration.n_qubits} to {coupling_map.size()}", UserWarning)
                else:
