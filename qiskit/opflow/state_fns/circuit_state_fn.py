...
class CircuitStateFn(StateFn):
    ...
    def tensor(self, other: OperatorBase) -> OperatorBase:
        ...
        if isinstance(other, CircuitStateFn) and other.is_measurement == self.is_measurement:
            ...
            if isinstance(c_op, CircuitOp):
                return CircuitStateFn(
                    primitive=c_op.primitive, coeff=c_op.coeff, is_measurement=self.is_measurement
                )
    ...
    def to_matrix(self, massive: bool = False) -> np.ndarray:
        ...
        statevector = (
            execute(qc, statevector_backend, optimization_level=0).result().get_statevector()
        )
        # pylint: disable=cyclic-import
    ...
    def eval(
        self, front: Optional[Union[str, Dict[str, complex], np.ndarray, OperatorBase]] = None
