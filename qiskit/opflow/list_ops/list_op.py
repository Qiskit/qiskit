...
class ListOp(OperatorBase):
    ...
    def adjoint(self) -> OperatorBase:
        ...
        if self.__class__ == ListOp:
            return ListOp(
                ...
                **self._state(coeff=self.coeff.conjugate()),
            )  # coeff is conjugated
    ...
    def traverse(
        ...
        if self.__class__ == ListOp:
            return ListOp(
                [convert_fn(op) for op in self.oplist], **self._state(coeff=coeff)  # type: ignore
            )
        return self.__class__(
            [convert_fn(op) for op in self.oplist],  # type: ignore
            coeff=coeff,
            abelian=self.abelian,
        )
    ...
    def to_spmatrix(self) -> Union[spmatrix, List[spmatrix]]:
        ...
        # Combination function must be able to handle classical values
        return self.combo_fn([op.to_spmatrix() for op in self.oplist]) * self.coeff  # type: ignore
    ...
    def eval(
        ...
        if self._combo_fn != ListOp([])._combo_fn:
            if all(isinstance(op, DictStateFn) for op in evals) or all(
                isinstance(op, VectorStateFn) for op in evals
            ):
                if not all(isinstance(op, type(evals[0])) for op in evals):
                    raise NotImplementedError(
                        "Combo_fn not yet supported for mixed " "VectorStateFn primitives"
                    )
    ...
    def to_circuit_op(self) -> OperatorBase:
        ...
        if self.__class__ == ListOp:
            return ListOp(
                [
                    op.to_circuit_op()  # type: ignore
                    if not isinstance(op, OperatorStateFn)
                    else op
                    for op in self.oplist
                ],
                **self._state(),
        ...
        return self.__class__(
            [
                op.to_circuit_op() if not isinstance(op, OperatorStateFn) else op  # type: ignore
                for op in self.oplist
            ],
            coeff=self.coeff,
            abelian=self.abelian,
