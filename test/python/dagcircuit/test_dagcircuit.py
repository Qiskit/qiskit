...
def raise_if_dagcircuit_invalid(dag):
    ...
    edges_outside_wires = [
        edge_data["wire"]
        for edge_data in dag._multi_graph.edges()
    ...
    for wire in dag.wires:
        ...
        while cur_node_id != out_node_id:
            ...
            edges_to_follow = [
                (src, dest, data) for (src, dest, data) in out_edges if data["wire"] == wire
            ]
    ...
    def test_apply_operation_back_conditional(self):
        ...
        self.assertEqual(
            ...
            sorted(
                [
                    (
                        self.dag.input_map[self.qubit2]._node_id,
                        h_node._node_id,
                        {"wire": self.qubit2, "name": "qr[2]"},
                    ),
                    (
                        self.dag.input_map[self.clbit0]._node_id,
                        h_node._node_id,
                        {"wire": self.clbit0, "name": "cr[0]"},
                    ),
                    (
                        self.dag.input_map[self.clbit1]._node_id,
                        h_node._node_id,
        ...
        self.assertEqual(
            ...
            sorted(
                [
                    (
                        h_node._node_id,
                        self.dag.output_map[self.qubit2]._node_id,
                        {"wire": self.qubit2, "name": "qr[2]"},
                    ),
                    (
                        h_node._node_id,
                        self.dag.output_map[self.clbit0]._node_id,
                        {"wire": self.clbit0, "name": "cr[0]"},
                    ),
                    (
                        h_node._node_id,
                        self.dag.output_map[self.clbit1]._node_id,
                        {"wire": self.clbit1, "name": "cr[1]"},
                    ),
                ]
            ),
    ...
    def test_apply_operation_back_conditional_measure(self):
        ...
        self.assertEqual(
            ...
            sorted(
                [
                    (
                        self.dag.input_map[self.qubit0]._node_id,
                        meas_node._node_id,
        ...
        self.assertEqual(
            ...
            sorted(
                [
                    (
                        meas_node._node_id,
                        self.dag.output_map[self.qubit0]._node_id,
                        {"wire": self.qubit0, "name": "qr[0]"},
                    ),
                    (
                        meas_node._node_id,
    ...
    def test_apply_operation_back_conditional_measure_to_self(self):
        ...
        self.assertEqual(
            ...
            sorted(
                [
                    (
                        self.dag.input_map[self.qubit1]._node_id,
                        meas_node._node_id,
        ...
        self.assertEqual(
            ...
            sorted(
                [
                    (
                        meas_node._node_id,
                        self.dag.output_map[self.qubit1]._node_id,
                        {"wire": self.qubit1, "name": "qr[1]"},
                    ),
                    (
                        meas_node._node_id,
