...
class TestScheduleBuilding(BaseTestSchedule):
    ...
    def test_out_of_order_parameters(self):
        ...
        def test_fun(y, x):
            sched = Schedule()
            sched += FrameChange(x)(DriveChannel(0))
            sched += FrameChange(y)(DriveChannel(0))
            return sched
        ...
        par_sched = ParameterizedSchedule(test_fun, parameters=['y', 'x'])

        self.assertEqual(ref_sched, par_sched.bind_parameters(0, 1))

    def test_ibmq_provider_u_gate_parameter_ordering(self):
        """Test if parameter ordering of circuit instruction map of u* gates
        generated by ibmq provider's cmd_def aligned with alphabetical parameter ordering
        ...
        self.assertEqual(u2_pnames, fake_default.circuit_instruction_map.get_parameters('u2', 0))
        self.assertEqual(u3_pnames, fake_default.circuit_instruction_map.get_parameters('u3', 0))
