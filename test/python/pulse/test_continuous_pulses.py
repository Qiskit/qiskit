...
class TestContinuousPulses(QiskitTestCase):
    ...
    def test_sech(self):
        ...
        sech_arr_zeroed = continuous.sech(np.array([-1, 10]), amp, center,
                                                  sigma)
        ...
        self.assertAlmostEqual(sech_arr_zeroed[0], 0., places=3)
        self.assertAlmostEqual(sech_arr_zeroed[1], amp)
        self.assertAlmostEqual(np.sum(sech_arr*dt), amp*np.sqrt(2*np.pi*sigma**2), places=3)
    ...
    def test_sech_deriv(self):
        ...
        sech_arr = sech_deriv_arr*deriv_prefactor
