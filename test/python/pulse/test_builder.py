...
class TestContexts(TestBuilder):
    ...
    def test_transpiler_settings(self):
        ...
        schedule = pulse.Schedule()
        with pulse.build(schedule, self.backend):
...
class TestUtilities(TestBuilder):
    ...
    def test_num_qubits(self):
        ...
        with pulse.build(pulse.Schedule(), self.backend):
...
class TestBuilderComposition(TestBuilder):
    ...
    def test_default_alignment_left(self):
        ...
        schedule = pulse.Schedule()
        with pulse.build(schedule, default_alignment='left'):
            pulse.delay(d0, 10)
            pulse.delay(d1, 20)

        reference = pulse.Schedule()
        with pulse.build(reference, self.backend):
            with pulse.align_left():
                pulse.delay(d0, 10)
                pulse.delay(d1, 20)
    ...
    def test_default_alignment_right(self):
        ...
        schedule = pulse.Schedule()
        with pulse.build(schedule, default_alignment='right'):
            pulse.delay(d0, 10)
            pulse.delay(d1, 20)

        reference = pulse.Schedule()
        with pulse.build(reference):
            with pulse.align_right():
                pulse.delay(d0, 10)
                pulse.delay(d1, 20)
    ...
    def test_default_alignment_sequential(self):
        ...
        schedule = pulse.Schedule()
        with pulse.build(schedule, default_alignment='sequential'):
            pulse.delay(d0, 10)
            pulse.delay(d1, 20)
