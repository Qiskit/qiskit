...
class TestUseCases(SchedulerTestCase):
    ...
    def test_chain(self):
        ...
        self.passmanager.add(PassC_TP_RA_PA())  # Request: PassA / Preserves: PassA
        self.passmanager.add(PassB_TP_RA_PA())  # Request: PassA / Preserves: PassA
        self.passmanager.add(PassD_TP_NR_NP(argument1=[1, 2]))  # Requires: {}/ Preserves: {}
        self.passmanager.add(PassB_TP_RA_PA())
    ...
    def test_conditional_passes_true(self):
        ...
        self.passmanager.add(PassE_AP_NR_NP(True))
        self.passmanager.add(PassA_TP_NR_NP(),
    ...
    def test_conditional_passes_false(self):
        ...
        self.passmanager.add(PassE_AP_NR_NP(False))
        self.passmanager.add(PassA_TP_NR_NP(),
                             condition=lambda property_set: property_set['property'])
        self.assertScheduler(self.dag, self.passmanager, ['run analysis pass PassE_AP_NR_NP',
    ...
    def test_loop_and_conditional(self):
        ...
        self.passmanager.add(PassK_check_fixed_point_property())
        self.passmanager.add(
            [PassK_check_fixed_point_property(),
             PassA_TP_NR_NP(),
             PassF_reduce_dag_property()],
    ...
    def test_do_not_repeat_based_on_preservation(self):
        ...
        run again"""
        self.passmanager.add([PassB_TP_RA_PA(), PassA_TP_NR_NP(), PassB_TP_RA_PA()])
        self.assertScheduler(self.dag, self.passmanager, ['run transformation pass PassA_TP_NR_NP',
                                                          'run transformation pass PassB_TP_RA_PA'])
    ...
    def test_do_not_repeat_based_on_idempotence(self):
        ...
        self.passmanager.add(PassA_TP_NR_NP())
        self.passmanager.add([PassA_TP_NR_NP(), PassA_TP_NR_NP()])
        self.passmanager.add(PassA_TP_NR_NP())
        self.assertScheduler(self.dag, self.passmanager, ['run transformation pass PassA_TP_NR_NP'])

    def test_non_idempotent_pass(self):
        """ Two or more runs of a non-idempotent pass cannot be optimized. """
        self.passmanager.add(PassF_reduce_dag_property())
        self.passmanager.add([PassF_reduce_dag_property(), PassF_reduce_dag_property()])
        self.passmanager.add(PassF_reduce_dag_property())
        self.assertScheduler(self.dag, self.passmanager,
                             ['run transformation pass PassF_reduce_dag_property',
                              'dag property = 6',
    ...
    def test_fenced_property_set(self):
        ...
        self.passmanager.add(PassH_Bad_TP())
    ...
    def test_fenced_dag(self):
        ...
        self.passmanager.add(PassI_Bad_AP())
    ...
    def test_ignore_request_pm(self):
        ...
        passmanager.add(PassC_TP_RA_PA())  # Request: PassA / Preserves: PassA
        passmanager.add(PassB_TP_RA_PA())  # Request: PassA / Preserves: PassA
        passmanager.add(PassD_TP_NR_NP(argument1=[1, 2]))  # Requires: {} / Preserves: {}
        passmanager.add(PassB_TP_RA_PA())
    ...
    def test_pass_non_idempotence_pm(self):
        ...
        passmanager = PassManager(ignore_preserves=True)
        passmanager.add(PassA_TP_NR_NP())
        passmanager.add(PassA_TP_NR_NP())  # Normally removed for optimization, not here.
        passmanager.add(PassB_TP_RA_PA())  # Normally required is ignored for optimization,
        # not here
        self.assertScheduler(self.dag, passmanager, ['run transformation pass PassA_TP_NR_NP',
                                                     'run transformation pass PassA_TP_NR_NP',
                                                     'run transformation pass PassA_TP_NR_NP',
    ...
    def test_pass_non_idempotence_passset(self):
        ...
        passmanager.add([PassA_TP_NR_NP(), PassB_TP_RA_PA()], ignore_preserves=True)
        self.assertScheduler(self.dag, passmanager, ['run transformation pass PassA_TP_NR_NP',
                                                     'run transformation pass PassA_TP_NR_NP',
                                                     'run transformation pass PassB_TP_RA_PA'])
    ...
    def test_analysis_pass_is_idempotent(self):
        ...
        passmanager = PassManager()
        passmanager.add(PassE_AP_NR_NP(argument1=1))
        passmanager.add(PassE_AP_NR_NP(argument1=1))
    ...
    def test_pass_option_precedence(self):
        ...
        passmanager.add(tp_pass, ignore_preserves=True)
    ...
    def test_pass_no_return_a_dag(self):
        ...
        self.passmanager.add(PassJ_Bad_NoReturn())
        self.assertSchedulerRaises(self.dag, self.passmanager,
...
class TestControlFlowPlugin(SchedulerTestCase):
    ...
    def test_control_flow_plugin(self):
        ...
        self.passmanager.add([PassB_TP_RA_PA(), PassC_TP_RA_PA()], do_x_times=lambda x: 3)
        self.assertScheduler(self.dag, self.passmanager, ['run transformation pass PassA_TP_NR_NP',
                                                          'run transformation pass PassB_TP_RA_PA',
                                                          'run transformation pass PassC_TP_RA_PA',
                                                          'run transformation pass PassB_TP_RA_PA',
    ...
    def test_callable_control_flow_plugin(self):
        ...
        self.passmanager.add([PassB_TP_RA_PA(), PassC_TP_RA_PA()],
