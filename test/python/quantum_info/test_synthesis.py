...
class CheckDecompositions(QiskitTestCase):
    ...
    def check_one_qubit_euler_angles(self, operator, basis="U3", tolerance=1e-12, phase_equal=True):
    ...
    def check_two_qubit_weyl_decomposition(self, target_unitary, tolerance=1.0e-7):
    ...
    def check_exact_decomposition(self, target_unitary, decomposer, tolerance=1.0e-7):
...
class TestOneQubitEulerDecomposer(CheckDecompositions):
    ...
    @combine(
        basis=["U3", "U1X", "PSX", "ZSX", "ZYZ", "ZXZ", "XYX", "RR"],
    ...
    def test_one_qubit_hard_thetas_all_basis(self, basis_tolerance):
        ...
        for gate in HARD_THETA_ONEQS:
            self.check_one_qubit_euler_angles(
                Operator(gate), basis_tolerance[0], basis_tolerance[1]
            )
