...
class TestPhaseOracleAndGate(QiskitTestCase):
    ...
    @unpack
    def test_phase_oracle_evaluate_bitstring_deprecated(
        self, expression, input_bitstring, expected
    ):
        """Deprecated PhaseOracle(...).evaluate_bitstring"""
        with self.assertWarns(DeprecationWarning):
            oracle = PhaseOracle(expression)
        result = oracle.evaluate_bitstring(input_bitstring)
        self.assertEqual(result, expected)

    # Test data for statevector tests
    statevector_test_data = [
        ("x | x", "01"),
        ("~x", "10"),
        ("x & y", "0001"),
        ("x & ~y", "0100"),
    ...
    @data(*statevector_test_data)
    @unpack
    def test_phase_oracle_gate_statevector(self, expression, truth_table):
        """Circuit generation with PhaseOracleGate"""
    ...
    @data(*statevector_test_data)
    @unpack
    def test_phase_oracle_statevector_deprecated(self, expression, truth_table):
        """Circuit generation with deprecated PhaseOracle"""
        with self.assertWarns(DeprecationWarning):
            oracle = PhaseOracle(expression)
        num_qubits = oracle.num_qubits
        circuit = QuantumCircuit(num_qubits)
    ...
    # Test data for variable order tests
    variable_order_test_data = [
        ("((A & C) | (B & D)) & ~(C & D)", None, [3, 7, 12, 13]),
        ("((A & C) | (B & D)) & ~(C & D)", ["A", "B", "C", "D"], [5, 7, 10, 11]),
    ]

    @data(*variable_order_test_data)
    @unpack
    def test_phase_oracle_gate_variable_order(self, expression, var_order, good_states):
        """Circuit generation with PhaseOracleGate and variable order"""
    ...
    @data(*variable_order_test_data)
    @unpack
    def test_phase_oracle_variable_order_deprecated(self, expression, var_order, good_states):
        """Circuit generation with deprecated PhaseOracle and variable order"""
        with self.assertWarns(DeprecationWarning):
