...
class TestParameterExpression(QiskitTestCase):
    ...
    def test_simplify_cancellation(self):
        """Test that simplify() collapses cancelling terms to zero."""
        a = Parameter("a")
        expr = a - a
        simplified = expr.simplify()
        self.assertEqual(complex(simplified.numeric(strict=False)), 0j)
    ...
    def test_simplify_no_cancellation(self):
        ...
        expr = a + b
        simplified = expr.simplify()
        bound = simplified.bind({a: 1.0, b: 2.0})
        self.assertAlmostEqual(complex(bound), 3.0)
