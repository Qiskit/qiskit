...
class TestParameters(QiskitTestCase):
    ...
    def test_raise_if_assigning_params_not_in_circuit(self):
        ...
        qc = QuantumCircuit(qr)
        qc.p(0.1, qr[0])
        self.assertRaises(CircuitError, qc.assign_parameters({x: 1}))
        qc.p(x, qr[0])
        self.assertRaises(CircuitError, qc.assign_parameters({x: 1, y: 2}))
        qc.p(z[1], qr[0])
        self.assertRaises(CircuitError, qc.assign_parameters({z: [3, 4, 5]}))
        self.assertRaises(CircuitError, qc.assign_parameters({"a_str": 6}))
        self.assertRaises(CircuitError, qc.assign_parameters({None: 7}))
