...
class TestIsometry(QiskitTestCase):
    ...
    @data(
        ...
        np.eye(4, 4)[:, np.random.RandomState(seed=seed).permutation(np.eye(4, 4).shape[1])][:, 0:2],
        np.eye(8, 8)[:, np.random.RandomState(seed=seed).permutation(np.eye(8, 8).shape[1])],
