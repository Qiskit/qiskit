...
class TestBackendConfiguration(QiskitTestCase):
    ...
    def test_get_rep_times(self):
        ...
        for i, _r in enumerate(_rep_times_s):
            self.assertAlmostEqual(self.config.rep_times[i], _r)
        for i, _r in enumerate(_rep_times_us):
            self.assertEqual(round(self.config.rep_times[i]*1e6), _r)
