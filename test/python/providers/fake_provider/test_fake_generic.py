...
class TestFakeGeneric(QiskitTestCase):
    ...
    def test_heavy_hex_coupling_map(self):
        ...
        cp_mp=[(0, 13), (1, 13), (1, 14), (2, 14), (3, 15), (4, 15), (4, 16), (5, 16), (6, 17), (7, 17), (7, 18), (8, 18), (0, 9), (3, 9), (5, 12), (8, 12), (10, 14), (10, 16), (11, 15), (11, 17)]
        self.assertEqual(list(FakeGeneric(num_qubits=19, coupling_map_type='heavy_hex').coupling_map.get_edges()), cp_mp)
    ...
    def test_grid_coupling_map(self):
        ...
        self.assertEqual(list(FakeGeneric(num_qubits=8, coupling_map_type='grid').coupling_map.get_edges()), cp_mp)

    def test_basis_gates(self):
        """ Test if the backend has a default basis gates, that includes dealy and measure"""
        self.assertEqual(FakeGeneric(num_qubits=8).operation_names, ['ecr', 'id', 'rz', 'sx', 'x', 'delay', 'measure', 'reset'])

    def test_if_cx_replaced_with_ecr(self):
        """ Test if cx is not reaplced with ecr"""
        self.assertEqual(FakeGeneric(num_qubits=8, replace_cx_with_ecr=False).operation_names, ['cx', 'id', 'rz', 'sx', 'x', 'delay', 'measure', 'reset'])

    def test_dynamic_true_basis_gates(self):
        """ Test if basis_gates includes ControlFlowOps when dynamic is set to True"""
        self.assertEqual(FakeGeneric(num_qubits=9, dynamic=True).operation_names,['ecr', 'id', 'rz', 'sx', 'x', 'delay', 'measure', 'if_else', 'while_loop', 'for_loop', 'switch_case', 'break', 'continue', 'reset'])

    def test_if_excludes_reset(self):
        """ Test if reset is excluded from the operation names when set enable_reset False"""
        self.assertEqual(FakeGeneric(num_qubits=9, enable_reset=False).operation_names, ['ecr', 'id', 'rz', 'sx', 'x', 'delay', 'measure'])
