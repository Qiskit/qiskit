...
class TestTextDrawerGatesInCircuit(QiskitTestCase):
    ...
    def test_wire_order(self):
        ...
        circuit.h(3)
        circuit.x(1)
        circuit.x(3).c_if(cr[1], 10)
        print("\n")
        print(str(_text_circuit_drawer(circuit, wire_order=[2, 1, 3, 0, 6, 8, 9, 5, 4, 7])))
        print(expected)
        self.assertEqual(
    ...
    def test_switch_case(self):
        ...
        with circuit.switch(creg) as case:
            ...
            with case(case.DEFAULT):
                circuit.cx(0, 1)
        circuit.h(0)
        print("\n", str(_text_circuit_drawer(circuit, fold=78, initial_state=False)))
        print(expected)
