...
class TestVQE(QiskitAlgorithmsTestCase):
    ...
    def test_circuit_input(self):
        ...
        vqe = VQE(
            var_form=wavefunction, optimizer=optimizer, quantum_instance=self.statevector_simulator
        )
        result = vqe.compute_minimum_eigenvalue(operator=self.h2_op)
    ...
    def test_missing_varform_params(self):
        ...
        vqe = VQE(var_form=circuit, quantum_instance=BasicAer.get_backend("statevector_simulator"))
    ...
    def test_max_evals_grouped(self, optimizer, places, max_evals_grouped):
        """ VQE Optimizers test """
        vqe = VQE(
            var_form=self.ryrz_wavefunction,
            optimizer=optimizer,
