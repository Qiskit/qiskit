...
class TestFidelity(QiskitTestCase):
    ...
    def test_1param_pair(self):
        ...
        job = fidelity.run(left_parameter_values = self._left_params[0],
                           right_parameter_values = self._right_params[0])
        result = job.result
    ...
    def test_4param_pairs(self):
        ...
        fidelity = Fidelity(self._sampler, self._circuit[0], self._circuit[1])
        job = fidelity.run(left_parameter_values = self._left_params,
                           right_parameter_values = self._right_params)
        results = job.result
    ...
    def test_symmetry(self):
        ...
        job_1 = fidelity.run(left_parameter_values = self._left_params,
                             right_parameter_values = self._right_params)
        job_2 = fidelity.run(left_parameter_values = self._right_params,
                             right_parameter_values = self._left_params)
    ...
    def test_not_set_circuits(self):
        ...
        with self.assertRaises(ValueError):
            _ = fidelity.run(left_parameter_values = self._left_params,
                             right_parameter_values = self._right_params)
    ...
    def test_set_circuits_during_run(self):
        ...
        job = fidelity.run(left_circuit = [self._circuit[0]],
                           ...
                           left_parameter_values = self._left_params,
                           right_parameter_values = self._right_params)
        results = job.result
        np.testing.assert_allclose(results, np.array([1.0, 0.5, 0.25, 0.0]), atol=1e-16)
