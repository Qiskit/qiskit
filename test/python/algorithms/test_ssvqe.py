...
from qiskit.algorithms.eigen_solvers import SSVQE, SSVQEResult
...
class TestSSVQE(QiskitAlgorithmsTestCase):
    ...
    def test_basic_aer_statevector(self):
        """Test SSVQE on BasicAer's statevector simulator."""
    ...
    @unpack
    def test_construct_circuits(self, expectation, num_circuits):
    ...
    def test_basic_aer_qasm(self):
        """Test SSVQE on BasicAer's QASM simulator."""
    ...
    def test_with_aer_statevector(self):
        """Test SSVQE with Aer's statevector_simulator."""
    ...
    def test_with_aer_qasm(self):
        """Test SSVQE with Aer's qasm_simulator."""
    ...
    def test_with_aer_qasm_snapshot_mode(self):
        """Test SSVQE using Aer's qasm_simulator snapshot mode."""
        ...
        result = ssvqe.compute_eigenvalues(operator=self.test_op)
    ...
    def test_reuse(self):
        ...
        with self.subTest(msg="assert minimum eigensolver interface works"):
            result = ssvqe.compute_eigenvalues(operator=operator)
    ...
    def test_backend_change(self, user_expectation):
        """Test that SSVQE works when backend changes."""
        ssvqe = SSVQE(
            num_states=1,
            weight_vector=[1],
            ansatz=TwoLocal(rotation_blocks=["ry", "rz"], entanglement_blocks="cz"),
        ...
        result0 = ssvqe.compute_eigenvalues(operator=self.h2_op)
        if user_expectation is not None:
            with self.subTest("User expectation kept."):
                self.assertEqual(ssvqe.expectation, user_expectation)

        ssvqe.quantum_instance = BasicAer.get_backend("qasm_simulator")

        # works also if no expectation is set, since it will be determined automatically
        result1 = ssvqe.compute_eigenvalues(operator=self.h2_op)
    ...
    def test_set_ansatz_to_none(self):
        ...
        ssvqe.ansatz = None
        self.assertIsInstance(ssvqe.ansatz, RealAmplitudes)
    ...
    def test_set_optimizer_to_none(self):
        ...
        ssvqe.optimizer = None
        self.assertIsInstance(ssvqe.optimizer, SLSQP)
    ...
    def test_aux_operators_list(self):
        ...
        result = ssvqe.compute_eigenvalues(self.h2_op, aux_operators=aux_ops)
        np.testing.assert_array_almost_equal(
            result.eigenvalues.real, self.h2_energy_excited, decimal=2
        )
    ...
    def test_aux_operators_dict(self):
        ...
        wavefunction = self.ry_wavefunction
        ssvqe = SSVQE(ansatz=wavefunction, quantum_instance=self.statevector_simulator)
