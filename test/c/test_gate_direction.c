 ...
 * Test running CheckGateDirection on a simple circuit.
 */
static int test_dag_check_gate_direction(void) {
    QkTarget *target = create_target();
    if (!target)
        return RuntimeError;
 ...
 * Test running GateDirection on a simple circuit.
 */
static int test_dag_gate_direction_simple(void) {
    QkTarget *target = create_target();
    if (!target)
        return RuntimeError;
...
int test_gate_direction(void) {
    ...
    num_failed += RUN_TEST(test_dag_check_gate_direction);
    num_failed += RUN_TEST(test_dag_gate_direction_simple);
