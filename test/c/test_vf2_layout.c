 ...
 * Test running VF2Layout on a line connectivity
 */
static int test_dag_vf2_average_line(void) {
    const uint32_t num_qubits = 5;
    QkTarget *target = qk_target_new(num_qubits);
    int result = Ok;
    result = build_target(target, num_qubits);
    ...
    qk_vf2_layout_configuration_set_call_limit(layout_config, 10000, 10000);
    QkVF2LayoutResult *layout_result =
        qk_dag_transpiler_pass_standalone_vf2_layout_average(dag, target, layout_config, false);
    if (!qk_vf2_layout_result_has_match(layout_result)) {
        printf("%s: No layout was found\n", __func__);
        result = EqualityError;
        goto layout_cleanup;
 ...
 * Test VF2Layout where a solution isn't possible
 */
static int test_dag_vf2_no_layout_found(void) {
    const uint32_t num_qubits = 5;
    QkTarget *target = qk_target_new(num_qubits);
    int result = Ok;
    result = build_target(target, num_qubits);
    ...
    QkVF2LayoutResult *layout_result =
        qk_dag_transpiler_pass_standalone_vf2_layout_average(dag, target, NULL, false);
    if (qk_vf2_layout_result_has_match(layout_result)) {
        printf("%s: Unexpected layout found when one shouldn't be possible", __func__);
        result = EqualityError;
        goto layout_cleanup;
 ...
 * Test exact-match VF2 layout when no solution is possible because of available 1q operations.
 */
static int test_dag_vf2_exact_no_layout_1q_semantics(void) {
    int result = RuntimeError;
    uint32_t num_qubits = 5;
    uint32_t line[5] = {0, 1, 2, 3, 4};
    ...
    QkVF2LayoutResult *layout_result =
        qk_dag_transpiler_pass_standalone_vf2_layout_exact(dag, target, NULL);
    if (qk_vf2_layout_result_has_match(layout_result)) {
        printf("%s: unexpected match\n", __func__);
        result = EqualityError;
        goto cleanup;
 ...
 * though the structure of the graph matches.
 */
static int test_dag_vf2_exact_no_layout_2q_semantics(void) {
    int result = RuntimeError;
    uint32_t num_qubits = 5;
    uint32_t line[5] = {0, 1, 2, 3, 4};
 ...
 * match this interaction graph, no matter the nodes.
 */
static int test_dag_vf2_exact_no_layout_2q_structure(void) {
    int result = RuntimeError;
    uint32_t num_qubits = 5;
    uint32_t line[5] = {0, 1, 2, 3, 4};
 ...
 * Test exact-match VF2 layout when the existing layout is already as good as it can be.
 */
static int test_dag_vf2_exact_no_improvement(void) {
    int result = RuntimeError;
    uint32_t num_qubits = 5;
    uint32_t line[5] = {0, 1, 2, 3, 4};
 ...
 * Test exact-match VF2 layout when there is a better mapping to find.
 */
static int test_dag_vf2_exact_remap(void) {
    int result = RuntimeError;
    uint32_t num_qubits = 5;
    uint32_t line[5] = {0, 1, 2, 3, 4};
...
int test_vf2_layout(void) {
    ...
    num_failed += RUN_TEST(test_dag_vf2_average_line);
    num_failed += RUN_TEST(test_dag_vf2_no_layout_found);

    num_failed += RUN_TEST(test_dag_vf2_exact_no_layout_1q_semantics);
    num_failed += RUN_TEST(test_dag_vf2_exact_no_layout_2q_semantics);
    num_failed += RUN_TEST(test_dag_vf2_exact_no_layout_2q_structure);
    num_failed += RUN_TEST(test_dag_vf2_exact_no_improvement);
    num_failed += RUN_TEST(test_dag_vf2_exact_remap);
