 ...
 * Test running sabre layout that requires layout and routing
 */
static int test_dag_sabre_layout_applies_layout(void) {
    int result = Ok;
    ...
    options.seed = 2025;
    QkTranspileLayout *layout_result =
        qk_dag_transpiler_pass_standalone_sabre_layout(dag, target, &options);
 ...
 * Test running sabre layout that performs no transformation.
 */
static int test_dag_sabre_layout_no_swap(void) {
    int result = Ok;
...
int test_sabre_layout(void) {
    ...
    num_failed += RUN_TEST(test_dag_sabre_layout_no_swap);
    num_failed += RUN_TEST(test_dag_sabre_layout_applies_layout);
