...
static int test_dag_split_2q_unitaries_no_unitaries(void) {
    ...
    QkTranspileLayout *split_result =
        qk_dag_transpiler_pass_standalone_split_2q_unitaries(dag, 1 - 1e-16, true);
    int result = Ok;
    if (split_result != NULL) {
        result = EqualityError;
        printf("Permutation returned for a circuit that shouldn't split\n");
...
static int test_dag_split_2q_unitaries_x_y_unitary(void) {
    QkDag *dag = qk_dag_new();
...
static int test_dag_split_2q_unitaries_swap_x_y_unitary(void) {
    QkDag *dag = qk_dag_new();
    QkQuantumRegister *qr = qk_quantum_register_new(2, "qr");
    qk_dag_add_quantum_register(dag, qr);
    QkComplex64 c0 = {0., 0.};
...
int test_split_2q_unitaries(void) {
    ...
    num_failed += RUN_TEST(test_dag_split_2q_unitaries_no_unitaries);
    num_failed += RUN_TEST(test_dag_split_2q_unitaries_x_y_unitary);
    num_failed += RUN_TEST(test_dag_split_2q_unitaries_swap_x_y_unitary);
