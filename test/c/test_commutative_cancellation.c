...
static int test_dag_commutative_cancellation_target(void) {
    const uint32_t num_qubits = 5;
    QkTarget *target = qk_target_new(num_qubits);
    qk_target_add_instruction(target, qk_target_entry_new(QkGate_Z));
    qk_target_add_instruction(target, qk_target_entry_new(QkGate_SX));
...
static int test_dag_commutative_cancellation_no_target(void) {
    int result = Ok;
...
int test_commutative_cancellation(void) {
    ...
    num_failed += RUN_TEST(test_dag_commutative_cancellation_target);
    num_failed += RUN_TEST(test_dag_commutative_cancellation_no_target);
