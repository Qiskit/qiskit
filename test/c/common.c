...
bool compare_dags(const QkDag *res, const QkDag *expected) {
    size_t res_num_ops = qk_dag_num_op_nodes(res);
    size_t expected_num_ops = qk_dag_num_op_nodes(expected);
    if (res_num_ops != expected_num_ops) {
        printf("Number of op nodes in DAG is mismatched\n");
        return false;
    }
    uint32_t *res_ops = malloc(res_num_ops * sizeof(*res_ops));
    uint32_t *expected_ops = malloc(expected_num_ops * sizeof(*expected_ops));
    qk_dag_topological_op_nodes(res, res_ops);
    qk_dag_topological_op_nodes(expected, expected_ops);
    QkCircuitInstruction res_inst;
    QkCircuitInstruction expected_inst;
    for (size_t i = 0; i < res_num_ops; i++) {
        qk_dag_get_instruction(res, res_ops[i], &res_inst);
        qk_dag_get_instruction(expected, expected_ops[i], &expected_inst);
        int result = strcmp(res_inst.name, expected_inst.name);
        if (result != 0) {
            printf("Gate %zu have different gates %s was found and expected %s\n", i, res_inst.name,
                   expected_inst.name);
        ...
        for (uint32_t j = 0; j < res_inst.num_qubits; j++) {
            if (res_inst.qubits[j] != expected_inst.qubits[j]) {
                printf("Qubit %d for gate %zu are different %d was found and expected %d\n", j, i,
                       res_inst.qubits[j], expected_inst.qubits[j]);
                printf("Expected DAG instructions:\n");
                print_dag(expected);
                printf("Result DAG:\n");
                print_dag(res);
                qk_circuit_instruction_clear(&res_inst);
