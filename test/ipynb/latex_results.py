...
class Results:
    ...
    def summary(self):
        ...
        if len(self.mismatch) >= 2:
            Results._zipfiles(self.mismatch, "latex/mismatch.zip")
            ret += (
                '<div><a href="latex/mismatch.zip">'
                "Download %s mismatch results as a zip</a></div>" % len(self.mismatch)
            )

        if len(self.missing) >= 2:
            Results._zipfiles(self.missing, "latex/missing.zip")
            ret += (
                '<div><a href="latex/missing.zip">'
...
if __name__ == "__main__":
    ...
    RESULTS = Results(sorted(RESULT_FILES), "latex")
    RESULTS.diff_images()
