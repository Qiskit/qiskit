...
class Pickler(StockPickler):
    ...
    dispatch: typing.Dict[type, typing.Callable[[Pickler, typing.Any], None]] = MetaCatchingDict(
        StockPickler.dispatch.copy()
    )
...
def _create_code(*args):
    ...
    args = (
        fields[k].encode() if k in ENCODE_PARAMS and hasattr(fields[k], "encode") else fields[k]
...
def save_type(pickler, obj, postproc_list=None):
    if obj in _typemap:
        ...
        if not obj._field_defaults:
            ...
            defaults = [
                obj._field_defaults[field] for field in obj._fields if field in obj._field_defaults
            ]
            _save_with_postproc(
                pickler,
                (_create_namedtuple, (obj.__name__, obj._fields, obj.__module__, defaults)),
                obj=obj,
                postproc_list=postproc_list,
            )
    ...
    # special caes: NoneType, NotImplementedType, EllipsisType, EnumMeta, etc
        ...
        if (
            ...
            if not hasattr(obj, "__orig_bases__"):
                ...
                _save_with_postproc(
                    pickler,
                    (new_class, (obj.__name__, obj.__orig_bases__, _metadict, _dict_update)),
                    obj=obj,
                    postproc_list=postproc_list,
                )
...
if HAS_CTYPES and hasattr(ctypes, "pythonapi"):
    ...
    _testcapsule = _PyCapsule_New(
        ctypes.cast(_PyCapsule_New, ctypes.c_void_p), ctypes.c_char_p(_testcapsule_name), None
    )
