...
def _importable(obj, alias="", source=None, enclosing=False, force=True, builtin=True, lstrip=True):
    ...
    if source:  # first try to get the source
        try:
            return getsource(
                obj, alias, enclosing=enclosing, force=force, lstrip=lstrip, builtin=builtin
            )
        except Exception:
            pass
    try:
    ...
    for name, obj in list(free_vars.items()):
        ...
        if src.lstrip().startswith("@"):  # we have a decorator
            ...
            if not candidate:
                ...
                candidate = [
                    line for line in lines if getname(fobj) in line and re.match(pat, line)
                ]
    ...
    if not func_vars:
        ...
        if not mod or name is func:  # then it can be handled by getimport
            ...
            candidate = [
                line
                for line in lines
                if getname(name) in line and re.match(r".*[\w\s]=\s*" + getname(name) + r"\(", line)
            ]
            if not len(candidate):
                raise TypeError("import could not be found")
            candidate = candidate[-1]
