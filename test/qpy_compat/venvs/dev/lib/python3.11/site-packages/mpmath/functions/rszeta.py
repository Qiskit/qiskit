...
def _coef(ctx, J, eps):
    ...
    **Definition**
    ...
    neweps6 = eps / 2.0  # compute with a slight more precision that are needed
...
def Rzeta_simul(ctx, s, der=0):
    ...
    for n in range(1, L):
        ...
        for k in range(0, 3 * n // 2 + 1):
            ...
            if m != 0:
                ...
                c3 = -(m + 1)
                xd[0, n, k] = (
                    c3 * xd[0, n - 1, k - 2] + c1 * xd[0, n - 1, k] + c2 * xd[0, n - 1, k - 1]
                )
            else:
                xd[0, n, k] = 0
                for r in range(0, k):
    ...
    for mu in range(1, der + 1):
        for n in range(0, L):
            ctx.prec = xwpd[n] + 10
            for k in range(0, 3 * n // 2 + 1):
                aux = (2 * mu - 2) * xd[mu - 2, n - 2, k - 3] + 2 * (xsigma + n - 2) * xd[
                    mu - 1, n - 2, k - 3
                ]
    ...
    for n in range(1, L):
        ...
        for k in range(0, 3 * n // 2 + 1):
            ...
            if m != 0:
                ...
                c3 = -(m + 1)
                yd[0, n, k] = (
                    c3 * yd[0, n - 1, k - 2] + c1 * yd[0, n - 1, k] + c2 * yd[0, n - 1, k - 1]
                )
            else:
                yd[0, n, k] = 0
                for r in range(0, k):
    ...
    for mu in range(1, der + 1):
        for n in range(0, L):
            ctx.prec = ywpd[n] + 10
            for k in range(0, 3 * n // 2 + 1):
                aux = (2 * mu - 2) * yd[mu - 2, n - 2, k - 3] + 2 * (ysigma + n - 2) * yd[
                    mu - 1, n - 2, k - 3
                ]
        ...
        for k in range(0, L):
            ...
            for ell in range(0, 3 * k // 2 + 1):
                ...
                for mu in range(0, chi + 1):
                    tcoefter = (
                        ctx.binomial(chi, mu) * ctx.power(la, mu) * xfortcoef[chi - mu, k, ell]
                    )
    ...
    for chi in range(0, der + 1):
        for k in range(0, L):
            ...
            for ell in range(0, 3 * k // 2 + 1):
                ...
                for mu in range(0, chi + 1):
                    tcoefter = (
                        ctx.binomial(chi, mu) * ctx.power(la, mu) * yfortcoef[chi - mu, k, ell]
                    )
    ...
    **Definition**
    ...
    for mu in range(1, der + 1):
        for n in range(0, L):
            ctx.prec = wpd[n] + 10
            for k in range(0, 3 * n // 2 + 1):
                aux = (2 * mu - 2) * d[mu - 2, n - 2, k - 3] + 2 * (sigma + n - 2) * d[
                    mu - 1, n - 2, k - 3
                ]
...
def z_offline(ctx, w, k=0):
    ...
    if ctx._re(s2) >= 0:
        ...
        M2 = (
            4 * ctx._im(s2) * (2 * ctx.pi) ** (ctx._re(s2) - 1) * abs(1 - s2) ** (0.5 - ctx._re(s2))
        )
