...
def test_calculate_nome():
    ...
    math1 = [
        (mpf(1) / 10, mpf("0.006584651553858370274473060")),
...
def test_jtheta_issue_79():
    ...
    res = (
        mpf("2.4439389177990737589761828991467471")
    ...
    assert jtheta(3, 4.5, 0.25, 50).ae(-6.14832772630905e33)
    mp.dps = 50
...
def test_jtheta_complex():
    ...
    res = (
        mpf("0.31618034835986160705729105731678285")
    ...
    res = (
        mpf("1.6530986428239765928634711417951828")
    ...
    res = (
        mpf("1.6520564411784228184326012700348340")
    ...
    res = (
        mpf("0.37619082382228348252047624089973824")
...
def test_djtheta():
    ...
    res = (
        mpf("1.5555195883277196036090928995803201")
    ...
    res = (
        mpf("0.19825296689470982332701283509685662")
    ...
    res = (
        mpf("0.36492498415476212680896699407390026")
    ...
    res = (
        mpf("-0.38936892528126996010818803742007352")
...
def test_sn_cn_dn_complex():
    ...
    res = (
        mpf("0.99639490163039577560547478589753039")
...
def test_elliptic_integrals():
    ...
    for n in [5, 15, 30, 60, 100]:
        mp.dps = n
        assert elliprf(1, 2, 0).ae(
            "1.3110287771460599052324197949455597068413774757158115814084108519003952935352071251151477664807145467230678763"
        )
        assert elliprf(0.5, 1, 0).ae(
            "1.854074677301371918433850347195260046217598823521766905585928045056021776838119978357271861650371897277771871"
        )
        assert elliprf(j, -j, 0).ae(
            "1.854074677301371918433850347195260046217598823521766905585928045056021776838119978357271861650371897277771871"
        )
        assert elliprf(j - 1, j, 0).ae(
            mpc(
                "0.79612586584233913293056938229563057846592264089185680214929401744498956943287031832657642790719940442165621412",
                "-1.2138566698364959864300942567386038975419875860741507618279563735753073152507112254567291141460317931258599889",
            )
        )
        assert elliprf(2, 3, 4).ae(
            "0.58408284167715170669284916892566789240351359699303216166309375305508295130412919665541330837704050454472379308"
        )
        assert elliprf(j, -j, 2).ae(
            "1.0441445654064360931078658361850779139591660747973017593275012615517220315993723776182276555339288363064476126"
        )
        assert elliprf(j - 1, j, 1 - j).ae(
            mpc(
                "0.93912050218619371196624617169781141161485651998254431830645241993282941057500174238125105410055253623847335313",
        ...
        assert elliprc(-j, j).ae(
            mpc(
                "1.2260849569072198222319655083097718755633725139745941606203839524036426936825652935738621522906572884239069297",
        ...
        assert elliprc(j, -1).ae(
            mpc(
                "0.77778596920447389875196055840799837589537035343923012237628610795937014001905822029050288316217145443865649819",
                "0.1983248499342877364755170948292130095921681309577950696116251029742793455964385947473103628983664877025779304",
            )
        )
        assert elliprj(0, 1, 2, 3).ae(
            "0.77688623778582332014190282640545501102298064276022952731669118325952563819813258230708177398475643634103990878"
        )
        assert elliprj(2, 3, 4, 5).ae(
            "0.14297579667156753833233879421985774801466647854232626336218889885463800128817976132826443904216546421431528308"
        )
        assert elliprj(2, 3, 4, -1 + j).ae(
            mpc(
                "0.13613945827770535203521374457913768360237593025944342652613569368333226052158214183059386307242563164036672709",
                "-0.38207561624427164249600936454845112611060375760094156571007648297226090050927156176977091273224510621553615189",
            )
        )
        assert elliprj(j, -j, 0, 2).ae(
            "1.6490011662710884518243257224860232300246792717163891216346170272567376981346412066066050103935109581019055806"
        )
        assert elliprj(-1 + j, -1 - j, 1, 2).ae(
            "0.94148358841220238083044612133767270187474673547917988681610772381758628963408843935027667916713866133196845063"
        )
        assert elliprj(j, -j, 0, 1 - j).ae(
            mpc(
                "1.8260115229009316249372594065790946657011067182850435297162034335356430755397401849070610280860044610878657501",
        ...
        assert elliprj(-1 + j, -1 - j, 1, -3 + j).ae(
            mpc(
                "-0.61127970812028172123588152373622636829986597243716610650831553882054127570542477508023027578037045504958619422",
        ...
        assert elliprj(-1 + j, -2 - j, -j, -1 + j).ae(
            mpc(
                "1.8249027393703805304622013339009022294368078659619988943515764258335975852685224202567854526307030593012768954",
        ...
        assert elliprg(2, 3, 4).ae(
            "1.7255030280692277601061148835701141842692457170470456590515892070736643637303053506944907685301315299153040991"
        )
        ...
        assert elliprg(j - 1, j, 0).ae(
            mpc(
                "0.44660591677018372656731970402124510811555212083508861036067729944477855594654762496407405328607219895053798354",
        ...
        assert elliprg(-j, j - 1, j).ae(
            mpc(
                "0.36023392184473309033675652092928695596803358846377334894215349632203382573844427952830064383286995172598964266",
                "0.40348623401722113740956336997761033878615232917480045914551915169013722542827052849476969199578321834819903921",
            )
        )
        assert elliprg(0, mpf("0.0796"), 4).ae(
            "1.0284758090288040009838871385180217366569777284430590125081211090574701293154645750017813190805144572673802094"
        )
    mp.dps = 15

    # more test cases for the branch of ellippi / elliprj
    assert elliprj(-1 - 0.5j, -10 - 6j, -10 - 3j, -5 + 10j).ae(
        0.128470516743927699 + 0.102175950778504625j, abs_eps=1e-8
    )
    assert elliprj(1.987, 4.463 - 1.614j, 0, -3.965).ae(
        -0.341575118513811305 - 0.394703757004268486j, abs_eps=1e-8
    )
    assert elliprj(0.3068, -4.037 + 0.632j, 1.654, -0.9609).ae(
        -1.14735199581485639 - 0.134450158867472264j, abs_eps=1e-8
    )
    assert elliprj(0.3068, -4.037 - 0.632j, 1.654, -0.9609).ae(
        1.758765901861727 - 0.161002343366626892j, abs_eps=1e-5
    )
    assert elliprj(0.3068, -4.037 + 0.0632j, 1.654, -0.9609).ae(
        -1.17157627949475577 - 0.069182614173988811j, abs_eps=1e-8
    )
    assert elliprj(0.3068, -4.037 + 0.00632j, 1.654, -0.9609).ae(
        -1.17337595670549633 - 0.0623069224526925j, abs_eps=1e-8
    )
    ...
    # issue #571
    assert ellippi(2.1 + 0.94j, 2.3 + 0.98j, 2.5 + 0.01j).ae(
        -0.40652414240811963438 + 2.1547659461404749309j
    )
