...
def tridiag_eigen(ctx, d, e, z=False):
    ...
    for l in xrange(n):
        ...
        while 1:
            ...
            if j >= iterlim:
                raise RuntimeError(
                    "tridiag_eigen: no convergence to an eigenvalue after %d iterations" % iterlim
                )
            ...
            for i in xrange(m - 1, l - 1, -1):
                ...
                if abs(f) > abs(
                    g
                ):  # this here is a slight improvement also used in gaussq.f or acm algorithm 726.
                    c = g / f
                    r = ctx.hypot(c, 1)
                    e[i + 1] = f * r
                    s = 1 / r
...
def gauss_quadrature(ctx, n, qtype="legendre", alpha=0, beta=0):
    ...
    if qtype == "legendre":
        ...
        for i in xrange(1, n):
            ...
            d[i] = a2b2 / ((abi - 2) * abi)
            e[i] = ctx.sqrt(
                4 * j * (j + alpha) * (j + beta) * (j + ab) / ((abi * abi - 1) * abi * abi)
            )
...
def svd(ctx, A, full_matrices=False, compute_uv=True, overwrite_a=False):
    ...
    if iscomplex:
        return ctx.svd_c(
            A, full_matrices=full_matrices, compute_uv=compute_uv, overwrite_a=overwrite_a
        )
    else:
        return ctx.svd_r(
            A, full_matrices=full_matrices, compute_uv=compute_uv, overwrite_a=overwrite_a
