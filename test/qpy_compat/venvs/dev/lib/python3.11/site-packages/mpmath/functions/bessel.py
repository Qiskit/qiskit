...
def bessely(ctx, n, z, derivative=0, **kwargs):
    ...
    return (
        ctx.besselj(n, z, derivative, **kwargs) * cos - ctx.besselj(-n, z, derivative, **kwargs)
    ) / sin
...
def mcmahon(ctx, kind, prime, v, m):
    ...
    if prime:
        ...
        s5 = (
            -64
            * (6949 * u**4 + 296492 * u**3 - 1248002 * u**2 + 7414380 * u - 5853627)
            / (105 * (8 * b) ** 7)
        )
...
def generalized_bisection(ctx, f, a, b, n):
    ...
    while 1:
        ...
        ok_intervals = [
            (points[i], points[i + 1]) for i in range(N - 1) if signs[i] * signs[i + 1] == -1
        ]
