...
def jacobi(ctx, n, a, b, x, **kwargs):
    if not ctx.isnpint(a):
        ...
        def h(n):
            return (
                ([], [], [a + n + 1], [n + 1, a + 1], [-n, a + b + n + 1], [a + 1], (1 - x) * 0.5),
            )
...
def legenq(ctx, n, m, z, type=2, **kwargs):
    ...
    if type == 2:
        ...
        def h(n, m):
            ...
            u = m / 2
            w = (1 - z) / 2
            T1 = [s, c, a, b], [-1, 1, u, -u], [], [1 - m], [-n, n + 1], [1 - m], w
            T2 = [-s, a, b], [-1, -u, u], [n + m + 1], [n - m + 1, m + 1], [-n, n + 1], [m + 1], w
            return T1, T2
    ...
    if type == 3:
        ...
        if abs(z) > 1:
            ...
            def h(n, m):
                T1 = (
                    [ctx.expjpi(m), 2, ctx.pi, z, z - 1, z + 1],
...
def spherharm(ctx, l, m, theta, phi, **kwargs):
    ...
    if l_natural and m_isint:
        ...
        def h(l, m):
            ...
            return (
                (C, P, [], [], [absm - l, l + absm + 1], [absm + 1], ctx.sin(0.5 * theta) ** 2),
            )
