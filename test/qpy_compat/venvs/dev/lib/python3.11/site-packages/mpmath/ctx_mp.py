...
class MPContext(BaseMPContext, StandardBaseContext):
    ...
    def init_builtins(ctx):
        ...
        eps = ctx.constant(
            lambda prec, rnd: (0, MPZ_ONE, 1 - prec, 1), "epsilon of working precision", "eps"
        )
        ctx.eps = eps
                ...
                while 1:
                    ...
                    if prec2 >= maxprec2:
                        raise ctx.NoConvergence(
                            "autoprec: prec increased to %i without convergence" % prec2
                        )
                    prec2 += int(prec2 * 2)
    ...
    def hypsum(ctx, p, q, flags, coeffs, z, accurate_small=True, **kwargs):
        ...
        while 1:
            ...
            if magnitude_check:
                ...
                mag_dict = {}
            zv, have_complex, magnitude = summator(
                coeffs, v, prec, wp, epsshift, mag_dict, **kwargs
            )
            cancel = -magnitude
            jumps_resolved = True
