...
class levin_class:
    ...
    **Examples for nsum**
    ...
    def factor_levin(self, i):
        ...
        return (
            (self.theta + i)
            * (self.theta + self.n - 1) ** (self.n - i - 2)
            / self.ctx.mpf(self.theta + self.n) ** (self.n - i - 1)
        )
    ...
    def factor_sidi(self, i):
        ...
        return (
            (self.theta + self.n - 1)
            * (self.theta + self.n - 2)
            / self.ctx.mpf((self.theta + 2 * self.n - i - 2) * (self.theta + 2 * self.n - i - 3))
        )
        ...
        while 1:
            ...
            if TRY_EULER_MACLAURIN:
                if ctx.almosteq(ctx.mpc(ctx.sign(partial[-1]) / ctx.sign(partial[-2])), -1):
                    if verbose:
                        print(
                            "NOT using Euler-Maclaurin: the series appears"
