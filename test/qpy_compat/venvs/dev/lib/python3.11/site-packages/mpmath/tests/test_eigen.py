...
def test_eig():
    ...
    A = mp.matrix([[2, 1, 0], [0, 2, 1], [0, 0, 2]])  # jordan block of size 3
    AS.append(A)
    AS.append(A.transpose())

    A = mp.matrix([[2, 0, 0], [0, 2, 1], [0, 0, 2]])  # jordan block of size 2
    AS.append(A)
    AS.append(A.transpose())

    A = mp.matrix([[2, 0, 1], [0, 2, 0], [0, 0, 2]])  # jordan block of size 2
    AS.append(A)
    AS.append(A.transpose())

    A = mp.matrix([[0, 0, 1], [1, 0, 0], [0, 1, 0]])  # cyclic
    AS.append(A)
    AS.append(A.transpose())
