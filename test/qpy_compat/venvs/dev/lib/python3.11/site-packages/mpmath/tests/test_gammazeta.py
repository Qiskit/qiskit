...
def test_gamma():
    ...
    assert loggamma("1e10000j").ae(
        mpc("-1.570796326794896619231322e10000", "2.302485092994045684017991e10004")
    )
...
def test_gamma_quotients():
    ...
    assert beta("-255.5815971722918", "-0.5119253100282322").ae(
        "18.157330562703710339"
    )  # issue 421
...
def test_polygamma():
    ...
    mp.dps = 30
    # issue #534
    assert digamma(-0.75 + 1j).ae(
        mpc("0.46317279488182026118963809283042317", "2.4821070143037957102007677817351115")
    )
    mp.dps = 15
...
def test_gamma_huge_5():
    ...
    assert (
        str(y.real)
        == "-3.27753899634941e-227396058973640224580963937571892628368354580620654233316839"
    )
    assert (
        str(y.imag)
        == "-7.1519888950416e-227396058973640224580963937571892628368354580620654233316841"
    )
    mp.dps = 50
    y = gamma(x)
    assert str(y.real) == (
...
def test_bell_polyexp():
    ...
    mp.dps = 80
    assert (
        bell(50, -50) == 347469029146297202586097646631767227177164818163463279814268368579055777450
    )
    assert bell(40, 50) == 5575520134721105844739265207408344706846955281965031698187656176321717550
    assert (
        bell(74) == 5006908024247925379707076470957722220463116781409659160159536981161298714301202
    )
    mp.dps = 15
...
def test_rs_zeta():
    ...
    assert ae(
        fp.zeta(0.75 + 100000j, derivative=1), -0.41742276699594321475 - 6.4453816275049955949j
    )
