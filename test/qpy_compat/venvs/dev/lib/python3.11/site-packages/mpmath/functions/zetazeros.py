...
def separate_zeros_in_block(ctx, zero_number_block, T, V, limitloop=None, fp_tolerance=None):
    ...
    while (variations < zero_number_block) and (loopnumber < limitloop):
        ...
        loopnumber += 1
        if (
            (limitloop > ITERATION_LIMIT)
            and (loopnumber > 2)
            and (variations + 2 == zero_number_block)
        ):
            dtMax = 0
            dtSec = 0
            kMax = 0
...
def search_supergood_block(ctx, n, fp_tolerance):
    ...
    while number_goodblocks < 2 * sb:
        ...
        zn = len(T) - 1
        A, B, separated = separate_zeros_in_block(
            ctx, zn, T, V, limitloop=ITERATION_LIMIT, fp_tolerance=fp_tolerance
        )
        Tf.pop()
        Tf.extend(A)
        Vf.pop()
