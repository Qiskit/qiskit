...
def mpc_zetasum(s, a, n, derivatives, reflect, prec):
    ...
    if (
        a > 0
        and n > ZETASUM_SIEVE_CUTOFF
        and not have_derivatives
        and not reflect
        and (n < 4e7 or sys.maxsize > 2**32)
    ):
    ...
    xs = [
        (from_man_exp(xa, -wp, prec, "n"), from_man_exp(xb, -wp, prec, "n"))
        for (xa, xb) in zip(xre, xim)
    ]
    ys = [
        (from_man_exp(ya, -wp, prec, "n"), from_man_exp(yb, -wp, prec, "n"))
    ...
    if exp >= -1:
        ...
        if n < 100 or gamma_size < 10 * wp:
            if sign:
                ...
                if type == 3:
                    return mpf_log(
                        mpf_shift(from_rational(w, abs(f), prec, rnd), -wp + n + 1), prec, rnd
                    )
            elif n == 0:
                if type == 0:
