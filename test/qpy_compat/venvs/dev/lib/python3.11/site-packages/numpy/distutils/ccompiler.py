    ...
    if isinstance(self, FCompiler):
        ...
        for obj in objects:
            if obj in objects_to_build:
                ...
                if Path(
                    src
                ).suffix.lower() in FORTRAN_COMMON_FIXED_EXTENSIONS and not has_f90_header(src):
...
def CCompiler_customize_cmd(self, cmd, ignore=()):
    ...
    if (
        hasattr(self, "compiler")
        and "clang" in self.compiler[0]
        and not (platform.machine() == "arm64" and sys.platform == "darwin")
    ):
...
def CCompiler_customize(self, dist, need_cxx=0):
    ...
    if hasattr(self, "compiler") and (
        "gcc" in self.compiler[0] or "g++" in self.compiler[0] or "clang" in self.compiler[0]
    ):
