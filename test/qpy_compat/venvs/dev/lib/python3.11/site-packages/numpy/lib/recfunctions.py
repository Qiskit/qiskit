    ...
    * XXX: I just obtained these values empirically
    ...
    if usemask:
        ...
        for a, n in zip(seqarrays, sizes):
            ...
            seqdata.append(itertools.chain(data, [fval] * nbmissing))
        output = np.fromiter(
            tuple(_izip_records(seqdata, flatten=flatten)), dtype=newdtype, count=maxlength
        )
        if asrecarray:
            output = output.view(recarray)
    # And we're done...
...
def append_fields(base, names, data, dtypes=None, fill_value=-1, usemask=True, asrecarray=False):
    ...
    output = ma.masked_all(
        max(len(base), len(data)), dtype=_get_fieldspec(base.dtype) + _get_fieldspec(data.dtype)
    )
    output = recursive_fill_fields(base, output)
...
def structured_to_unstructured(arr, dtype=None, copy=False, casting="unsafe"):
    ...
    flattened_fields = np.dtype(
        {"names": names, "formats": dts, "offsets": offsets, "itemsize": arr.dtype.itemsize}
    )
...
def unstructured_to_structured(
    ...
    flattened_fields = np.dtype(
        {"names": names, "formats": dts, "offsets": offsets, "itemsize": out_dtype.itemsize}
    )
