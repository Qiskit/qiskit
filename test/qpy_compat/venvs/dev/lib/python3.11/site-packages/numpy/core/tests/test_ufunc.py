...
class TestUfunc:
    ...
    @pytest.mark.parametrize(
        "get_kwarg", [lambda dt: dict(dtype=x), lambda dt: dict(signature=(x, None, None))]
    )
    ...
    def test_sum_initial(self):
        ...
        assert_equal(
            np.sum(np.ones((2, 3, 5), dtype=np.int64), axis=(0, 2), initial=2), [12, 12, 12]
        )
    ...
    def test_sum_where(self):
        ...
        assert_equal(
            np.sum([[1.0, 2.0], [3.0, 4.0]], axis=0, initial=5.0, where=[True, False]), [9.0, 5.0]
        )
    ...
    @pytest.mark.parametrize(
        ["arr", "out"],
        [([2], np.empty(())), ([1, 2], np.empty(1)), (np.ones((4, 3)), np.empty((4, 1)))],
        ids=["(1,)->()", "(2,)->(1,)", "(4, 3)->(4, 1)"],
    )
    ...
    def test_keepdims_argument(self):
        ...
        w = np.arange(20, 26).reshape((2, 3))
        assert_array_equal(
            umt.innerwt(a, b, w, keepdims=True), np.sum(a * b * w, axis=-1, keepdims=True)
        )
        assert_array_equal(
            umt.innerwt(a, b, w, axis=0, keepdims=True), np.sum(a * b * w, axis=0, keepdims=True)
        )
        # Check errors.
        # Not a boolean
    ...
    def test_object_array_reduction(self):
        ...
        assert_equal(
            np.array([[1], [2, 3]], dtype=object).sum(initial=[0], where=[False, True]), [0, 2, 3]
        )
    ...
    def test_operand_flags(self):
        ...
        assert_equal(
            a,
            np.array([[0, 2, 4, 3], [7, 9, 11, 7], [14, 16, 18, 11], [12, 13, 14, 15]], dtype="l"),
        )
    ...
    def test_custom_ufunc_forced_sig(self):
        ...
        with assert_raises(TypeError):
            np.multiply(
                _rational_tests.rational(1), 1, signature=(_rational_tests.rational, int, None)
            )
...
def test_ufunc_out_casterrors():
    ...
    arr = np.array(
        [value] * int(np.BUFSIZE * 1.5) + ["string"] + [value] * int(1.5 * np.BUFSIZE), dtype=object
    )
...
def test_ufunc_input_casterrors(bad_offset):
    value = 123
    arr = np.array(
        [value] * bad_offset + ["string"] + [value] * int(1.5 * np.BUFSIZE), dtype=object
    )
    with pytest.raises(ValueError):
...
class TestLowlevelAPIAccess:
    ...
    @pytest.mark.skipif(
        not hasattr(ct, "pythonapi"), reason="`ctypes.pythonapi` required for capsule unpacking."
    )
