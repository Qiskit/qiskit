...
@pytest.mark.skipif(
    LD_INFO.precision + 2 >= repr_precision, reason="repr precision not enough to show eps"
)
...
def test_fromstring_complex():
    for ctype in ["complex", "cdouble", "cfloat"]:
        ...
        assert_equal(
            np.fromstring("1, 2 ,  3  ,4", sep=",", dtype=ctype), np.array([1.0, 2.0, 3.0, 4.0])
        )
...
def test_fromstring_bogus():
    with assert_warns(DeprecationWarning):
        assert_equal(
            np.fromstring("1. 2. 3. flop 4.", dtype=float, sep=" "), np.array([1.0, 2.0, 3.0])
        )
...
@pytest.mark.skipif(
    not (IS_MUSL and platform.machine() == "x86_64"), reason="only need to run on musllinux_x86_64"
)
