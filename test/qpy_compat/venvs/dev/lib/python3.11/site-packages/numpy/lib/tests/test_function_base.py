...
class TestAverage:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ([[1, 2, 5], [1, 6, 11]], 0, [[1.0, 4.0, 8.0]], [1, 3], [[1.0, 5.0, 9.5]], [[4, 4, 4]]),
        ],
    )
...
class TestSelect:
    ...
    def test_basic(self):
        ...
        assert_array_equal(
            select(conditions, choices, default=15), self._select(conditions, choices, default=15)
        )
...
class TestInsert:
    ...
    def test_multidim(self):
        ...
        assert_equal(insert(a, 1, a[:, :, 3], axis=-1), insert(a, 1, a[:, :, 3], axis=2))
        assert_equal(insert(a, 1, a[:, 2, :], axis=-2), insert(a, 1, a[:, 2, :], axis=1))
    ...
    def test_basic(self):
        ...
        for ctype in [
            ...
            if ctype in ["1", "b"]:
                ...
                assert_array_equal(
                    np.cumprod(a, axis=-1), np.array([1, 2, 20, 220, 1320, 6600, 26400], ctype)
                )
                assert_array_equal(
                ...
                assert_array_equal(
                    ...
                    np.array([[1, 2, 6, 24], [5, 30, 210, 1890], [10, 30, 120, 600]], ctype),
                )
...
class TestDiff:
    ...
    def test_subclass(self):
        x = ma.array(
            ...
            mask=[[False, False], [True, False], [False, True], [True, True], [False, False]],
        )
        out = diff(x)
...
class TestGradient:
    ...
    def test_args(self):
        ...
        assert_raises_regex(
            ValueError, ".*scalars or 1d", gradient, f_2d, np.stack([dx] * 2, axis=-1), 1
        )
...
class TestTrimZeros:
    ...
    @pytest.mark.parametrize(
        "arr", [np.array([0, 2**62, 0]), np.array([0, 2**63, 0]), np.array([0, 2**64, 0])]
    )
...
class TestCorrCoef:
    ...
    def test_empty(self):
        with warnings.catch_warnings(record=True):
            ...
            assert_array_equal(
                corrcoef(np.array([]).reshape(2, 0)), np.array([[np.nan, np.nan], [np.nan, np.nan]])
            )
...
class TestCov:
    ...
    def test_empty(self):
        with warnings.catch_warnings(record=True):
            ...
            assert_array_equal(
                cov(np.array([]).reshape(2, 0)), np.array([[np.nan, np.nan], [np.nan, np.nan]])
            )
    ...
    def test_aweights(self):
        ...
        assert_allclose(
            cov(self.x1, aweights=3.0 * self.weights), cov(self.x1, aweights=self.weights)
        )
    ...
    def test_unit_fweights_and_aweights(self):
        ...
        assert_allclose(
            cov(self.x1, fweights=self.frequencies, aweights=self.unit_weights), self.res2
        )
        assert_allclose(
            cov(self.x1, fweights=self.unit_frequencies, aweights=self.unit_weights), self.res1
        )
        assert_allclose(
            cov(self.x1, fweights=self.unit_frequencies, aweights=self.weights), self.res3
        )
        assert_allclose(
...
class TestKaiser:
    ...
    def test_simple(self):
        ...
        assert_almost_equal(
            kaiser(5, 1.0), np.array([0.78984831, 0.94503323, 1.0, 0.94503323, 0.78984831])
        )
        assert_almost_equal(
            kaiser(5, 1.56789), np.array([0.58285404, 0.88409679, 1.0, 0.88409679, 0.58285404])
        )
...
class TestPiecewise:
    ...
    def test_simple(self):
        ...
        assert_raises_regex(
            ValueError, "1 or 2 functions are expected", piecewise, [0, 0], [[False, True]], []
        )
        assert_raises_regex(
            ValueError,
    ...
    def test_0d_comparison(self):
        ...
        assert_raises_regex(
            ValueError, "2 or 3 functions are expected", piecewise, x, [x <= 3, x > 3], [1]
        )
        assert_raises_regex(
            ValueError, "2 or 3 functions are expected", piecewise, x, [x <= 3, x > 3], [1, 1, 1, 1]
        )
...
class TestPercentile:
    ...
    H_F_TYPE_CODES = [(int_type, np.float64) for int_type in np.typecodes["AllInteger"]] + [
    ...
    def test_extended_axis(self):
        ...
        assert_equal(
            np.percentile(x, [25, 60], axis=(0, 1, 2)), np.percentile(x, [25, 60], axis=None)
        )
        assert_equal(np.percentile(x, [25, 60], axis=(0,)), np.percentile(x, [25, 60], axis=0))

        d = np.arange(3 * 5 * 7 * 11).reshape((3, 5, 7, 11))
        np.random.shuffle(d.ravel())
        assert_equal(
            np.percentile(d, 25, axis=(0, 1, 2))[0], np.percentile(d[:, :, :, 0].flatten(), 25)
        )
        assert_equal(
        ...
        assert_equal(
            np.percentile(d, 25, axis=(3, 1, -4))[2], np.percentile(d[:, :, 2, :].flatten(), 25)
        )
        assert_equal(
            np.percentile(d, 25, axis=(3, 1, 2))[2], np.percentile(d[2, :, :, :].flatten(), 25)
        )
        assert_equal(
            np.percentile(d, 25, axis=(3, 2))[2, 1], np.percentile(d[2, 1, :, :].flatten(), 25)
        )
        assert_equal(
            np.percentile(d, 25, axis=(1, -2))[2, 1], np.percentile(d[2, :, :, 1].flatten(), 25)
        )
        assert_equal(
            np.percentile(d, 25, axis=(1, 3))[2, 2], np.percentile(d[2, :, 2, :].flatten(), 25)
        )
    ...
    def test_keepdims(self):
        ...
        assert_equal(np.percentile(d, 7, axis=(0, 1), keepdims=True).shape, (1, 1, 7, 11))
        assert_equal(np.percentile(d, 7, axis=(0, 3), keepdims=True).shape, (1, 5, 7, 1))
        assert_equal(np.percentile(d, 7, axis=(1,), keepdims=True).shape, (3, 1, 7, 11))
        assert_equal(np.percentile(d, 7, (0, 1, 2, 3), keepdims=True).shape, (1, 1, 1, 1))
        assert_equal(np.percentile(d, 7, axis=(0, 1, 3), keepdims=True).shape, (1, 1, 7, 1))

        assert_equal(np.percentile(d, [1, 7], axis=(0, 1, 3), keepdims=True).shape, (2, 1, 1, 7, 1))
        assert_equal(np.percentile(d, [1, 7], axis=(0, 3), keepdims=True).shape, (2, 1, 5, 7, 1))
    ...
    def test_nan_behavior(self):
        ...
        # axis02 not zerod
        b = np.percentile(np.arange(24, dtype=float).reshape(2, 3, 4), [0.3, 0.6], (0, 2))
        b[:, 1] = np.nan
        b[:, 2] = np.nan
        assert_equal(np.percentile(a, [0.3, 0.6], (0, 2)), b)
        # axis02 not zerod with method='nearest'
        b = np.percentile(
            np.arange(24, dtype=float).reshape(2, 3, 4), [0.3, 0.6], (0, 2), method="nearest"
        )
        b[:, 1] = np.nan
        b[:, 2] = np.nan
...
class TestQuantile:
    ...
    def test_quantile_identification_equation(self, method, alpha):
        ...
        if method in ("higher",):
            ...
            # "sample precision".
            assert_allclose(
                np.mean(self.V(x, y, alpha)), 0, atol=1 / n / np.amin([alpha, 1 - alpha])
            )
...
class TestLerp:
    @hypothesis.given(
        ...
        a=st.floats(allow_nan=False, allow_infinity=False, min_value=-1e300, max_value=1e300),
        b=st.floats(allow_nan=False, allow_infinity=False, min_value=-1e300, max_value=1e300),
    )
...
class TestMedian:
    ...
    def test_overwrite_keyword(self):
        ...
        assert_allclose(
            np.median(a4, axis=None), np.median(a4.copy(), axis=None, overwrite_input=True)
        )
