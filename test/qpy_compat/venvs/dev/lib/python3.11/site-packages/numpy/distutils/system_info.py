...
if sys.platform == "win32":
    ...
    default_lib_dirs = libpaths(
        ["/usr/local/lib", "/opt/lib", "/usr/lib", "/opt/local/lib", "/sw/lib"], platform_bits
    )
    default_runtime_dirs = []
    default_include_dirs = [
        "/usr/local/include",
        ...
        if env_var and env_var in os.environ:
            ...
            if os.path.isfile(d):
                ...
                if len(l) == 1:
                    ...
                    if b[:3] == "lib":
                        log.info(
                            "Replacing _lib_names[0]==%r with %r" % (self._lib_names[0], b[3:])
                        )
...
class blis_info(blas_info):
    ...
    def calc_info(self):
        ...
        incl_dirs = self.get_include_dirs()
        dict_append(
            info, language="c", define_macros=[("HAVE_CBLAS", None)], include_dirs=incl_dirs
        )
        self.set_info(**info)
    ...
    def calc_info(self):
        ...
        if sys.platform == "darwin" and not os.getenv("_PYTHON_HOST_PLATFORM", None):
            ...
            if args:
                ...
                self.set_info(
                    extra_compile_args=args, extra_link_args=link_args, define_macros=macros
                )
...
class _pkg_config_info(system_info):
    ...
    def calc_info(self):
        ...
        if version:
            macros.append(
                (self.__class__.__name__.split(".")[-1].upper(), _c_string_literal(version))
            )
