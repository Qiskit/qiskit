...
def get_docdir():
    ...
    return os.path.abspath(
        os.path.join(os.path.dirname(__file__), "..", "..", "..", "doc", "source", "f2py", "code")
    )


pytestmark = pytest.mark.skipif(
    not os.path.isdir(get_docdir()),
    reason=("Could not find f2py documentation sources" f" ({get_docdir()} does not exists)"),
)
...
class TestDocAdvanced(util.F2PyTest):
    ...
    sources = [_path("asterisk1.f90"), _path("asterisk2.f90"), _path("ftype.f")]
