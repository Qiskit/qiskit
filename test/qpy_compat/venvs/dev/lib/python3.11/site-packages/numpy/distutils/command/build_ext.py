...
class build_ext(old_build_ext):
    ...
    help_options = old_build_ext.help_options + [
        ("help-fcompiler", None, "list available Fortran compilers", show_fortran_compilers),
    ]
    ...
    def run(self):
        ...
        if self.distribution.has_c_libraries():
            if self.inplace:
                if self.distribution.have_run.get("build_clib"):
                    log.warn(
                        "build_clib already run, it is too late to "
        ...
        for ext in self.extensions:
            ...
            if c_libs:
                ...
                log.info(
                    "updating extension %r libraries from %r to %r"
            ...
            if not ext.language or has_fortran:
                if l and l != ext_language and ext.language:
                    log.warn(
                        "resetting extension %r language from %r to %r."
    ...
    def build_extension(self, ext):
        ...
        if fcompiler is not None:
            fcompiler.extra_f77_compile_args = (
                (ext.extra_f77_compile_args or []) if hasattr(ext, "extra_f77_compile_args") else []
            )
            fcompiler.extra_f90_compile_args = (
                (ext.extra_f90_compile_args or []) if hasattr(ext, "extra_f90_compile_args") else []
            )
        ...
        if cxx_sources and cxx_compiler is None:
            raise DistutilsError(
                "extension %r has C++ sources" "but no C++ compiler found" % (ext.name)
            )
        if (f_sources or fmodule_sources) and fcompiler is None:
            raise DistutilsError(
                "extension %r has Fortran sources " "but no Fortran compiler found" % (ext.name)
            )
        ...
        if not self.disable_optimization:
            ...
            for _srcs, _dst, _ext in (
                ...
                ((c_sources, cxx_sources), copt_cxx_sources, (".dispatch.cpp", ".dispatch.cxx")),
            ):
                for _src in _srcs:
        ...
        if c_sources:
            ...
            c_objects += self.compiler.compile(
                ...
                include_dirs=include_dirs,
                debug=self.debug,
                extra_postargs=(extra_args + copt_baseline_flags + extra_cflags),
                **kws,
            )
        if cxx_sources:
            log.info("compiling C++ sources")
            c_objects += cxx_compiler.compile(
                ...
                include_dirs=include_dirs,
                debug=self.debug,
                extra_postargs=(extra_args + copt_baseline_flags + extra_cxxflags),
                **kws,
            )
        ...
        if fmodule_sources:
            ...
            module_build_dir = os.path.join(
                self.build_temp, os.path.dirname(self.get_ext_filename(fullname))
            )
        ...
        if fcompiler is not None:
            objects, libraries = self._process_unlinkable_fobjects(
                objects, libraries, fcompiler, library_dirs, unlinkable_fobjects
            )
    ...
    def _add_dummy_mingwex_sym(self, c_sources):
        ...
        objects = self.compiler.compile(
            [os.path.join(build_src, "gfortran_vs2003_hack.c")], output_dir=self.build_temp
        )
    ...
    def _process_unlinkable_fobjects(
        self, objects, libraries, fcompiler, library_dirs, unlinkable_fobjects
    ):
        ...
        if unlinkable_fobjects:
            ...
            wrapped = fcompiler.wrap_unlinkable_objects(
                fobjects, output_dir=self.build_temp, extra_dll_dir=self.extra_dll_dir
            )
