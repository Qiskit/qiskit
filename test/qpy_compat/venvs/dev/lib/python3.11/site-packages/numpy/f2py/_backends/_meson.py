...
class MesonTemplate:
    ...
    def meson_build_template(self) -> str:
        if not self.build_template_path.is_file():
            raise FileNotFoundError(
                errno.ENOENT,
                "Meson build template" f" {self.build_template_path.absolute()}" " does not exist.",
            )
    ...
    def libraries_substitution(self) -> None:
        ...
        self.substitutions["lib_declarations"] = "\n".join(
            [f"{lib} = declare_dependency(link_args : ['-l{lib}'])" for lib in self.libraries]
        )
...
def _prepare_sources(mname, sources, bdir):
    ...
    extended_sources = [
        Path(source).name for source in extended_sources if not Path(source).suffix == ".pyf"
    ]
