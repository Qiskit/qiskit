...
class TestIntegration(base.BaseTestCase):
    ...
    def test_integration(self):
        ...
        venv = self.useFixture(
            test_packaging.Venv("sdist", modules=["pip", "wheel", PBRVERSION], pip_cmd=PIP_CMD)
        )
        python = venv.python
        self.useFixture(base.CapturedSubprocess("sdist", [python, "setup.py", "sdist"], cwd=path))
        venv = self.useFixture(
            test_packaging.Venv("tarball", modules=["pip", "wheel", PBRVERSION], pip_cmd=PIP_CMD)
        )
        python = venv.python
        ...
        python = venv.python
        self.useFixture(
            base.CapturedSubprocess("install-git", [python] + pip_cmd + ["git+file://" + path])
        )
        ...
        venv = self.useFixture(
            test_packaging.Venv("install-e", modules=["pip", "wheel", PBRVERSION], pip_cmd=PIP_CMD)
        )
        root = venv.path
        python = venv.python
...
class TestInstallWithoutPbr(base.BaseTestCase):
    ...
    def test_install_without_pbr(self):
        ...
        os.mkdir(dist_dir)
        self._run_cmd(
            sys.executable, ("setup.py", "sdist", "-d", dist_dir), allow_fail=False, cwd=PBR_ROOT
        )
        ...
        self._run_cmd(
            sys.executable, ("setup.py", "sdist", "-d", dist_dir), allow_fail=False, cwd=req_pkg_dir
        )
        # A venv to test within
        # We install setuptools because we rely on setup.py below.
        venv = self.useFixture(test_packaging.Venv("nopbr", ["pip", "wheel", "setuptools"]))
        python = venv.python
        # Install both packages
        self.useFixture(
            base.CapturedSubprocess("nopbr", [python] + ["setup.py", "install"], cwd=test_pkg_dir)
        )
        # Execute code that should only be present if the install worked.
        self.useFixture(
            base.CapturedSubprocess("nopbr", [python] + ["-m", "pkgReq"], cwd=test_pkg_dir)
        )
        pbr_cmd = os.path.join(venv.path, "bin", "pbr")
        self.useFixture(base.CapturedSubprocess("nopbr", [pbr_cmd] + ["freeze"], cwd=test_pkg_dir))
...
class TestMarkersPip(base.BaseTestCase):
    ...
    def test_pip_versions(self):
        ...
        pkgs = self._run_cmd(bin_python, ["-m", "pip", "freeze"], cwd=venv.path, allow_fail=False)[
            0
        ]
