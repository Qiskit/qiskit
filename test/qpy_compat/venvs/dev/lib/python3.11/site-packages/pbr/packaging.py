...
PY_REQUIREMENTS_FILES = [
    x % sys.version_info[0] for x in ("requirements-py%d.txt", "tools/pip-requires-py%d")
]
...
def parse_requirements(requirements_files=None, strip_markers=False):
    ...
    for line in get_reqs_from_files(requirements_files):
        ...
        if re.match(r"^\s*(-i|--index-url|--extra-index-url|--find-links).*", line):
            continue
        ...
        if line.startswith("-r"):
            ...
            requirements += parse_requirements([req_file], strip_markers=strip_markers)
            continue
...
def _get_version_from_git_target(git_dir, target_version):
    ...
    if target_version is not None and new_version > target_version:
        raise ValueError(
            ...
            "version is %(target)s" % dict(new=new_version, target=target_version)
        )
    if distance == 0:
        return last_semver
...
def get_version(package_name, pre_version=None):
    ...
    """
    version = os.environ.get("PBR_VERSION", os.environ.get("OSLO_PACKAGE_VERSION", None))
    if version:
        return version
