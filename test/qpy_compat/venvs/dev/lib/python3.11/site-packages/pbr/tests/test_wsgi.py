...
class TestWsgiScripts(base.BaseTestCase):
    ...
    def _get_path(self):
        ...
        return ".:%s/python%s.%s/site-packages" % (path, sys.version_info[0], sys.version_info[1])
    ...
    def _test_wsgi(self, cmd_name, output, extra_args=None):
        ...
        p = subprocess.Popen(
            popen_cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, cwd=self.temp_dir, env=env
        )
        ...
        print(stdoutdata)
        m = re.search(rb"(http://[^:]+:\d+)/", stdoutdata)
