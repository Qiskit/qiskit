...
from ._sputils import isshape, getdtype, getdata, to_native, upcast, get_index_dtype, check_shape
...
class bsr_matrix(_cs_matrix, _minmax_mixin):
    ...
    **Blocksize**
    ...
    def __init__(self, arg1, shape=None, dtype=None, copy=False, blocksize=None):
        ...
        if isspmatrix(arg1):
            ...
            if isshape(arg1):
                ...
                # (data,(row,col)) format
                self._set_self(
                    self._coo_container(arg1, dtype=dtype, shape=shape).tobsr(blocksize=blocksize)
                )
                ...
                if self.data.ndim != 3:
                    raise ValueError(
                        "BSR data must be 3-dimensional, got shape=%s" % (self.data.shape,)
                    )
                if blocksize is not None:
                    if not isshape(blocksize):
                        raise ValueError("invalid blocksize=%s" % (blocksize,))
                    if tuple(blocksize) != self.data.shape[1:]:
                        raise ValueError(
                            "mismatching blocksize=%s vs %s" % (blocksize, self.data.shape[1:])
                        )
            else:
    ...
    def check_format(self, full_check=True):
        ...
        *Parameters*:
        ...
        if len(self.indptr) != M // R + 1:
            raise ValueError(
                "index pointer size (%d) should be (%d)" % (len(self.indptr), M // R + 1)
            )
        if self.indptr[0] != 0:
    ...
    def getnnz(self, axis=None):
        if axis is not None:
            raise NotImplementedError("getnnz over an axis is not implemented " "for BSR format")
        R, C = self.blocksize
    ...
    def diagonal(self, k=0):
        ...
        _sparsetools.bsr_diagonal(
            k, rows // R, cols // C, R, C, self.indptr, self.indices, np.ravel(self.data), y
        )
        return y
    ...
    def _mul_vector(self, other):
        ...
        bsr_matvec(
            M // R, N // C, R, C, self.indptr, self.indices, self.data.ravel(), other, result
        )
    ...
    def _mul_sparse_matrix(self, other):
        ...
        idx_dtype = get_index_dtype((self.indptr, self.indices, other.indptr, other.indices))
    ...
    def _with_data(self, data, copy=True):
        ...
        if copy:
            return self.__class__(
                (data, self.indices.copy(), self.indptr.copy()), shape=self.shape, dtype=data.dtype
            )
        else:
            return self.__class__(
